...
void AbstractNumerical::printCode(
        int mode,
        std::ostream &out
        ) const
{
...
csl::Type Float::getType() const { return csl::Type::Float;}
...
long double Float::evaluateScalar() const { return value;}

optional<Expr> Float::evaluate(
        csl::eval::mode
        ) const
{
    return nullopt;
...
Expr Float::multiplication_own(const Expr& expr, bool) const
{
    ...
    switch(expr->getType()) {
        ...
        if (value == round(value))
            return intfraction_s(value*expr->getNum(),
                               expr->getDenom());
...
Expr Float::division_own(const Expr& expr) const
{
    ...
    switch(expr->getType()) {
        ...
        case csl::Type::IntFraction:
        if (value == round(value))
            return intfraction_s(value*expr->getDenom(),
                               expr->getNum());
...
optional<Expr> Integer::evaluate(
        csl::eval::mode
        )  const
{
    return nullopt;
...
Expr Integer::multiplication_own(const Expr& expr, bool) const
{
    ...
    if (expr->getType() == csl::Type::Complex)
        return expr->multiplication_own(copy());
    if (expr == nullptr or expr->getPrimaryType() != csl::PrimaryType::Numerical)
        return CSL_0;
...
optional<Expr> IntFraction::evaluate(
...
Expr IntFraction::exponentiation_own(const Expr& expr) const
{
    ...
    if (expr->isInteger() and value > 0) // Integer
        ...
        return intfraction_s(pow(denom,(int)(-value)),pow(num,(int)(-value)));
    else if (expr->getType() == csl::Type::Float) 
    ...
    imag(CSL_0)
{
    ...
    if (real->getType() == csl::Type::Complex)
        *this = Complex(GetRealPart(real),
                        GetImaginaryPart(real));
}
...
void Complex::checkNumerical(Expr const& expr) const
{
    CSL_ASSERT_SPEC(expr->getPrimaryType() == csl::PrimaryType::Numerical,
            CSLError::TypeError,
            "Expected numerical type in complex, " + toString(expr->getType())
            + " given.");
}
...
long double Complex::evaluateScalar() const
{
    CSL_ASSERT_SPEC(
            false, 
            CSLError::ValueError,
            "Cannot get value for a complex number. Use getRealPart() instead."
            )
    return real->evaluateScalar();
}

std::optional<Expr> Complex::evaluate(
        csl::eval::mode) const
{
    return std::nullopt;
}
...
{
    if (expr->getType() == csl::Type::NumericalEval)
        return csl::complex_s(
                expr->multiplication_own(real),
...
Expr Complex::division_own(const Expr& expr) const
{
    if (expr->getType() == csl::Type::NumericalEval)
        return csl::complex_s(
                real->division_own(expr),
                imag->division_own(expr)
                );
    if (expr->getType() != csl::Type::Complex) {
        return complex_s(real->division_own(expr),
                        imag->division_own(expr));
    }
