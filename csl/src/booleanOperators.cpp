...
namespace csl {
  ...
  BooleanOperator::BooleanOperator(
        ...
        :AbstractMultiFunc({A, B, exprIfTrue, exprIfFalse}),
        type(t_type)
    {
    }
  ...
  BooleanOperator::BooleanOperator(
        ...
        :AbstractMultiFunc({A, exprIfTrue, exprIfFalse}),
        type(t_type)
    {
    }
    ...
    {
        return std::all_of(argument.end() - 2, argument.end(),
            [](csl::Expr const &arg) {
                return arg->isReal();
            });
    }
    ...
    std::optional<csl::Expr> BooleanOperator::getComplexModulus() const 
    {
        auto cpy = *this;
        cpy.applyToArgs([](csl::Expr &arg){ 
            arg = arg->getComplexModulus().value_or(arg);
        });
        return cpy.copy();
    }
    ...
    std::optional<csl::Expr> BooleanOperator::getComplexArgument() const 
    {
        auto cpy = *this;
        cpy.applyToArgs([](csl::Expr &arg){ 
            arg = arg->getComplexArgument().value_or(arg);
        });
        return cpy.copy();
    }

    void BooleanOperator::print(
            int mode,
            std::ostream& out,
            bool lib
            ) const
    ...
    void BooleanOperator::printCode(
            int mode,
            std::ostream &out
            ) const 
    {
    ...
    std::optional<Expr> BooleanOperator::evaluate(
            ...
            )
    {
        ...
        return std::make_shared<BooleanOperator>(
            type, A, B, ifTrue, ifFalse
            );
    }
