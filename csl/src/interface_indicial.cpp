                           ...
                           int pos)
{
    ...
    CSL_ASSERT_SPEC(pos >= 0 and pos < int(spaces.size()),
            CSLError::IndexError,
            "Space in pos " + toString(pos) + " out of bounds for tensor with "
...
csl::Tensor Unitary(std::string const& name,
                    csl::Space   const* space)
{
    ...
    U->addSelfContraction(U({i, j}),
                          U({k, j}),
                          delta({i, k}),
                          [](Expr_info A, Expr_info B) {
                            return A->isComplexConjugate() 
                               xor B->isComplexConjugate();
                          });
    U->addSelfContraction(U({j, i}),
                          U({j, k}),
                          delta({i, k}),
                          [](Expr_info A, Expr_info B) {
                            return A->isComplexConjugate() 
                    ...
                    if (!found)
                        ...
                        if (verbose) {
                            std::cout << "Index " << structure[i]
                                << " appearing at least 3 times"<< std::endl;
                        }
                        return true;
                    }
                }
