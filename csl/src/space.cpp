    ...
    inverseMetric(delta)
{
    CSL_ASSERT_SPEC(not defaultIndexNames.empty(),
            CSLError::ValueError,
            "You must provide at least one default index name for the space "
            + name + ".");
        ...
        ) const
{
    ...
    out << indent << "    " << "\"" << csl::Abstract::regularLiteral(name) 
        << "\",\n";
    out << indent << "    " << dim << ",\n";
    if (signedIndex) {
        out << indent << "    " << "\"" 
...
void Space::buildEpsilon()
{
    ...
    if (dim < 5 and dim >= 2) {
        ...
        epsilon->addSelfContraction(
                epsilon(indices), epsilon(indices2),
                Expanded(det * sum_s(res)));
    }
}
...
bool Space::hasSpecialTraceProperty(const csl::vector_expr&) const
{
    callError(cslError::AbstractFuncCalled,
            "Space::hasSpecialTraceProperty(const csl::vector_expr& tensors) const");
    return false;
}
