...
Diagonalizer::Diagonalizer(
        const Matrix& matrix,
        bool          t_forceDeterminantZero)
    :forceDeterminantZero(t_forceDeterminantZero),
    init_matrix(matrix)
{
...
Diagonalizer::Diagonalizer(
        ...
        bool        t_forceDeterminantZero)
    :Diagonalizer(
            pointer_to_object_ref<Matrix>(matrix),
...
void Diagonalizer::diagonalizeNonHermitic() 
{
    ...
    init_matrix.print();
    init_matrix = pointer_to_object<Matrix>(
            init_save->hermitian()->dot(init_save));
    init_matrix.print();
    diagonalize();
    ...
    init_matrix = pointer_to_object<Matrix>(
            init_save->dot(init_save->hermitian()));
...
void Diagonalizer::checkSpecialCases()
{
    ...
    switch (init_matrix.size()) {
        case 1: diagonalize_1x1(); break;
        case 2: diagonalize_2x2(); break;
        case 3: diagonalize_3x3(); break;
    }
}
...
void Diagonalizer::diagonalize_2x2()
{
    ...
    if (Expr trial = c_star / (lambda_plus - b);
            trial < factor_plus)
        factor_plus = trial;

    Expr factor_minus = (lambda_minus - a) / c;
    if (Expr trial = c_star / (lambda_minus - b);
            trial < factor_minus)
        factor_minus = trial;
    ...
    D = pointer_to_object<Matrix>(diagonal_s({factor*lambda_plus,
                                         factor*lambda_minus}));
    U     = Matrix(2);
    U_inv = Matrix(2);
...
double Diagonalizer::extractNumber(const Expr& number)
{
    ...
    if (number->getType() == csl::Type::Prod
            and number->size() == 2
            and (*number)[1] == CSL_I)
...
gsl_matrix_complex* Diagonalizer::csl_to_gsl(
...
{
    return float_s(GSL_REAL(gsl_expr)) 
         + float_s(GSL_IMAG(gsl_expr)) * CSL_I;
}
