...
optional<Expr> Cos::getComplexModulus() const
{
    Expr re = GetRealPart(argument);
    Expr im = GetImaginaryPart(argument);
    return sum_s(prod_s(pow_s(cosh_s(im),CSL_2),
                        pow_s(cos_s(re),CSL_2)),
                 prod_s(pow_s(sinh_s(im),CSL_2),
                        pow_s(sin_s(re),CSL_2)));
}
...
optional<Expr> Tan::getRealPart() const
{
    Expr re = GetRealPart(argument);
    Expr im = GetImaginaryPart(argument);
    Expr modulus = sum_s(prod_s(pow_s(cosh_s(im),CSL_2),
                                pow_s(cos_s(re),CSL_2)),
                         prod_s(pow_s(sinh_s(im),CSL_2),
                                pow_s(sin_s(re),CSL_2)));
...
optional<Expr> Tan::getComplexModulus() const
{
    ...
    Expr modulus2 = sum_s(prod_s(pow_s(cosh_s(im),CSL_2),
                                 pow_s(sinh_s(im),CSL_2)),
                          prod_s(pow_s(sin_s(re),CSL_2),
                                 pow_s(cos_s(im),CSL_2)));
...
optional<Expr> ACos::getComplexModulus() const
{
    ...
    return sum_s(prod_s(pow_s(acosh_s(im),CSL_2),
                        pow_s(acos_s(re),CSL_2)),
                 prod_s(pow_s(asinh_s(im),CSL_2),
                        pow_s(asin_s(re),CSL_2)));
}
...
optional<Expr> Cosh::getComplexModulus() const
{
    Expr re = GetRealPart(argument);
    Expr im = GetImaginaryPart(argument);
    return sum_s(prod_s(pow_s(cos_s(im),CSL_2),
                        pow_s(cosh_s(re),CSL_2)),
                 prod_s(pow_s(sin_s(im),CSL_2),
                        pow_s(sinh_s(re),CSL_2)));
}
...
optional<Expr> Tanh::getRealPart() const
{
    Expr re = GetRealPart(argument);
    Expr im = GetImaginaryPart(argument);
    Expr modulus = sum_s(prod_s(pow_s(cos_s(im),CSL_2),
                                pow_s(cosh_s(re),CSL_2)),
                         prod_s(pow_s(sin_s(im),CSL_2),
                                pow_s(sinh_s(re),CSL_2)));
...
optional<Expr> Tanh::getComplexModulus() const
{
    ...
    Expr modulus2 = sum_s(pow_s(prod_s(sin_s(im),cos_s(im)),
                               CSL_2),
                          pow_s(prod_s(sin_s(im),cos_s(im)),
                               CSL_2));
...
optional<Expr> Tanh::derive(Expr_info expr) const
{
    return prod_s(Derived(argument, expr),
                  sum_s(CSL_1,
                        prod_s(CSL_M_1,pow_s(tanh_s(argument),
                                             CSL_2))));
}
...
optional<Expr> ACosh::derive(Expr_info expr) const
{
    return prod_s(Derived(argument, expr),
                  fraction_s(CSL_1,
                            pow_s(sum_s(pow_s(argument,CSL_2),
                                       CSL_M_1),
                                intfraction_s(1,2))));
}
...
{
    return make_unique<Angle>(argument[0],
                              argument[1]);
}
...
{
    return angle_s(argument[0]->refresh(),
                   argument[1]->refresh());
}

Expr Angle::deepRefresh() const
{
    return angle_s(argument[0]->deepRefresh(),
...
{
    if (!argument[0]->dependsOn(expr)
            and !argument[1]->dependsOn(expr))
        return CSL_0;
...
bool Angle::operator==(Expr_info expr) const
{
    ...
    if (expr->getType() != csl::Type::Angle)
        return false;
    return (*fraction_s(argument[0],
                       argument[1])==fraction_s(expr->getArgument(0),
                                               expr->getArgument(1)));
}
...
Expr DiracDelta::applyDiracDelta(const Expr& expr, const Expr& variable) const
{
    ...
    if (polynomial->getArgument(0)->dependsExplicitlyOn(variable.get())
        ...
        CALL_SMERROR_SPEC(CSLError::NotImplementedError,
                     "polynomial of degree higher than 2 in dirac delta.");
    }
    ...
    if (IsIndicialTensor(variable)) {
        ...
        if (argument->getType() == csl::Type::Sum) {
            for (int i = 0; i != argument->getNArgs(); ++i) {
                if (argument->getArgument(i)->isPolynomial(variable.get()) > 0) {
                    fact = GetPolynomialTerm(argument->getArgument(i),
                                             variable.get(),
                                             1);
...
bool DiracDelta::operator==(Expr_info other) const 
{
    ...
    if (expr->isIndexed() 
        ...
        IndexStructure freeStruct2 = 
            argument->getFreeIndexStructure();
    ...
    bool res = (Comparator::freeIndexComparison(argument.get(),
                                                expr->getArgument().get())
             or Comparator::freeIndexComparison(argument.get(),
                 Expanded((-1*expr->getArgument())).get()));
...
std::optional<Expr> Log::evaluate(csl::eval::mode user_mode) const
{
    ...
    if (arg->getPrimaryType() == csl::PrimaryType::Numerical
        ...
        return csl::Evaluated(
                log_s(GetComplexModulus(arg)) + CSL_I*GetComplexArgument(arg),
                user_mode
                );
    }
...
std::optional<Expr> Cos::evaluate(csl::eval::mode user_mode) const
{
    ...
    if (arg->getPrimaryType() == csl::PrimaryType::Numerical
        ...
        Expr b = GetImaginaryPart(arg);
        return csl::Evaluated(
                cos_s(a)*cosh_s(b) - CSL_I*sin_s(a)*sinh_s(b),
                user_mode
                );
    }
...
std::optional<Expr> Sin::evaluate(csl::eval::mode user_mode) const
{
    ...
    if (arg->getPrimaryType() == csl::PrimaryType::Numerical
        ...
        Expr b = GetImaginaryPart(arg);
        return csl::Evaluated(
                sin_s(a)*cosh_s(b) + CSL_I*cos_s(a)*sinh_s(b),
                user_mode
                );
    }
...
std::optional<Expr> Tan::evaluate(csl::eval::mode user_mode) const
{
    ...
    if (arg->getPrimaryType() == csl::PrimaryType::Numerical
        ...
        Expr b = GetImaginaryPart(arg);
        return csl::Evaluated(
                (sin_s(2*a) + CSL_I*sinh_s(2*b)) / (cos_s(2*a) + cosh_s(2*b)),
                user_mode
                );
    }
...
std::optional<Expr> Cosh::evaluate(csl::eval::mode user_mode) const
{
    ...
    if (arg->getPrimaryType() == csl::PrimaryType::Numerical
        ...
        Expr b = GetImaginaryPart(arg);
        return csl::Evaluated(
                cosh_s(a)*cos_s(b) + CSL_I*sinh_s(a)*sin_s(b),
                user_mode
                );
    }
...
std::optional<Expr> Sinh::evaluate(csl::eval::mode user_mode) const
{
    ...
    if (arg->getPrimaryType() == csl::PrimaryType::Numerical
        ...
        Expr b = GetImaginaryPart(arg);
        return csl::Evaluated(
                sinh_s(a)*cos_s(b) + CSL_I*cosh_s(a)*sin_s(b),
                user_mode
                );
    }
...
std::optional<Expr> Tanh::evaluate(csl::eval::mode user_mode) const
{
    ...
    if (arg->getPrimaryType() == csl::PrimaryType::Numerical
        ...
        Expr b = GetImaginaryPart(arg);
        return csl::Evaluated(
                (sinh_s(2*a) + CSL_I*sin_s(2*b)) / (cosh_s(2*a) + cos_s(2*b)),
                user_mode
                );
    }
