    ...
    if (sign == -1) {
        ...
        if (A->isAnOperator()) {
            ...
            if (not A->operatorAppliesOn(B.get())) 
                ...
                return CSL_UNDEF;
            //if (*Commutation(A->getArgument(), B) == CSL_0 and A->getOrder() == 1)
...
optional<Expr> Commutator::evaluate(
        ...
        ) const
{
    ...
    if (operator!=(com) and not (*com == CSL_UNDEF))
        ...
        return (argument[0]*argument[1]
                +sign*argument[1]*argument[0])->evaluate(user_mode);
}

unique_Expr Commutator::copy_unique() const
{
    return make_unique<Commutator>(argument[0],
                                   argument[1],
                                   sign);
}
...
{
    return csl::make_shared<Commutator>(argument[0]->deepCopy(),
                                   argument[1]->deepCopy(),
                                   sign);
}
...
{
    return commutator_(argument[0]->deepRefresh(),
...
Expr commutator_(const Expr& A, const Expr& B, int sign)
{
    ...
    if (sign == -1) {
        if (A->getType() == csl::Type::Prod) {
            ...
            return left*commutator_(right,B)
                 + commutator_(left,B)*right;
        }
            ...
            return left*commutator_(A,right)
                 + commutator_(A,left)*right;
        }
    }
