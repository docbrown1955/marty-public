...
class LiteralParent: public AbstractParent { 
    ...
    explicit
    LiteralParent(std::string const& t_name = "");
    ...
    virtual
    Expr generateInstance() = 0;

    private:

    virtual
    bool isInstance(Expr_info expr) const = 0;
...
class ConstantParent: public LiteralParent {
    ...
    explicit
    ConstantParent(std::string const& t_name = "");
...
class VariableParent: public LiteralParent {
    ...
    explicit
    VariableParent(std::string const& t_name = "");
    ...
    bool commutesWith(Expr_info expr,
                      int       sign = -1);
