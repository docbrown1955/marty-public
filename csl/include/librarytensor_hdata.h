...
void print_librarytensor_hdata(std::ostream &out) {
    ...
    out << "\n";
    out << "        static std::size_t getTotalDimension(std::vector<std::size_t> const& dimensions)\n";
    out << "        {\n";
    out << "            std::size_t tot = 1;\n";
    out << "            for (std::size_t d : dimensions)\n";
    out << "                tot *= d;\n";
    out << "            return tot;\n";
    out << "        }\n";
    out << "\n";
    out << "        LibraryTensor(std::vector<std::size_t> const& t_dimensions)\n";
    ...
    out << "        }\n";
    out << "     \n";
    out << "        std::vector<std::size_t> const& getDimensions() const {\n";
    ...
    out << "        }\n";
    out << "\n";
    out << "        std::size_t getIndex(std::vector<std::size_t> const& indices) const {\n";
    out << "            if (dimensions.size() == 1)\n";
    out << "                return indices[0];\n";
    out << "            std::size_t index = 0;\n";
    ...
    out << "#endif\n";
    out << "\n";
    out << "        Type& operator[](std::size_t pos) {\n";
    out << "            return data[pos];\n";
    out << "        }\n";
    out << "        Type const& operator[](std::size_t pos) const {\n";
    out << "            return data[pos];\n";
    out << "        }\n";
    out << "\n";
    out << "        Type& operator[](std::vector<std::size_t> const& indices) {\n";
    out << "            return data[getIndex(indices)];\n";
    out << "        }\n";
    out << "        Type const& operator[](std::vector<std::size_t> const& indices) const {\n";
    out << "            return data[getIndex(indices)];\n";
    out << "        }\n";
    out << "\n";
    out << "        public:\n";
    out << "\n";
    out << "        std::vector<std::size_t> dimensions;\n";
    out << "\n";
