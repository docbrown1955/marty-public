...
class Timer {
    ...
    static
    int getSecondsFromTime(std::clock_t time)
    {
        return time * 1./CLOCKS_PER_SEC;
    }

    static
    int getModulo(int& init, int modulo)
    {
        int res = init / modulo;
        init -= modulo * res;
        return res;
    ...
    static
    std::string getStringFromTime(int number, int nDigits = 2)
    {
    ...
    static
    std::string elapsedTime(std::clock_t time)
    {
        ...
        return getStringFromTime(nHours) + "h "
             + getStringFromTime(nMin)   + "m " 
             + getStringFromTime(nSec)   + "s "
