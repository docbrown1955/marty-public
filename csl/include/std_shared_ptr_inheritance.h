...
#define INHERIT_SHARED_PTR_CONSTRUCTOR(Class, type)\
    ...
    using _Constructible  \
        = std::shared_ptr<type>::_Constructible<Args...>; \
    template<class Arg> \
    using _Assignable  \
        = std::shared_ptr<type>::_Assignable<Arg>; \
 \
    constexpr Class(): std::shared_ptr<type>() {}; \
 \
    template< class Y , class = _Constructible<Y*> > \
    explicit Class( Y* ptr ) \
        :std::shared_ptr<type>(ptr) {} \
 \
    template< class Y, class Deleter,  \
        class = _Constructible<Y*, Deleter> > \
    Class( Y* ptr, Deleter d )  \
        :std::shared_ptr<type>(ptr, d) {} \
 \
 ...
 \
    constexpr Class( std::nullptr_t )  \
            :std::shared_ptr<type>(nullptr) {} \
 \
    template< class Deleter > \
    Class( std::nullptr_t, Deleter d )  \
        :std::shared_ptr<type>(nullptr, d) {} \
 \
 ...
 \
    template< class Y > \
    Class( const shared_ptr<Y>& r, type *ptr )  \
        :std::shared_ptr<type>(r, ptr) {} \
 \
    Class( const shared_ptr<type>& r )  \
            :std::shared_ptr<type>(r) {} \
 \
    template< class Y , class = _Constructible<const std::shared_ptr<Y>&> > \
    Class( const shared_ptr<Y>& r )  \
        :std::shared_ptr<type>(r) {} \
 \
    Class( shared_ptr<type>&& r )  \
            :std::shared_ptr<type>(r) {} \
 \
    template< class Y, class = _Constructible<std::shared_ptr<Y>> > \
    Class( shared_ptr<Y>&& r )  \
        :std::shared_ptr<type>(r) {} \
 \
    template< class Y, class = _Constructible<const std::weak_ptr<Y>&> > \
    explicit Class( const std::weak_ptr<Y>& r )  \
        :std::shared_ptr<type>(r) {} \
 \
    template< class Y, class Deleter,  \
