...
bool FermionChain::applyOn(csl::Expr& prod,
                           bool  makeAbbreviation) const
{
    ...
    for (const auto& chain : chains) {
        ...
        if (chainTensors.size() > 1 and
                not chainTensors[0]->isIndexed()) {
        ...
        if (chainTensors.size() > 0) {
            if (not makeAbbreviation) {
                ...
                (*prod)[chain.pos[0]] = factor * csl::Abbrev::makeAbbreviation(
