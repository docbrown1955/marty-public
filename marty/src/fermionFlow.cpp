...
std::pair< FieldList, FieldList > separatePartners(
...
bool compareFields(
        mty::QuantumField const &A,
        mty::QuantumField const &B
        )
{
...
FieldList::iterator getExternalBegin(
        FieldList &fields
        )
{
...
FieldList::iterator getInternalBegin(
        FieldList &fields
        )
{
    auto iter = fields.begin();
...
FieldList::iterator getPartner(
        FieldList::iterator A,
        FieldList          &fieldsA,
        FieldList          &partners
        )
{
...
csl::Index diracIndexOf(QuantumField const *field)
{
    ...
    CallHEPError(mty::error::IndexError,
            "Field " + toString(*field) + " has no Dirac index !")
    return csl::Index();
}

void testBeginExternalLeg(
        QuantumField const *ext,
        std::vector<ConjugationInfo> &info
        )
{
    if (ext->isExternal() 
            and ext->isComplexConjugate()) {
...
{
    if (!second->isComplexConjugate() 
            and first->isComplexConjugate()) {
        info.emplace_back(
                diracIndexOf(first),
                diracIndexOf(second),
                preIndex,
        ...
        )
{
    ...
    // if (!ruleMode)
    //     testPropagator(first, second, preIndex, postIndex, initialOrder, info);
}
...
std::vector<FermionLine> chainFermions(
        FieldList const &init)
{
...
void f(FieldList const &init,
        FieldList const &line)
{
    ...
    for (const auto &f : line)
        std::cout << *f << std::endl;
    std::cout << "Sign = " << wick::getCommutationSign(init, line) << std::endl;
}
        ...
        )
{
    ...
    std::vector<ConjugationInfo> res;
    ...
    for (std::size_t i = 0; i != info.size(); ++i) {
        ...
        for (std::size_t j = 0; j < info.size(); ++j)  {
            ...
            if (info[i].getConjugated() 
                        and info[i].getA() == info[j].getA()) {
                info[j].setInfo(
                        csl::Index(), info[j].getB(), 
                        info[j].getPreA(), info[j].getPostB()
                        );
                info.erase(info.begin() + i);
                --i;
                break;
...
void ConjugationInfo::applyConjugation(
        ...
        )
{
    ...
    {
        csl::IndexStructure structure = sub->getIndexStructure(
                &mty::dirac4
                );
        if (found)
            return true;
        if (structure.size() != 2
            ...
            return false;
        if (structure[0] == first
                and structure[1] != prev) {
            prev = first;
            first = structure[1];
            found = true;
            return true;
        }
        if (structure[1] == first
                and structure[0] != prev) {
            prev = first;
            first = structure[0];
            found = true;
            return true;
...
void ConjugationInfo::insertInChain(
        csl::Expr &expr
        ) const
{
    csl::Tensor C = mty::dirac4.C_matrix;
    bool foundA = (A == csl::Index()) or csl::FirstOfNode(expr, [&](csl::Expr &sub)
    {
        csl::IndexStructure structure = sub->getIndexStructure(
                &mty::dirac4
                );
    ...
    bool foundB = (B == csl::Index()) or csl::FirstOfNode(expr, [&](csl::Expr &sub)
    {
        csl::IndexStructure structure = sub->getIndexStructure(
                &mty::dirac4
                );
...
void ConjugationInfo::insertExternal(
            csl::Expr &expr
            ) const
    {
