        ...
        )
{
    ...
    csl::VisitEachLeaf(newExpression, [&](csl::Expr const &sub)
    {
        if (IsOfType<mty::QuantumField>(sub)) {
            auto parent = sub->getParent();
            auto localPart = std::dynamic_pointer_cast<QuantumFieldParent>(parent);
        ...
        std::function<bool(csl::Expr const&)> condition)
{
    ...
    HEPAssert(uSpaces.size() == 2
            and uSpaces[0] == uSpaces[1],
            mty::error::TypeError,
...
std::vector<csl::Expr> ModelBuilder::getFullMassMatrix(
        std::vector<mty::Particle> const &fields
        ) const
{
...
void ModelBuilder::applyRotation(
        mty::Particle const &field,
        ...
        )
{
    ...
    for (size_t i = 0; i != newFields1.size(); ++i)  {
        ...
        f2->setMass(mass);
        addLagrangianTerm(
                mty::MassTerm(mass, f1.get(), f2.get()), 
                false
                );
    }
...
void ModelBuilder::doSetGaugeChoice(
...
void ModelBuilder::doPromoteToGoldstone(
        mty::Particle& t_goldstone,
        ...
        )
{
    ...
    HEPAssert(gaugeBoson->getGaugeIrrep() == t_goldstone->getGaugeIrrep(),
            ...
            "(here " + toString(t_goldstone->getName()) + " has rep " 
            + toString(t_goldstone->getGaugeIrrep()) + ") as "
        ...
        )
{
    ...
    HEPAssert(gaugeBoson->getGaugeIrrep() == t_ghost->getGaugeIrrep(),
            ...
            "(here " + toString(t_ghost->getName()) + " has rep " 
            + toString(t_ghost->getGaugeIrrep()) + ") as "
            "its corresponding gauge boson (here " 
            + toString(gaugeBoson->getName()) 
            + toString(gaugeBoson->getGaugeIrrep()) + ").");
...
void ModelBuilder::integrateOutParticle(
...
{
    if (leftWeyl->getDiracParent() 
            or rightWeyl->getDiracParent())
        return;
        ...
        std::vector<mty::Lagrangian::TermType>   &interaction)
{
    ...
    for (size_t i = 0; i != interaction.size(); ++i) {
        ...
        csl::ForEachLeaf(term, [&](csl::Expr& expr)
        {
            ...
            mty::QuantumField *field = ConvertToPtr<mty::QuantumField>(expr);
...
void ModelBuilder::breakU1GaugeSymmetry(
        ...
        std::vector<std::string>              const &newFlavorNames)
{
    ...
    for (const auto &p : brokenFields)
        HEPAssert(p->getFlavorIrrep(brokenFlavor).getDim() > 1,
                mty::error::ValueError,
                "Field " + toString(p->getName()) + " has no representation "
                "for flavor " + toString(brokenFlavor->getName()) + " and can "
                "thus not be broken !")
    std::vector<mty::FlavorGroup*> newFlavorGroups;
    ...
    for (size_t i = 0; i != brokenFields.size(); ++i) {
        ...
        if (brokenFields[i]->hasFieldStrength())
            breakLagrangian(
                    brokenFields[i]->getFieldStrength(),
                    brokenFlavor->getFundamentalSpace(),
                    newSpaces);
        removeParticle(brokenFields[i]);
    }
        ...
        csl::vector_expr                  &newTerms)
{
    ...
    --i;
    lagrangian.insert(lagrangian.end(),
                      newMass.begin(),
                      newMass.end());
}
...
std::vector<mty::Particle> ModelBuilder::uniqueContent(
...
std::vector<mty::Particle>::const_iterator ModelBuilder::findInContent(
        ...
        )
{
    return std::find_if(
            content.begin(), 
            content.end(),
            [&](mty::Particle const &field2) {
                return field.get() == field2.get();
            });
}

bool ModelBuilder::fieldInContent(
        std::vector<mty::Particle> const &content,
        mty::Particle              const &field
        )
{
...
csl::Expr ModelBuilder::getMassMatrixOf(MassBlock const &block)
{
    ...
    for (auto &term : terms) {
        std::vector<mty::Particle> termContent
            = term->getParticles();
        ...
        if (termContent.size() == 1) {
            ...
            massMatrix[j1][j2] = massMatrix[j1][j2]
                                    - term->getGlobalFactor() / 2;
            massMatrix[j2][j1] = massMatrix[j2][j1]
                                    - term->getGlobalFactor() / 2;
        }
    }
    return massMatrix;
...
bool ModelBuilder::diagonalizeExplicitely(
...
bool ModelBuilder::doDiagonalizeSymbolically(
        mty::Particle const &field,
        bool                 forceDetZero
        )
{
    ...
    for (auto& block : blocks) {
        if (block.particles.size() == 2) {
            if (block.particles[0] != field
                    and block.particles[1] != field)
                continue;
            massBlock = block;
    ...
    mty::Particle new1
        = massBlock.particles[0].get()->generateSimilar(
...
void ModelBuilder::diagonalizeMassMatrices()
{
    ...
    for (const auto& block : blocks) {
        if (block.particles.size() == 2
            ...
            mty::Particle new1
                = block.particles[0].get()->generateSimilar(
...
void ModelBuilder::diagonalizeYukawa(
        ...
        for (size_t i = 0; i != initMix.size1(); ++i)
            for (size_t j = 0; j != initMix.size2(); ++j) {
                ...
                csl::ForEachLeaf(massTerm, [&](csl::Expr &sub) {
                    ...
                    else if (csl::Size(sub) == 0
                            and !csl::IsNumerical(sub)) {
...
void ModelBuilder::writeSpectrum(
        std::ostream &out,
        int           indentSize
        ) const
{
...
void ModelBuilder::addMassAbbreviations(
        csl::LibraryGenerator &lib
        )
{
