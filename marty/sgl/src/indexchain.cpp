...
namespace sgl {
    ...
    bool IndexChain::hasCommonIndex(IndexChain const &other) const
    {
        ...
        auto isConjugated = [](IndexChain const &chain, bool left) {
            return !chain.m_argument.empty()
                && isC(left ? chain.m_argument.front() : chain.m_argument.back());
        };
    ...
    bool IndexChain::hasPropertyWith(GExpr const &other) const
    {
        ...
        if (!IsType<IndexChain>(other)
                || ConvertTo<IndexChain>(other)->isTrace())
            return false;
            ...
            ) const
    {
        ...
        std::vector<GExpr> leftArgs(
                m_argument.begin(),
                m_argument.begin() + pos
                );
        std::vector<GExpr> rightArgs(
                m_argument.begin() + pos + len,
                m_argument.end()
                );
    ...
    std::optional<std::pair<size_t, size_t>> IndexChain::firstMove() const
    {
        ...
        if (first != m_argument.end()
                && nonChiral != m_argument.end()
                && first != m_argument.end() - 1
                && conjugation == m_argument.end()) {
            return std::make_pair(
                    std::distance(m_argument.begin(), first),
                    size() - 1
                    );
        }
    ...
    std::optional<std::pair<size_t, size_t>> IndexChain::lastMove() const
    {
        for (size_t i = 0; i+1 < m_argument.size(); ++i) {
            ...
            if (!IsType<GammaIndex>(m_argument[i])
                ...
                continue;
            if (ConvertTo<GammaIndex>(m_argument[j])->leftOf(
                        *ConvertTo<GammaIndex>(m_argument[i]))) {
            ...
            ) const
    {
        ...
        while (init != target) {
            ...
            auto first  = *dynamic_cast<GammaIndex const*>(current.m_argument[init].get());
            auto second = *dynamic_cast<GammaIndex const*>(current.m_argument[init+di].get());
            Commutation res = commute(
                    (di == 1) ? first : second,
                    (di == 1) ? second : first,
                    chain.getBorderIndices().first,
                    chain.getBorderIndices().second
    ...
    std::optional<GExpr> IndexChain::reduceMomenta(
    ...
    GExpr IndexChain::applyGeneralFierz(
        ...
        for (int c = 0; c != 5; ++c) {
            ...
            for (int d = 0; d != 5; ++d) {
                ...
                if (!tr->isZero()) {
                    res.push_back(
                            fc * fd / factor * tr
                            * expr1 * C_dual.copy() * expr2 * D_dual.copy()
                            );
    ...
    GExpr IndexChain::applyGeneralFierzTwice(
            IndexChain const &other,
            bool              chiral
            ) const
    {
            ...
            ) const
    {
        ...
        if (A.isGamma5()) {
            return cslexpr_s(
                    B.isP_R() ? CSL_1 : CSL_M_1
                    );
        }
        else {
            // Here B is gamma5 and A is a projector
            return cslexpr_s(
                    A.isP_R() ? CSL_HALF : CSL_M_HALF
                    );
        }
    }
    ...
    GExpr IndexChain::getContributionTo(IndexChain const &other) const
    {
        ...
        if (isAEnd && isBEnd)
            return cslexpr_s(CSL_1);
        if (isAEnd ^ isBEnd) {
            return identityFactor(*ConvertTo<GammaIndex>(
                        isAEnd ? *iterB : *iterA
                        ));
        }
        return factor(
                *ConvertTo<GammaIndex>(*iterA),
