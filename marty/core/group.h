...
typedef class AbstractGroup {
     ...
     */
    virtual 
    SemiSimpleAlgebra* getAlgebra() const;
     ...
     */
    virtual
    Irrep highestWeightRep(const std::vector<int>& highestWeight) const;
     ...
     */
    virtual
    const csl::Space* getVectorSpace(const Irrep& irrep) const = 0;
     ...
     */
    virtual
    AlgebraState getHighestWeight(const csl::Space* t_space) const;
     ...
     */
    virtual
    std::vector<const csl::Space*> getAllVectorSpace() const = 0;
    ...
    csl::Index getCorrespondingIndex(const std::string&        index_name, 
                                    const QuantumFieldParent& parent) = 0;
 ...
 * \note Two identical representations of two instances of SemiSimpleGroup (with 
 * the same algebra) will live in different vector spaces. For example, spin and
 * weak-isospin will not mix, the two csl::Space will be totally independant.
 */
class SemiSimpleGroup: public AbstractGroup {
     ...
     */
    Irrep highestWeightRep(
            const std::vector<int>& highestWeight) const override;
     ...
     */
    SumIrrep tensorProduct(const Irrep& rep1,
                           const Irrep& rep2) const;
     ...
     * SemiSimpleAlgebra::highestWeightRep() to instantiate representations from
     * their highest weight, and then computes the tensor product.
     * \param rep1 First representation in the product, caracterized by its
     * dinkin labels in a std::vector of integers.
     * \param rep2 Second representation in the product, caracterized by its
     * dinkin labels in a std::vector of integers.
     * \return A SumIrrep containing decomposition of the product in a direct
     * sum of irreducible representations.
     */
     ...
     * is created (csl::Space) and returned. The spaces created in this function
     * are specialized in color::ColorSpace in order to implement trace properties
     * in simplifications. See documentation of color::ColorSpace.
     * \note In this function only the highest weights of the irreps are 
     * compared. This means that a representation of \f$ F_4 \f$ may be 
     * successfully compared to a representation of \f$A_4\f$ because they have
     * the same number of labels.
     * \param irrep Representation from which we want the vector space.
     * \return A pointer to the color::ColorSpace of the representation (creates 
     * the space if it had not already been created).
     */
     ...
     * \details The QuantumFieldParent is important to know which representation
     * is wanted. For example in QCD, if 'g' is the gluon and 'q' a quark that 
     * are respectively in octet and triplet representations of a 
     * SemiSimpleGroup 'SU3_c',
     * \code 
     ...
     * \param index_name Name of the csl::Index.
     * \param parent     QuantumFieldParent in which we search a representation
     * of the group.
     * \return An index of name \b index_name living in the space of \b parent's
...
/*!
 * 
 * \brief Special unitary group. The algebra of \f$ SU(N) \f$ is \f$ A_{N-1} \f$
 * , see mty::algebra::A.
 */
 ...
 */
std::unique_ptr<mty::SemiSimpleGroup> createGroup(
        group::Type       type,
