...
struct enable_model {
    using type = typename std::enable_if<
        std::is_same<
            mty::Model,
            std::remove_reference<std::remove_cv<Type>>>::value
        >::type;
};
 ...
 * of csl::Minkowski, i.e. one of \f$ \mu, \nu, \rho, \sigma, \lambda, \tau \f$.
 * For example, a Minkowski index may be created with the following code:
 * \code
 ...
 * \details If no name is provided, the name will be chosen in the name cycle 
 * of csl::Minkowski, i.e. one of \f$ \mu, \nu, \rho, \sigma, \lambda, \tau \f$.
 * For example, Minkowski indices may be created with the following code:
 * \code
 ...
 * given in the form of a std::string, the name of the group. Finally, the field
 * is given as a mty::Particle. For example, for the standard model 'SM' (a 
 * mty::Model object), the 'SU3_c' group, one can get an index with the right
 * name in the octet representation of the gluon 'g' with the following code:
 * \code 
 ...
 * \details The model must be given to access the flavor group. The group can be
 * given in the form of a std::string, the name of the group. Finally, the field
 * is given as a mty::Particle.
 * @tparam GroupType mty::FlavorGroup* or valid type for the function getGroup()
 * @param model     Model that generated the index.
...
csl::Index FlavorIndex(mty::Model  const& model,
                      GroupType       && group)
{
    return model.generateIndex(
            std::forward<GroupType>(group)
            );
}
 ...
 * in a certain group.
 * \details The model must be given to access the gauge group. The group must be
 * given in the form of a std::string, the name of the group. Finally, the field
 * is given as a mty::Particle. For example, for the standard model 'SM' (a 
 * mty::Model object), the 'SU3_c' group, one can get an index with the right
 * name in the octet representation of the gluon 'g' with the following code:
 ...
 * representation in the model, in particular to generate indices in that space.
 * For example in QCD, a quark q and a gluon g would yield for \f$ SU(3) \f$
 * respectively a 3D and a 8D vector spaces. The code would look like:
 * \code 
 ...
 * \sa GenerateIndex, csl::Space, csl::Index, csl::TensorParent, csl::TensorElement.
 */
...
/*!
 * \brief Replaces an expression in all kinetic / mass / interactions terms of a 
 * model.
 * \param model  Model in which we do the replacement.
 * \param init   Expression to replace.
 * \param target New expression.
 */
 ...
 * \brief Replaces an expression in all kinetic / mass / interactions terms of a 
 * model.
 * \details The new expression should have a free index structure corresponding
 * to the tensor that is replaced.
 * \param model  Model in which we do the replacement.
 * \param init   Tensor to replace.
 * \param target New expression.
 */
void Replaced(mty::Model  & model,
             csl::Tensor& init,
 ...
 * \param model  Model in which we do the replacement.
 * \param init   Tensor to replace.
 * \param target New tensor, should have the same structure as the replaced one.
 */
void Replaced(mty::Model  & model,
             csl::Tensor& init,
             csl::Tensor& target);
 ...
 * them by rotating fields. For example, in a theory with two real scalar fields
 * \f$ \phi _1(X) \f$ and \f$ \phi _2(X) \f$ with a lagrangian containing
 * \f[
 *     \mathcal{L} \ni -\frac{1}{2}m^2(\phi _1^2 + 2\phi _1\phi _2 + \phi _2^2)
 ...
 * boson of name \b nameGroup. Depending on this choice, the propagator of gauge 
 * bosons is modified.
 * \details The propagator of a gauge boson is \f[
 * -i\frac{g_{\mu\nu} - (1-\xi)\frac{p_\mu p_\nu}{p^2 - \xi M^2}}{p^2-M^2}
 * \f], with \f$ M \f$ the mass of the 
 * particle. The Lorentz gauge corresponds to \f$ \xi=0\f$, the Feynman gauge to 
 * \f$ \xi=1\f$, the Unitary gauge to \f$ \xi=\infty\f$. Finally the choice NoGauge
 * let \f$ \xi\f$ free, for the so-called \f$\mathcal{R}_\xi\f$ gauge. For more
 * information see gauge::Type.
 * \param model        Model in which the particle is present.
 * \param nameParticle The name of the GaugeBoson to modify.
 * \param choice       New gauge fixing choice, element of 
 * gauge::Type.
 */
 ...
 * let \f$ \xi\f$ free, for the so-called \f$\mathcal{R}_\xi\f$ gauge. For more
 * information see gauge::Type.
 * \param particle Particle (must be a GaugeBoson) for gauge fixing.
 * \param choice   New gauge fixing choice, element of gauge::Type.
 */
