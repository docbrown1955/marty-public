...
/*!
 * 
 * \brief Handles the irreducible representation of a given semi-simple algebra.
 * \details The irrep is defined by a highest weight state, and all derived 
 * states (related by annihilation operators) with their multiplicities. This 
 * class is mostly a containor for the set of states (and their multiplicities)
 * in the representation. Computations are done by SemiSimpleAlgebra.
 */
class Irrep{
     ...
     * of all states in it.
     * \details This functions returns the total dimension of the representation
     * , counting multiplicities. This is the physically relevant number we use 
     * to define representations (triplet, octet, doublet, ...).
     * \return The dimension of the representation.
     */
    int getDim() const;

    /*!
     * \brief Returns the charge (csl::Expr, can be a fraction) of a \f$ U(1) \f$ 
     * representation.
     * \details This function should be called only for an Irrep of the \b R 
     * algebra, i.e. a \f$ U(1) \f$ representation. The function returns then 
     * a csl::Expr pointing to a csl::Integer (or a csl::IntFraction if the 
     * denominator is not one). The numerator is stored in highestWeight[0] 
     * and the denominator is stored in highestWeight[1].
     ...
     * SemiSimpleAlgebra::highestWeightRep() to have more informations about the
     * process. Note that taking the number of states in the Irrep does not 
     * yield its dimension as some states may have a multiplicity different than
     * \f$ 1 \f$. To have the dimension of the representation see 
     * Irrep::getDim().
     ...
     * \details Each multiplicity may be equal to 1 or greater. The multiplicity
     * of the highesst weight state (in first position) is always 1. There is a 
     * 1 to 1 correspondance between the multiplicities returned by this 
     * function and the AlgebraStates returned by Irrep::getRep(). Taking the 
     * sum of the multiplicities, one gets the total dimension of the Irrep (
     * also given by Irrep::getDim()):\f[d = \sum _{s}m_s,\f]
     * \f$ m_s \f$ the multiplicity of the state \f$ s \f$ summed over all 
     * states in the representation.
     * \return The set of multiplicities of all the states in the rep, \b mult.
     */
    std::vector<int> getMult() const;

    /*!
     * \brief Creates and returns the conjugated rep, i.e. the rep with inverted
     * dinkin labels.
     * \details For a reprentation of labels \f$ a_i \f$, \f$i\f$ from 0 to 
     * \f$l-1\f$, the conjugated representation has labels \f$ c_i = a_{l-i-1} 
     * \f$.
     * \return The conjugated representation.
     */
    ...
    /*!
     * \brief Implements the sum of the Irrep with another (\b other), stores it
     * in a SumIrrep and returns it.
     * \details For two representations \f$ \mathcal{R}_1 \f$ and
     * \f$ \mathcal{R}_2 \f$, the sum is simply \f$ \mathcal{R}_1\oplus
     * \mathcal{R}_2\f$, stored (and sorted by increasing dimension) in a 
     * SumIrrep.
     * \return \b True  if \b other has a dimension less than \b *this.
     * \return \b False else.
    ...
    /*!
     * \brief Implements the sum of the Irrep with a SumIrrep (\b other), stores
     * it in a SumIrrep and returns it.
     * \details Creates a SumIrrep similar to \b other and append to it the 
     * current Irrep *this. The new Irrep is then automatically inserted and 
     * sorted regarding its dimension. For example \f$(1\oplus 8\oplus 10)+8
     * = 1\oplus 8\oplus 8\oplus 10\f$.
     * \param other Second operand in the sum.
     * \return The SumIrrep of ***this** and **other**.
     */
     ...
     * \details For more details on how the decomposition is determined from the 
     * two initial Irrep objects, see SemiSimpleAlgebra::tensorProduct(). For 
     * example in \f$ SU(3) \f$ we have \f[ 8\times 8=1\oplus 8\oplus 8\oplus 10
     * \oplus 10\oplus 27.\f] The two initial Irrep of dimension 8 give a total
     * dimension of 64, decomposing into 5 irreducible representations of total
     * dimension \f$1+8+8+10+10+27=64\f$.
     * \param other Right operand in the product of representation.
     * \return The product decomposed in SumIrrep.
     */
    SumIrrep operator*(const Irrep& other) const;

    /*!
     * \brief Calculates and returns the product of ***this** and **other** in 
     * the form of a sum of irreducible representations. **other** being already
     * a sum, the product is developped before being calculated.
...
class SumIrrep: public std::vector<Irrep>{
     ...
     * \details For example, the dimension of \f$ 1\oplus 8\oplus8 \oplus 10 \f$
     * is 27.
     * \return The dimension of the SumIrrep.
     */
    int getDim() const;
