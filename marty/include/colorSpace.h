...
class ColorSpace: public csl::Space {
    ...
    csl::Expr applySingleTraceIdentity(
            TraceIndices const &trace,
    ...
    std::vector<TraceIndices> contractStructureConstants(
            TraceIndices const &indices,
            csl::Expr         const &CR,
            csl::Expr         const &CA) const;

    using Modifier = std::function<
    ...
    bool contractFarIndices(
            std::vector<TraceIndices> &indices,
            csl::Expr                const &CR,
            csl::Expr                const &CA) const;

    bool contractStructureConstants(
            std::vector<TraceIndices> &indices,
            csl::Expr                const &CR,
            csl::Expr                const &CA) const;
    ...
    std::vector<csl::Index> exprToIndices(
            std::vector<csl::Expr> const &tensors
            ) const;

    std::tuple<csl::Index, csl::Index, int> getFreeIndices(
            const csl::Expr& tensorA,
            const csl::Expr& tensorB
            ) const;

    std::tuple<csl::Index, csl::Index, int> getFreeIndicesAdjoint(
            const csl::Expr& tensorA,
            const csl::Expr& tensorB
            ) const;
