...
std::ostream &operator<<(std::ostream &out, ParticleType type)
{
    ...
    case ParticleType::Scalar:
        out << "scalar";
        break;
    case ParticleType::ChargedScalar:
        out << "charged scalar";
        break;
    ...
    case ParticleType::Gluon:
        out << "gluon";
        break;
    case ParticleType::Majorana:
        out << "";
        break;
    case ParticleType::Gaugino:
        out << "Gaugino";
        break;
    case ParticleType::Gluino:
        out << "Gluino";
        break;
...
LatexLinker::LatexLinker(Graph const &t_graph)
    : graph(t_graph), edges(0), nodes(t_graph.size())
{
    graph.center();
    graph.flip(1);
    graph.placeTopLeft();
        ...
        for (size_t k = 0; k != graph.size(); ++k)
            if (graph.getAdjacency()(i, k) == 2) {
                ...
                auto pos = std::find_if(
                    edges.begin(), edges.end(), [&](Edge const &e) {
...
void LatexLinker::setVertexName(size_t pos, std::string const &name)
{
    nodes[pos].name = name;
}

void LatexLinker::setEdgeName(size_t i, size_t j, std::string const &name)
{
    for (auto &e : edges)
        if (e.i == i and e.j == j) {
            e.name = name;
...
void LatexLinker::setEdgeSign(size_t i, size_t j, bool sign)
{
    for (auto &e : edges)
        if (e.i == i and e.j == j) {
            e.sign = sign;
...
void LatexLinker::write(std::ostream &out)
{
    ...
    for (size_t i = 0; i != graph.size(); ++i)
        writeVertex(out, i, graph.nodes[i], getNameVertex(i), nodes[i].name);
...
void LatexLinker::getFlippedEdgeLabels() const
{
    ...
    for (auto &e : edges)
        if (graph.getAdjacency()(e.i, e.j) > 1) {
            multiplicity[std::make_pair(std::min(e.i, e.j),
                                        std::max(e.i, e.j))]++;
            // // if (e.i < e.j)
            // //     mode = 1-mode;
            // e.curve = mode;
    ...
    const static std::array<double, 5> curvatureDef = {0, 1, -1, 0.5, -0.5};
...
void LatexLinker::getStringValue(std::ostream &out, double value)
{
    out << std::fixed << std::setprecision(2) << scaleFactor * std::abs(value);
}
    ...
    if (nameParticle.empty())
        ...
        for (size_t j = 0; j != graph.size(); ++j)
            if (i != j and graph.adjacency(i, j) != 0) {
                auto angleLabel = -int(
                    180 * (graph.nodes[i] - graph.nodes[j]).angle() / M_PI);
                out << "cm of " << nameVertex << "_it, label=" << angleLabel
                    << ":\\(" << nameParticle << "\\)](" << nameVertex
                    << ");\n";
                break;
            }
}
...
void LatexLinker::saveToNode(JSON::Node *node) const
{
    ...
    for (size_t i = 0; i != graph.getNodes().size(); ++i) {
        ...
        auto colorLeaf
            = JSON::Leaf<std::string>::make("color", nodes[i].color.toStr());
    ...
    for (const auto &e : edges) {
        ...
        auto colorLeaf
            = JSON::Leaf<std::string>::make("color", e.color.toStr());
...
void LatexLinker::loadFromNode(JSON::Node *node)
{
    ...
    if (graphNode) {
        for (const auto &point : *graphNode) {
            double x = *JSON::Parser::parseArgument<double>(point, "x", true);
            double y = *JSON::Parser::parseArgument<double>(point, "y", true);
            std::optional<int> op
    ...
    //        std::string name =
    //        JSON::Parser::interpretObject<std::string>(node.get());
...
void LatexLinker::saveMultiple(std::string const &             fileName,
...
std::vector<LatexLinker> LatexLinker::loadMultiple(std::string const &fileName)
{
    std::unique_ptr<JSON::Node> tree_unique
        = JSON::Reader::loadFromFile(fileName);
