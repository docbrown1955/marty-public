...
class Reader {
  ...
  private:
    ...
    /*!
     * \brief Raises an error when the end of the file is reached while
     * reading.
     */
     ...
     */
    static char
    getBooleanLeaf(Child &child, std::string const &specifier, char first);
     ...
     */
    static char
    getNumberLeaf(Child &child, std::string const &specifier, char first);
     ...
     */
    static char
    readObject(Child &child, std::string const &specifier, char first);
...
class Parser {
  ...
  public:
    ...
    static std::optional<Targ> parseArgument(Node *             node,
     ...
     */
    template <typename Targ>
    static std::optional<Targ>
    parseArgument(std::unique_ptr<Object> const &object,
                  std::string const &            specifier,
                  bool                           mandatory = false)
    {
     ...
     * This function is not recursive: We search in children only, not
     * grand-children.
     * \param parent    unique_ptr to the Object in which we search the
     * sub-Node. \param specifier Specifier to search. \param mandatory Tells
     * if the presence of the sub-Node is mandatory. If true and not found,
     * this function raises an error. \return A pointer to the sub-Node if
     * found. \return nullptr else.
     */
    ...
    static Targ interpretObject(Object *object)
    {
        JSONAssert(typeid(*object) == typeid(Leaf<Targ>),
                   "Bad type required in JSON::interpretObject for object \""
                       ...
                       + (std::string)(typeid(Leaf<Targ>).name())
                       + "\" expected.");
