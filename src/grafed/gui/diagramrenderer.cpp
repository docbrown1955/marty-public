...
DiagramRenderer::DiagramRenderer(QString const &nameFile, QWidget *parent)
...
void DiagramRenderer::init()
{
    ...
    connect(table,
            ...
            this,
            SLOT(diagramDoubleClicked(qint32, qint32)));

    connect(table,
            SIGNAL(cellClicked(int, int)),
            this,
            SLOT(cellPressed(int, int)));
}
...
void DiagramRenderer::setTable()
{
    ...
    table->cellWidget(iRow, iCol)
        ->setToolTip(QString("%1").arg(iRow * nColumns + iCol));
    table->update();
}
...
bool DiagramRenderer::setDiagram(qint32 i)
{
    ...
    diag->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
...
void DiagramRenderer::refreshLinks()
{
    ...
    if (offset + nDiags > nextDiagram) {
        links.insert(links.begin() + offset,
                     offset + nDiags - nextDiagram,
                     drawer::LatexLinker());
    }
    else if (offset + nDiags < nextDiagram) {
        links.erase(links.begin() + offset,
                    links.begin() + nextDiagram - nDiags);
    }
...
void DiagramRenderer::erase()
{
    ...
    QString message = (selectedDiagrams.size() == 1)
                          ...
                          : QString("Are you sure you want to delete these ")
                                + QString::number(selectedDiagrams.size())
                                + " diagrams?";
    ...
    if (ret == QMessageBox::Yes) {
        ...
        for (int i = 0; i != diagrams.size(); ++i) {
            if (iCol != iColNew or iRow != iRowNew) {
                if (diagrams[i]) {
                    diagrams[i]->moveTo(cellSize * iColNew,
                                        cellSize * iRowNew);
                    auto graph = new DiagramWidget(scene,
                                                   cellSize * iColNew,
                                                   cellSize * iRowNew,
                                                   this,
                                                   diagrams[i]);
                    ...
                    graph->resetTransform();
                    graph->centerScene();
                    graph->setHorizontalScrollBarPolicy(
                    ...
                    graph->setEnabled(false);
                    graph->update();
                    table->setCellWidget(iRowNew, iColNew, graph);
                }
            }
            if (!diagrams[i]) {
