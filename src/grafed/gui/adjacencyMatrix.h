...
class Base_iterable {
  ...
  public:
    ...
    Base_iterable() : elements(std::vector<Type>())
    {
    }

    explicit Base_iterable(size_t N) : elements(std::vector<Type>(N, Type()))
    {
    }

    explicit Base_iterable(std::vector<Type> const &t_elements)
        : elements(t_elements)
    {
    }

    explicit Base_iterable(std::vector<Type> &&t_elements)
        : elements(std::move(t_elements))
    {
    }

    virtual ~Base_iterable()
    {
    }
...
class Vector : public Base_iterable<T1, T2> {
  ...
  public:
    ...
    Vector(size_t N) : Base_iterable<T1, T2>(N)
    {
    }
...
template <class U1, class U2>
std::ostream &operator<<(std::ostream &out, Vector<U1, U2> const &vec)
{
...
class Matrix : public Base_iterable<T1, T2> {
  ...
  public:
    ...
    Matrix(size_t M) : Base_iterable<T1, T2>(M * M), N(M)
    {
    }

    Matrix(std::vector<Type> const &t_containor)
        : Base_iterable<T1, T2>(t_containor), N(t_containor.size())
    {
    }

    Matrix(std::vector<Type> &&t_containor)
        : Base_iterable<T1, T2>(std::move(t_containor)), N(t_containor.size())
    {
    }
...
template <class U1, typename U2>
std::ostream &operator<<(std::ostream &out, Matrix<U1, U2> const &mat)
{
...
Vector<T1, T2> operator+(Vector<T1, T2> const &vec, T3 value)
{
    Vector<T1, T2> copy(vec);
    for (auto icopy = copy.begin(), ivec = vec.begin(); icopy != copy.end();
         ++icopy, ++ivec)
        icopy *= value;
...
template <class T1, class T2, class T3>
Vector<T1, T2> operator+(T3 value, Vector<T1, T2> const &vec)
{
    Vector<T1, T2> copy(vec);
    for (auto icopy = copy.begin(), ivec = vec.begin(); icopy != copy.end();
         ++icopy, ++ivec)
...
template <class T1, class T2, class T3>
Matrix<T1, T2> operator+(Matrix<T1, T2> const &vec, T3 value)
{
...
template <class T1, class T2, class T3>
Matrix<T1, T2> operator+(T3 value, Matrix<T1, T2> const &vec)
{
    Matrix<T1, T2> copy(vec);
    for (auto icopy = copy.begin(), ivec = vec.begin(); icopy != copy.end();
         ++icopy, ++ivec)
...
T1 operator*(Vector<T1, T2> const &left, Vector<T3, T4> const &right)
{
...
template <class U1, class U2>
Matrix<U1, U2> operator*(Matrix<U1, U2> const &A, Matrix<U1, U2> const &B)
{
...
template <class U1, class U2>
Matrix<U1, U2> &operator*=(Matrix<U1, U2> &A, Matrix<U1, U2> const &B)
{
