...
class FeynmanDiagram {
  ...
  public:
     ...
     */
    std::vector<mty::Particle> const &getParticles(DiagramParticleType type
                                                   = Any) const;
     ...
     * @return \b False else.
     */
    bool contains(mty::Particle const &part,
                  DiagramParticleType  type = Any) const;
     ...
     *      bool isTriangleOrBox = diagram.isTopology(mty::Topology::Triangle |
     * mty::Topology::Box);
    ...
    {
        return contains(model->getParticle(std::forward<T>(part)), type);
    }
  ...
  private:
    ...
    {
        addParticle(model->getParticle(std::forward<T>(part)), type);
    }

    void
    loadParticlesFromVertices(std::vector<mty::wick::Vertex> const &vertices);
