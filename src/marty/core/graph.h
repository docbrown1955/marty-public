 ...
 * by several graphs) and the pointer to the partner is then a std::weak::ptr
 * to avoid a circular owning pattern.
 */
class Node : public std::enable_shared_from_this<Node> {
  ...
  public:
     ...
     */
    explicit Node(const mty::QuantumField *t_field) : field(t_field){};
    ...
    copyGraph(std::vector<std::shared_ptr<Node>> const &init);
...
class Vertex : public std::vector<std::shared_ptr<Node>> {
  ...
  public:
     ...
     * correlator).
     */
    explicit Vertex(const std::vector<const mty::QuantumField *> &fields,
                    size_t                                        id,
    ...
    /*!
     * \brief Returns the field associated to the first contractible Node of
     * the Vertex. This function calls an error if the Vertex is fully
     * contracted. \return The mty::QuantumField of the first contractible
     * Node.
     */
    ...
    /*!
     * \brief Returns the first Node contractible with \b field if there is
     * one. \param field mty::QuantumField we want to contract. \return A
     * shared_ptr to the the first Node contractible with \b field if ther is
     * one. \return \b nullptr else.
     */
    std::shared_ptr<Node>
    getFirstContractibleNode(const mty::QuantumField *field) const;
    ...
    getContractibleNodes(const mty::QuantumField *field) const;
     ...
     * the two final vertices have a pointer to the same Node in memory. This
     * is because in the contraction algorithm, a contracted Node do not change
     * at all after that. However, free nodes are really copied (the two
     * Vertices have two different Nodes) because in two different graphs, the
     * two Nodes can be contracted with different possible partners, and then
     * will be different. \param other Vertex to copy. \return A reference to
     * the modified Vertex.
     */
     ...
     */
    friend std::ostream &operator<<(std::ostream &fout, const Vertex &v);
...
class ConnectedComponent {
  ...
  public:
     ...
     * the calculation: only connected Graph are considered in Wick
     * contraction. \return \b True  if the component has free Node left.
     * \return \b False else.
     */
    bool hasValenceLeft() const;

    /*!
     * \brief This function returns the mty::QuantumField of the first
     * contractible Node if there is one. If no, this function calls an error.
     * \return The mty::QuantumField of the first contractible Node in the
     * component.
     */
    const mty::QuantumField *getFirstContractibleField() const;
     ...
     * \details The first constractible vertex is not put in the returned
     * vector except is its degeneracy (see Vertex::getDegeneracy()) is bigger
     * than one, i.e. has more than one Node contractible with \b field. This
     * is because at each contraction step we contract the first Node of the
     ...
     */
    std::vector<int>
    getContractibleVertices(const mty::QuantumField *field) const;
...
class Graph {
  ...
  public:
    ...
    /*!
     * \brief Constructor with one parameter.
     * \param field std::vector of mty::QuantumField on which apply
     * contractions.
     */
  ...
  public:
    ...
    getConnectedFieldsInVertex(const mty::QuantumField *init) const;

    const mty::QuantumField *
    getConnectedField(const mty::QuantumField *init) const;
  ...
  private:
    ...
    getSymmetry(const Vertex &vertex, const mty::QuantumField *field) const;
     ...
     */
    std::vector<int>
    getContractibleExtVertices(const mty::QuantumField *field) const;
     ...
     */
    std::vector<int>
    getContractibleIntVertices(const mty::QuantumField *field) const;
     ...
     */
    std::vector<int>
    getContractibleConnectedVertices(const mty::QuantumField *field) const;
  ...
  private:
    mutable std::vector<std::shared_ptr<Node>> sortedNodes;
    /*!
    ...
    mutable bool fullyConnected = false;
...
class WickCalculator {
  ...
  public:
     ...
     * \return \b False else.
     */
    static bool isContractionZero(const std::vector<mty::QuantumField> &fields,
                                  size_t maxLoops  = size_t(-1),
                                  size_t nVertices = size_t(-1));

    static bool
    isContractionZero(const std::vector<mty::QuantumField const *> &fields,
                      size_t maxLoops  = size_t(-1),
                      size_t nVertices = size_t(-1));
  ...
  private:
     ...
     * \param field Initial std::vector of mty::QuantumField to split in
     * species. \return The splitted std::vector into groups of
     * mty::QuantumField from the same species.
     */
    static std::vector<std::vector<mty::QuantumField>>
    splitFields(const std::vector<mty::QuantumField> &field);

    static std::vector<std::vector<mty::QuantumField const *>>
    splitFields(const std::vector<mty::QuantumField const *> &field);
  ...
  private:
     ...
     */
    explicit WickCalculator(const Graph &freeDiagram);
    ...
    getDiagrams(Model const *model, FeynOptions const &options);
...
/*!
 * \brief Tells if two mty::QuantumField are degenerate, i.e. have the **same
 * value** in the contraction sens. They must be the same mty::QuantumField and
 * have the same conjugation property. \details Example: \f$ \psi^*(X)_{\alpha}
 * \f$ and \f$ \psi(Y)_{\beta} \f$ are contractible together but not
 * degenerate. \f$ \psi^*(X)_{\alpha} \f$ and \f$ \psi^*(Y)_{\beta} \f$ are not
 ...
 * sets of mty::QuantumField. \f$ sgn = (-1)^{N_c}\f$, with \f$ N_c \f$ the
 * number of fermion permutations necessary to Transform \b A to \b B. \param A
 * Initial order of fields. \param B Same field content as \b A but in a
