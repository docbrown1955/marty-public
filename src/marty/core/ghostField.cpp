...
GhostBoson::GhostBoson(std::shared_ptr<VectorBoson> const &t_vectorBoson)
    : GhostBoson("c_" + std::string(t_vectorBoson->getName()), t_vectorBoson)
{
}

GhostBoson::GhostBoson(std::string const &                 t_name,
...
GhostBoson::GhostBoson(std::string const &t_name, GaugeIrrep const &rep)
    : ScalarBoson(t_name, rep)
{
    setDrawType(drawer::ParticleType::Ghost);
}

void GhostBoson::printDefinition(std::ostream &out,
                                 int           indentSize,
                                 bool          header) const
{
                      ...
                      bool               conjugated)
{
    ...
    return ghostboson_s(
        name, ConvertToShared<VectorBoson>(t_vectorBoson), conjugated);
}

Particle ghostboson_s(Particle const &t_vectorBoson, bool conjugated)
{
    HEPAssert(IsOfType<VectorBoson>(t_vectorBoson)
                  or IsOfType<GaugeBoson>(t_vectorBoson),
              mty::error::TypeError,
