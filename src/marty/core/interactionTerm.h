 ...
 * abstract representation to lighten calculations of Feynman rules (that is
 * the full quantum calculation of tree-level matrix elements). It is used as
 * the term container in the mty::Lagrangian.
 *
...
class InteractionTerm {
  ...
  public:
     ...
     *
     * @details If enabled, abbreviations will be introduced for all
     * interaction terms. This can save some calculations but will make the
     * lagrangian much less readable.
     */
  ...
  public:
    static void fillContent(csl::Expr const &               prod,
    ...
    /**
     * @brief Returns a const reference to the field content of the
     * interaction.
     *
     * @return #content
     */
    ...
    /**
     * @brief Counts, in the content, how many particles are of a given
     * species.
     *
     ...
     * \code
     *      csl::Expr fullExpression =
     * applyFactorAndSymmetriesOn(getFieldProduct()); \endcode
     *
     ...
     * this function on the result to recover couplings, index contractions
     * etc.
     *
     * @param init Initial expression on which factors and symmetries must be
     * applied.
     *
     * @return The expression on which factors and symmetries have been
     * applied.
     *
     * @sa getFieldProduct()
     */
     ...
     *  \f$ \psi \f$ contains \f$ \psi_L \f$ but \f$ \psi_L \f$ does not
     * contain \f$ \psi \f$. Otherwise, this function is equivalent to
     *  containsExactly().
     *
  ...
  private:
    ...
    std::pair<csl::Expr, csl::IndexStructure>
    buildContent(QuantumField expr, csl::IndexStructure const &structure);

    static void recallIndices(permutation &              perm,
    ...
    static bool areSimilarContent(std::vector<QuantumField> const &A,
                                  std::vector<QuantumField> const &B);

    static csl::Expr
    applyPermutation(permutation const &               perm,
                     csl::Expr const &                 init,
