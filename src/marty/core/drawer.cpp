...
drawer::LatexLinker Drawer::buildDiagram(
    std::shared_ptr<wick::Graph> const &graph, bool external, bool showMomenta)
{
    ...
    for (size_t i = 0; i != N; ++i) {
        if (vertices[i][0]->field->isExternal()) {
            std::string name = std::string(
                vertices[i][0]->field->getPoint()->getLatexName());
            for (char &c : name)
                if (c == 'X')
                    c = 'p';
            link.setVertexName(
                     ...
                     std::map<size_t, size_t>           &mapping)
{
    ...
    for (size_t i = 0; i != N; ++i) {
        if (vertices[i][0]->field->isExternal()) {
            newGraph.setSide(mapping[i],
                             (vertices[i][0]->field->isIncoming() ? -1 : 1));
        }
        else
            newGraph.setSide(mapping[i], 0);
    }
    ...
    for (size_t i = 0; i != N; ++i) {
        if (vertices[i][0]->field->isExternal()) {
            ...
            link.setVertexName(
                mapping[i],
                std::string(vertices[i][0]->field->getLatexName()) + "(" + name
                    + ")");
        }
        for (size_t j = 0; j != vertices[i].size(); ++j)
            for (size_t k = j + 1; k < vertices[i].size(); ++k)
                ...
                for (const auto &nodeB : vertices[j])
                    if (nodeA->partner.lock() == nodeB) {
                        ...
                        if (!nodeA->field->isComplexConjugate())
                            ...
                            link.setParticlesType(
                                mapping[i], mapping[j], type, edgeName, true);
                    }
        }
    }
    return link;
...
std::vector<drawer::LatexLinker>
Drawer::buildDiagrams(std::vector<std::shared_ptr<wick::Graph>> const &graphs,
                      bool showMomenta)
{
    for (const auto &g : graphs)
        HEPAssert(g, mty::error::ValueError, "Null graph encountered ! ")
            std::vector<drawer::LatexLinker>
                links(graphs.size());
...
void Drawer::launchViewer(
    std::vector<std::shared_ptr<wick::Graph>> const &graphs, bool showMomenta)
{
...
std::string Drawer::buildLatex(std::shared_ptr<wick::Graph> const &graph)
{
...
void Drawer::exportPDF(std::string const &,
                       std::vector<std::shared_ptr<wick::Graph>> const &graphs,
                       std::string const &)
{
    if (graphs.empty())
...
drawer::ParticleType Drawer::getType(mty::QuantumField const &field)
{
    ...
    if (spinDim == 1)
        return field.isSelfConjugate() ? drawer::ParticleType::Scalar
                                       : drawer::ParticleType::ChargedScalar;
    if (spinDim == 2)
        return field.isSelfConjugate() ? drawer::ParticleType::Majorana
                                       : drawer::ParticleType::Fermion;
    if (spinDim == 3)
...
drawer::Graph Drawer::convertGraph(std::shared_ptr<wick::Graph> const &graph,
                                   bool minimize,
                                   bool external)
{
    ...
    for (size_t i = 0; i != N; ++i) {
        ...
        for (const auto &nodeA : vertices[i]) {
            ...
            for (size_t j = 0; j != N; ++j)
                for (const auto &nodeB : vertices[j]) {
                    auto pos = std::find(
                        alreadySeen.begin(), alreadySeen.end(), nodeB);
