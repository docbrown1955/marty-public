 ...
 * mty::FeynOptions::LagrangianFilter and mty::FeynOptions::DiagramFilter,
 * using the documentation of mty::InteractionTerm and mty::FeynmanDiagram
 * objects. Writing the above filters by hand could look like (using C++ lambda
 * functions): \code mty::FeynOptions options; Particle Z =
 * model.getParticle("Z"); Particle t = model.getParticle("t");
 *      options.addFilters(
 *          [&](mty::InteractionTerm const &term) { // disable Z,
 * mty::FeynOptions::LagrangianFilter return !term.contains(Z); // keep if Z
 * absent
 *          },
 *          [&](mty::FeynmanDiagram const &diagram) { // force t,
 * mty::FeynOptions::DiagramFilter return diagram.contains(t); // keep if t
 * present
 *          }
 *      );
 *      auto res = model.computeAmplitude(order, insertions, options);
 ...
 * mty::FeynOptions::DiagramFilter but this would imply unnecessary
 * calculations because vertices can be filtered before searching for any
 * diagram containing it. However forcing the top quark must be a
 * mty::FeynOptions::DiagramFilter because one cannot remove vertices that
 * contain the top quark, the condition is that the whole diagram must contain
...
auto disableParticles(ParticleIterator first, ParticleIterator last)
{
    return [particles = std::vector<mty::Particle>(first, last)](
        ...
        return std::all_of(
            begin(particles), end(particles), [&](Particle const &p) {
 ...
 * particles contained in the \b particles range.
 */
inline auto disableParticles(std::initializer_list<std::string_view> names,
 ...
 */
inline auto disableParticle(std::string_view  name,
                            mty::Model const &model = *Model::current)
{
 ...
 */
inline auto
disableParticleCombination(std::vector<mty::Particle> const &particles)
{
 ...
 */
inline auto
disableParticleCombination(std::initializer_list<mty::Particle> particles)
{
    return disableParticleCombination(begin(particles), end(particles));
}
 ...
 */
inline auto
disableParticleCombination(std::initializer_list<std::string_view> names,
                           mty::Model const &model = *Model::current)
{
 ...
 */
inline auto
forceParticleCombination(std::vector<mty::Particle> const &particles)
{
 ...
 */
inline auto
forceParticleCombination(std::initializer_list<mty::Particle> particles)
{
    return forceParticleCombination(begin(particles), end(particles));
}
 ...
 */
inline auto
forceParticleCombination(std::initializer_list<std::string_view> names,
                         mty::Model const &model = *Model::current)
{
 ...
 * do not contain the given particle.
 */
inline auto forceParticle(std::string_view  name,
                          mty::Model const &model = *Model::current)
{
 ...
 * not contain any of the particles given in the range.
 */
inline auto forceParticles(std::initializer_list<std::string_view> names,
                           mty::Model const &model = *Model::current)
{
