...
std::vector<std::vector<size_t>> Expander::findNonZeroDiagrams()
{
    ...
    if (!options.getFeynRuleCalculation() && mty::option::testRepresentations
    ...
    std::vector<std::vector<size_t>> numberVertices(1, std::vector<size_t>{});
    ...
    for (int iter = 0; iter != order; ++iter) {
        ...
        if (order > 1) {
            newNumbers.reserve(
                std::min(static_cast<size_t>(1e6),
        ...
        if (iter != order - 1) {
            for (const auto &numbers : numberVertices) {
                ...
                if (not wick::WickCalculator::isContractionZero(
                        finalCorrelator,
                        nLoops,
    ...
    numberVertices.insert(
        numberVertices.end(), fixedSets.begin(), fixedSets.end());
    if (options.verboseAmplitude) {
        const auto plural = (numberVertices.size() == 1) ? "" : "s";
        std::cout << numberVertices.size() << " possible set" << plural
                  << " of vertices found." << '\n';
    }
                                   ...
                                   size_t maxInteractionLegs)
{
    if (order >= maxOrder - 2) {
        std::vector<QuantumField const *> finalCorrelator;
        finalCorrelator.reserve(insertions.size() + newFields.size()
                                + fieldPos.size() * 5);
        for (auto &field : insertions)
            finalCorrelator.push_back(&field);
        ...
        if (order == maxOrder - 1) {
            ...
            for (const auto &f : newFields)
                for (size_t i = 0; i != finalCorrelator.size() - n; ++i)
                    if (f.isExactlyContractiblewith(*finalCorrelator[i])) {
                        ++n_connected;
                        break;
                    }
    ...
    for (size_t iF = 0; iF < fieldPos.size(); iF++) {
        std::vector<QuantumField> const &content
