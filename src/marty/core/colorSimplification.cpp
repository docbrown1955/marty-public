...
static std::set<color::ColorSpace const *>
getColorSpaces(csl::Expr const &expr)
{
    std::set<color::ColorSpace const *> colors;
    csl::VisitEachLeaf(expr, [&](csl::Expr const &sub) {
        if (csl::IsIndicialTensor(sub)) {
            std::vector<csl::Space const *> space
                = sub->getParent()->getSpace();
            if (space.size() != 3 or space[0] == space[1]
                or space[1] != space[2])
                return;
...
static std::set<color::ColorSpace const *>
getAdjointSpaces(csl::Expr const &expr)
{
    std::set<color::ColorSpace const *> colors;
    csl::VisitEachLeaf(expr, [&](csl::Expr const &sub) {
        if (csl::IsIndicialTensor(sub)) {
...
static std::vector<size_t> getPos_fABC(csl::Expr &              prod,
...
static bool isFTrace(csl::Expr const &tensorA,
                     csl::Expr const &tensorB,
                     csl::Expr const &tensorC)
{
    return hasCommonIndex(tensorA, tensorB) && hasCommonIndex(tensorB, tensorC)
           && hasCommonIndex(tensorC, tensorA);
}

static int placeCommonIndex(csl::IndexStructure &si,
...
static csl::Expr fTrace(csl::Expr const &       fi,
                        csl::Expr const &       fj,
                        csl::Expr const &       fk,
...
static bool CalculateFColorTrace_impl(csl::Expr &              prod,
                                      color::ColorSpace const *space)
{
    HEPAssert(csl::IsProd(prod),
        ...
        std::vector<size_t>
            pos_fABC = getPos_fABC(prod, space);
    if (pos_fABC.size() < 3)
        return false;
        ...
        for (size_t j = i + 1; j < pos_fABC.size(); ++j) {
            ...
            for (size_t k = j + 1; k < pos_fABC.size(); ++k) {
                ...
                if (isFTrace(fi, fj, fk)) {
                        ...
                        fi, fj, fk, space->getGroup()->getGaugedGroup());
                    fj = fk = CSL_1;
                    return true;
                }
            }
