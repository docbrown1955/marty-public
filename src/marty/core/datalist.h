...
/**
 * @file datalist.h
 * @brief Contains the template data comparator utility for doc brown's
 * debugger.
...
struct DefaultComparator
    : public std::equal_to<
          std::remove_reference_t<std::remove_cv_t<Comparable>>> {
};
...
class DataList {
  ...
  public:
    ...
    DataList(
        DataList<ValueType, t_Comparator, t_HardComparator, t_Printer> const
            &other)
        : DataList(other.data)
    {
    }
    ...
    [[nodiscard]] DataDiff<ValueType, Comparator, HardComparator, Printer>
    getDiff(DataList<ValueType, Comparator, HardComparator, Printer> const
                &other) const;

    [[nodiscard]] DataDiff<ValueType, Comparator, HardComparator, Printer>
    getHardDiff(DataList<ValueType, Comparator, HardComparator, Printer> const
                    &other) const;
    ...
    const
{
    ...
    using HardList = typename doc::
        DataList<ValueType, HardComparator, HardComparator, Printer>;
    return HardList(std::move(diff.lhs))
        .getDiff(HardList(std::move(diff.rhs)));
}
