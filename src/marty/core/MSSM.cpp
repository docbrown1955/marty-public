...
MSSM_Model::MSSM_Model(std::string const &t_saveFile, bool init)
    : mty::Model(), mty::MSSM_Data(), saveFile(t_saveFile)
{
    if (init) {
        std::ofstream save;
        if (!saveFile.empty())
...
{
    s_B = mty::weylfermion_s(
        "sB ; \\tilde{B}", B->getGaugeIrrep(), Chirality::Left);
    s_Wi = mty::weylfermion_s(
        "sW ; \\tilde{W}", Wi->getGaugeIrrep(), Chirality::Left);
    s_G = mty::weylfermion_s(
        "sG; \\tilde{G}", G->getGaugeIrrep(), Chirality::Left);
...
void MSSM_Model::initSLeptons()
{
    s_Li = mty::scalarboson_s(
        "sL_L ; \\tilde{L_L}", Li->getGaugeIrrep(), Li->getFlavorIrrep());
    s_Ei = mty::scalarboson_s(
        "sE_R ; \\tilde{E_R}", Ei->getGaugeIrrep(), Ei->getFlavorIrrep());
    ...
    M2_s_L->addHermitianProperty(flavorSpace, M2_s_L({I, J}), M2_s_L({I, J}));
    M2_s_E->addHermitianProperty(flavorSpace, M2_s_E({I, J}), M2_s_E({I, J}));
...
void MSSM_Model::initSQuarks()
{
    s_Qi = mty::scalarboson_s(
        "sQ_L ; \\tilde{Q_L}", Qi->getGaugeIrrep(), Qi->getFlavorIrrep());
    s_Ui = mty::scalarboson_s(
        "sU_R ; \\tilde{U_R}", Ui->getGaugeIrrep(), Ui->getFlavorIrrep());
    s_Di = mty::scalarboson_s(
        "sD_R ; \\tilde{D_R}", Di->getGaugeIrrep(), Di->getFlavorIrrep());
    ...
    M2_s_Q->addHermitianProperty(flavorSpace, M2_s_Q({I, J}), M2_s_Q({I, J}));
    M2_s_U->addHermitianProperty(flavorSpace, M2_s_U({I, J}), M2_s_U({I, J}));
    M2_s_D->addHermitianProperty(flavorSpace, M2_s_D({I, J}), M2_s_D({I, J}));
...
{
    s_Hu = mty::weylfermion_s("sHu ; \\tilde{Hu}",
                              Hu->getGaugeIrrep(),
                              Hd->getFlavorIrrep(),
                              Chirality::Left);
    s_Hd = mty::weylfermion_s("sHd ; \\tilde{Hd}",
                              Hd->getGaugeIrrep(),
                              Hd->getFlavorIrrep(),
                              Chirality::Left);
    ...
    csl::Expr   hermitic = csl::GetHermitianConjugate(
        -mu_h * s_Hu({i, a}) * eps({i, j}) * s_Hd({j, a}), &mty::dirac4);
    addLagrangianTerm(-mu_h * s_Hu({i, a}) * eps({i, j}) * s_Hd({j, a}), true);
}
...
void MSSM_Model::addSUSYPair(mty::Particle const &first,
                                         ...
                                         mty::GaugedGroup *   gauged)
{
    ...
    for (auto &scalar : particles) {
        ...
        csl::Expr connexion
            = (isLeft) ? dirac4.C_matrix({a, b}) : -dirac4.getDelta()({a, b});
        csl::Expr generator = isLeft ? T({A, i, j}) : T({A, j, i});
        addLagrangianTerm(csl::DeepRefreshed(csl::prod_s({-csl::sqrt_s(2),
                                                          coupling,
                                                          generator,
                                                          scal,
                                                          ferm,
...
void MSSM_Model::initHiggsPotential()
{
    ...
    addLagrangianTerm(-mu_2 * (cc(Hu(i)) * Hu(i) + cc(Hd(i)) * Hd(i)));
    addLagrangianTerm(
...
void MSSM_Model::initYukawas()
{
    ...
    addLagrangianTerm(-cc(Ui({I, A, a})) * Yu({I, J}) * Qi({J, A, i, a})
                          * eps({i, j}) * Hu(j),
                      true // Add also the complex conjugate of this term
    );
    addLagrangianTerm(-cc(s_Ui({I, A})) * Yu({I, J}) * Qi({J, A, i, a})
                          * eps({i, j}) * C({a, b}) * s_Hu({j, b}),
                      true // Add also the complex conjugate of this term
    );
    addLagrangianTerm(-cc(Ui({I, A, a})) * Yu({I, J}) * s_Qi({J, A, i})
                          * eps({i, j}) * s_Hu({j, a}),
                      true // Add also the complex conjugate of this term
    );

    // Three supersymmetric terms for Down-type coupling
    addLagrangianTerm(cc(Di({I, A, a})) * Yd({I, J}) * Qi({J, A, i, a})
                          * eps({i, j}) * Hd(j),
                      true // Add also the complex conjugate of this term
    );
    addLagrangianTerm(cc(s_Di({I, A})) * Yd({I, J}) * Qi({J, A, i, a})
                          * eps({i, j}) * C({a, b}) * s_Hd({j, b}),
                      true // Add also the complex conjugate of this term
    );
    addLagrangianTerm(cc(Di({I, A, a})) * Yd({I, J}) * s_Qi({J, A, i})
                          * eps({i, j}) * s_Hd({j, a}),
                      true // Add also the complex conjugate of this term
    );
    ...
    addLagrangianTerm(
        cc(Ei({I, a})) * Ye({I, J}) * Li({J, i, a}) * eps({i, j}) * Hd(j),
        true // Add also the complex conjugate of this term
    );
    addLagrangianTerm(cc(s_Ei(I)) * Ye({I, J}) * Li({J, i, a}) * eps({i, j})
                          * C({a, b}) * s_Hd({j, b}),
                      true // Add also the complex conjugate of this term
    );
    addLagrangianTerm(cc(Ei({I, a})) * Ye({I, J}) * s_Li({J, i}) * eps({i, j})
                          * s_Hd({j, a}),
                      true // Add also the complex conjugate of this term
    );
}
void MSSM_Model::initTrilinears()
{
    ...
    addLagrangianTerm(
        -cc(s_Ui({I, A})) * Au({I, J}) * s_Qi({J, A, i}) * eps({i, j}) * Hu(j),
        true // Add also the complex conjugate of this term
    );
    // Down-type tri-linear from Ad
    addLagrangianTerm(
        cc(s_Di({I, A})) * Ad({I, J}) * s_Qi({J, A, i}) * eps({i, j}) * Hd(j),
        true // Add also the complex conjugate of this term
    );
    // Lepton tri-linear from Ae
    addLagrangianTerm(
        cc(s_Ei({I})) * Ae({I, J}) * s_Li({J, i}) * eps({i, j}) * Hd(j),
        true // Add also the complex conjugate of this term
    );

    // Up-type tri-linear from the super potential
    addLagrangianTerm(
        cc(mu_h) * cc(s_Ui({J, A})) * Yu({J, I}) * s_Qi({I, A, i}) * cc(Hd(i)),
        true // Add also the complex conjugate of this term
    );
    // Down-type tri-linear from the super potential
    addLagrangianTerm(
        cc(mu_h) * cc(s_Di({J, A})) * Yd({J, I}) * s_Qi({I, A, i}) * cc(Hu(i)),
        true // Add also the complex conjugate of this term
    );
    // Lepton tri-linear from the super potential
    addLagrangianTerm(
        cc(mu_h) * cc(s_Ei({J})) * Ye({J, I}) * s_Li({I, i}) * cc(Hu(i)),
        true // Add also the complex conjugate of this term
    );
}
    ...
    for (size_t i = 0; i != gauge->size(); ++i) {
        for (size_t p1 = 0; p1 != particles.size(); ++p1) {
            ...
            for (size_t p2 = 0; p2 < particles.size(); ++p2) {
                ...
                addLagrangianTerm(
                    csl::Refreshed(sign * CSL_HALF * quadratic1 * quadratic2));
            }
        }
    }
}
...
void MSSM_Model::initQuarticSLeptons()
{
    ...
    // ~L~L~E~E interaction
    addLagrangianTerm(-Ye({I, J}) * cc(Ye({L, K})) * cc(s_Li({K, i}))
                      * s_Li({J, i}) * cc(s_Ei({I})) * s_Ei({L}));
}
void MSSM_Model::initQuarticSQuarks()
{
    DEFINE_GAUGE_INDICES
    // ~Q~Q~U~U interaction
    addLagrangianTerm(-Yu({I, J}) * cc(Yu({L, K})) * cc(s_Qi({K, A, i}))
                      * s_Qi({J, A, i}) * cc(s_Ui({I, B})) * s_Ui({L, B}));
    // ~Q~Q~D~D interaction
    addLagrangianTerm(-Yd({I, J}) * cc(Yd({L, K})) * cc(s_Qi({K, A, i}))
                      * s_Qi({J, A, i}) * cc(s_Di({I, B})) * s_Di({L, B}));
}
void MSSM_Model::initQuarticSLeptonHiggs()
{
    DEFINE_GAUGE_INDICES
    // ~L~LH_dH_d interaction
    addLagrangianTerm(-Ye({I, J}) * cc(Ye({I, K})) * cc(s_Li({K, k}))
                      * s_Li({J, i}) * eps({i, j}) * Hd(j) * eps({k, l})
                      * cc(Hd(l)));
    // ~E~EH_dH_d interaction
    addLagrangianTerm(-Ye({I, J}) * cc(Ye({L, J})) * cc(s_Ei({I})) * s_Ei({L})
                      * Hd(i) * cc(Hd(i)));
}
void MSSM_Model::initQuarticSQuarkHiggs()
{
    ...
    addLagrangianTerm(-Yu({I, J}) * cc(Yu({I, K})) * cc(s_Qi({K, A, k}))
                      * s_Qi({J, A, i}) * eps({i, j}) * Hu(j) * eps({k, l})
                      * cc(Hu(l)));
    // ~Q~QH_dH_d interaction
    addLagrangianTerm(-Yd({I, J}) * cc(Yd({I, K})) * cc(s_Qi({K, A, k}))
                      * s_Qi({J, A, i}) * eps({i, j}) * Hd(j) * eps({k, l})
                      * cc(Hd(l)));

    // ~U~UH_uH_u interaction
    addLagrangianTerm(-Yu({I, J}) * cc(Yu({L, J})) * cc(s_Ui({I, A}))
                      * s_Ui({L, A}) * Hu(i) * cc(Hu(i)));
    // ~D~DH_dH_d interaction
    addLagrangianTerm(-Yd({I, J}) * cc(Yd({L, J})) * cc(s_Di({I, A}))
                      * s_Di({L, A}) * Hd(i) * cc(Hd(i)));

    // ~U~DH_uH_d interaction
    addLagrangianTerm(Yu({I, J}) * cc(Yd({L, J})) * cc(s_Ui({I, A}))
                          * s_Di({L, A}) * Hu(i) * cc(Hd(i)),
...
void MSSM_Model::initQuarticSQuarkSLeptons()
{
    DEFINE_GAUGE_INDICES
    // ~D~Q~L~E interaction
    addLagrangianTerm(Yd({I, J}) * cc(Ye({L, K})) * cc(s_Di({I, A}))
                          * s_Qi({J, A, i}) * cc(s_Li({K, i})) * s_Ei({L}),
                      true // Adding complex conjugate
    );
}
...
void MSSM_Model::breakSU2LGaugeSymmetry()
{
    ...
    breakGaugeSymmetry(
        ...
        {{"Hup", "Hu0"},
         ...
         {"sW1   ; \\tilde{W}", "sW2  ; \\tilde{W}^2", "sW3 ; \\tilde{W}^3"},
...
void MSSM_Model::expandAroundVEVs()
{
    ...
    Replaced(*this, Hu0, (rho_u + CSL_I * eta_u + vu) / csl::sqrt_s(2));
    Replaced(*this, Hup, phi_u_c);

    Replaced(*this, Hdm, cc(phi_d_c));
    Replaced(*this, Hd0, (rho_d + CSL_I * eta_d + vd) / csl::sqrt_s(2));
    csl::Expr mu_2 = mu_h * csl::GetComplexConjugate(mu_h);
    M_A0           = csl::constant_s("M_A");
    Replaced(
        *this, b_h, csl::sin_s(beta_h) * csl::cos_s(beta_h) * (M_A0 * M_A0));
}
void MSSM_Model::diagonalize2By2Matrices()
{
    ...
    Replaced(
        *this, m_sHu, csl::sqrt_s(M_A0 * M_A0 - m_sHd * m_sHd - 2 * mu_2));
    Replaced(*this,
             m_sHd,
             csl::sqrt_s(csl::Expanded(-MZ2 * c2beta / 2)
...
void MSSM_Model::diagonalizeYukawas()
{
    ...
    csl::Tensor M_u = csl::tensor_s(
        "M_u",
        {flavorSpace, flavorSpace},
        csl::matrix_s({{m_u, 0, 0}, {0, m_c, 0}, {0, 0, m_t}}));

    csl::Tensor M_d = csl::tensor_s(
        "M_d",
        {flavorSpace, flavorSpace},
        csl::matrix_s({{m_d, 0, 0}, {0, m_s, 0}, {0, 0, m_b}}));
    ...
    replace(Ye, csl::sqrt_s(2) / (v_h * csl::cos_s(beta_h)) * M_e({f_i, f_j}));
    replace(Yu, csl::sqrt_s(2) / (v_h * csl::sin_s(beta_h)) * M_u({f_i, f_j}));
    replace(Yd({f_i, f_j}),
    ...
    replace(sD_L({f_j, A}), V_CKM({f_j, f_k}) * sD_L({f_k, A}));
    replace(sD_R({f_i, A}), V_CKM({f_i, f_j}) * sD_R({f_j, A}));
...
void MSSM_Model::adjustCouplingConstants()
{
    ...
    replace(g_L * g_L + g_Y * g_Y,
            csl::pow_s(g_L, 2) * (1 + csl::pow_s(csl::tan_s(thetaW), 2)));
    replace(g_Y, e / csl::cos_s(thetaW));
    replace(g_L, e / csl::sin_s(thetaW));
    // replace(
    //         csl::cos_s(thetaW),
...
void MSSM_Model::diagonalizeCharginos()
{
    ...
    rotateFields({C1_init, C2_init},
                 {C1p, C2p},
                 {{V_Wp1, V_Wp2}, {V_u1, V_u2}},

                 {C3_init, C4_init},
                 {C1m, C2m},
                 {{U_Wm1, U_Wm2}, {U_d1, U_d2}});
        ...
        for (auto &block : massBlocks) {
            ...
            if (block.particles.size() >= 2) {
                ...
                if (diagonalizedSymbolically) {
                    std::cout << "Interaction Lagrangian has know " << L.size()
                              << " terms." << std::endl;
                    break;
                }
                break;
...
void MSSM_Model::renameSFermions()
{
    ...
    for (auto const &init_name : names) {
        ...
        newName  = init_name + "_1; \\tilde{"
                  + std::string(name.begin() + 1, name.end()) + "_1}";
        renameParticle(susyName, newName);
        susyName = init_name + "_R";
        newName  = init_name + "_2; \\tilde{"
                  + std::string(name.begin() + 1, name.end()) + "_2}";
        renameParticle(susyName, newName);
    }
}
...
MSSM_HEM::MSSM_HEM(std::string const &saveFile) : MSSM_Model(saveFile, false)
{
    std::ofstream save;
    if (!saveFile.empty())
        save.open(saveFile);
