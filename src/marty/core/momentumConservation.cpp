...
MomentumConservater::MomentumConservater(
    ...
    : signs(insertions.size(), true), momenta(t_momenta)
{
...
std::vector<csl::Expr> MomentumConservater::isMomentumSum(csl::Expr const &sub)
{
    if (!csl::AllOfLeafs(sub, [&](csl::Expr const &subsub) {
            return csl::IsNumerical(subsub) || csl::IsLiteral(subsub)
...
MomentumConservater::applyOnMomentumSum(std::vector<csl::Expr> &factors)
{
...
std::pair<int, csl::Expr>
MomentumConservater::getMomentumStructure(csl::Expr const &term)
{
...
bool MomentumConservater::simplify(std::vector<csl::Expr> &factors)
{
    ...
    for (size_t i = 0; i != factors.size(); ++i) {
        if (factors[i] != CSL_0) {
            auto test = factors;
            std::for_each(test.begin(), test.end(), [&](csl::Expr &el) {
                el = el - factors[i];
            });
            auto nZero = getNZero(test);
            if (nZero > nZeroBest
                || (nZero == nZeroBest && CSL_0 != test[posSimplestMomenta])) {
                nZeroBest  = nZero;
                factors    = test;
                simplified = true;
            }
...
csl::Expr
MomentumConservater::recoverExpr(std::vector<csl::Expr> const &factors)
{
