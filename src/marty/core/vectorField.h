...
class VectorBoson : public BaseVectorBoson {
  ...
  protected:
    ...
    /**
     * @brief Pointer to the conjugated GhostBoson associated to the vector, if
     * there is one (for example for the W boson that has two ghosts).
     */
  ...
  public:
    ...
    void setBrokenParts(const csl::Space *           broken,
  ...
  protected:
    ...
    void updateBrokenGhost(csl::Space const *                 space,
 ...
 *
 * @details This does not correspond to a new particle, simply another
 * structure for a vector boson in MARTY, with the field strength \f$ F \f$
 * defined from the initial vector boson \f$ A \f$ as \f[
...
class FieldStrength : public BaseVectorBoson {
  ...
  private:
    /**
     * @brief Raw pointer to the vector boson associated with the
     * FieldStrength.
     */
  ...
  public:
    ...
    bool isSameSpecies(QuantumFieldParent const *other) const override
    {
        return this == other || vectorParent == other;
    }
...
/**
 * @brief Delegates the construction of a field strength and returns the
 * result.
 *
 * @details This function can take any argument that the constructors
