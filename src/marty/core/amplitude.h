...
class Amplitude {
  ...
  public:
     ...
     * @param t_kinematics Kinematics of the process.
     */
    Amplitude(FeynOptions const &                     t_options,
     ...
     */
    Amplitude
    filterOut(std::function<bool(mty::FeynmanDiagram const &)> filter) const;
    ...
    {
        return filterOut(std::forward<First>(first))
            .filterOut(std::forward<Next>(next)...);
    }
