 ...
 */
inline constexpr auto topologyList = {Tadpole, Mass, Triangle, Box, Pentagon};
...
{
    return static_cast<Type>(((nLegs == 1) * Tadpole) | ((nLegs == 2) * Mass)
                             | ((nLegs == 3) * Triangle) | ((nLegs == 4) * Box)
                             | ((nLegs == 5) * Pentagon));
}
...
inline std::ostream &operator<<(std::ostream &out, Type t)
{
    if (t == Any)
        ...
        out << *(names.begin() + static_cast<int>(t) / 2);
    return out;
}
} // namespace mty::Topology
