...
Amplitude::Amplitude(FeynOptions const &t_options,
                     Kinematics const & t_kinematics)
    : options(t_options), kinematics(t_kinematics)
{
}
...
Amplitude::Amplitude(FeynOptions const &                     t_options,
    ...
    : options(t_options), diagrams(t_diagrams), kinematics(t_kinematics)
{
}
...
csl::Expr &Amplitude::expression(size_t pos)
{
    HEPAssert(pos < diagrams.size(),
              ...
              "Index " + std::to_string(pos)
                  + " out of bounds of amplitude "
                    "of size "
                  + std::to_string(size()) + ".") return diagrams[pos]
        .getExpression();
}
...
std::vector<std::shared_ptr<mty::wick::Graph>> Amplitude::obtainGraphs() const
{
...
{
    add(std::make_move_iterator(begin(t_diagrams)),
        std::make_move_iterator(end(t_diagrams)));
}

Amplitude Amplitude::filterOut(
    std::function<bool(mty::FeynmanDiagram const &)> filter) const
{
...
void Amplitude::setKinematics(Kinematics const &t_kinematics, bool replace)
{
    if (replace) {
        for (auto &diag : diagrams) {
            Kinematics::replace(
                diag.getExpression(), kinematics, t_kinematics);
        }
    }
    kinematics = t_kinematics;
}
