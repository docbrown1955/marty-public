...
unique_ptr<Model> JSONToHEP::readModel(std::string const& nameFile)
{
    ...
    vector<shared_ptr<QuantumFieldParent>> fields 
        = readField(jsonTree.get(),
                    ptr_gauge,
                    ptr_flavor,
                    model.get(),
                    numbers);
    ...
    vector<pair<string, JSON::Node*>> yukawas = readYukawaNodes(
            JSON::Parser::parseNode(jsonTree.get(), "field"));
    for (const auto& [nameScalar, node] : yukawas) {
        csl::Expr interaction = interpretYukawaCoupling(model.get(),
                                                   nameScalar,
                                                   node);
...
unique_ptr<Gauge> JSONToHEP::readGauge(JSON::Node* tree)
{
    ...
    for (const auto& groupNode : *gaugeNode) {
        ...
        JSON::Node* bosonNode = 
            JSON::Parser::parseNode(groupNode, "boson");
...
unique_ptr<Flavor> JSONToHEP::readFlavor(JSON::Node* tree)
{
    ...
    for (const auto& flav : *flavorNode) {
        string name = JSON::Parser::parseArgument<string>(
                flav,
                "name",
                true).value();

        int nFlavors = JSON::Parser::parseArgument<int>(
                flav,
                "nF",
                true).value();
...
vector<QuantumNumber> JSONToHEP::readQuantumNumbers(JSON::Node* tree)
{
    ...
    for (const auto& num : *qnNode) {
        string name = JSON::Parser::interpretObject<string>(
                num.get());
        auto pos = find_if(numbers.begin(),
                           numbers.end(),
                           [&](QuantumNumber const& n) {
...
{
    string name = 
        JSON::Parser::parseArgument<string>(couplingNode, "name", true).value();
...
vector<shared_ptr<QuantumFieldParent>> JSONToHEP::readField(
        JSON::Node* tree,
        Gauge* gauge,
        Flavor* flavor,
        Model*  model,
...
{ 
    ...
    for (const auto& node : *fieldNode) {
        ...
        JSON::Node* massNode =
            JSON::Parser::parseNode(node, "mass");
        ...
        if (spin == 0)
            ...
            newField = diracfermion_s(
                       name,
                       irrep,
                       flavorRep);
        else if (spin == 2)
            newField = vectorboson_s(
                       name,
                       irrep,
                       flavorRep);
        else
            CallHEPError(mty::error::ValueError,
                    "Spin " + toString(spin) + " not taken into account in"
                    + " json reader.");
        if (massNode)
            readMass(massNode, newField.get());
        JSON::Node* qNumNode = JSON::Parser::parseNode(node, "quantum numbers");
        if (qNumNode) 
...
vector<pair<string, JSON::Node*>> JSONToHEP::readYukawaNodes(
        JSON::Node* fields)
{
    ...
    for (const auto& node : *fields) {
        ...
        if (yukawa) {
            string nameScalar = JSON::Parser::parseArgument<string>(
                    node, "name", true).value();
            for (const auto& coupling : *yukawa)
                res.push_back(
                        make_pair(nameScalar,
                                  JSON::Parser::convert(coupling)));
        }
    }
    return res;
}
...
csl::Expr JSONToHEP::interpretYukawaCoupling(Model*        model,
                                        ...
                                        JSON::Node*   node)
{
    string flavorName = JSON::Parser::parseArgument<string>(
            node, "flavor", true).value();
    string couplingName = JSON::Parser::parseArgument<string>(
            node, "coupling", true).value();
    ...
    const Space* flavorSpace = flavorGroup->getVectorSpace(
            field1->getFlavorIrrep(flavorGroup));
    ...
    HEPAssert(scalarIndices.size() == 1,
            ...
            + static_cast<string>(" 1, ") + toString(scalarIndices.size()) 
            + " given.");
    ...
    SumGaugeIrrep prod = scalarRep 
                       * field1Rep.getConjugatedRep() 
                       * field2Rep;
    if (prod.find(gauge->getTrivialRep()) == -1) {
        prod = scalarRep.getConjugatedRep()
             * field1Rep.getConjugatedRep()
             * field2Rep;
        ...
        expression = Yukawa({flavorIndex1, flavorIndex2})
             * gaugeIndex.getSpace()->getEpsilon()({gaugeIndex, 
                                                    gaugeIndex2})
    ...
    return - expression - GetHermitianConjugate(expression,
                                                &dirac4);
}
...
GaugeIrrep JSONToHEP::readIrrep(JSON::Node* node, Gauge* gauge)
{
    ...
    for (size_t i = 0; i != groupName.size(); ++i) {
        ...
        if (irrepSpec) {
            ...
            // we add the denominator 1 to the fraction before creating the rep.
...
FlavorIrrep JSONToHEP::readFlavorIrrep(JSON::Node* node,
                                       Flavor*     flavor)
{
    ...
    for (const auto& flavorNode : *node) {
        string fName = JSON::Parser::interpretObject<string>(
                flavorNode.get());
...
void JSONToHEP::readMass(JSON::Node*         massNode,
