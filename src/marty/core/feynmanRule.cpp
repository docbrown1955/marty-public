...
FeynmanRule::FeynmanRule() : expr(CSL_0)
{
}
...
FeynmanRule::FeynmanRule(mty::Model &model, TermType const &t_term)
    : FeynmanRule()
{
    ...
    for (size_t i = 0; i != fieldProduct.size(); ++i) {
        ...
        if (typeid(FieldStrength) == typeid(pointed)) {
            auto vectorBoson = dynamic_cast<FieldStrength *>(
                                   fieldProduct[i].getParent().get())
                                   ->getVectorParent();
            bool conjug = fieldProduct[i].isComplexConjugate();
            fieldProduct[i]
                = ConvertTo<QuantumField>(vectorBoson->getInstance());
        ...
        momenta[i]
            = csl::Tensor("p_" + std::to_string(i + 1), &csl::Minkowski);
        ...
        csl::Transform(ampl, [&](csl::Expr &el) {
            if (el->getType() == csl::Type::TensorElement) {
                if (fieldProjected and dirac4.isProjector(el)) {
                    // If the field is projected we do not project the vertex
                    // also
                             ...
                             std::vector<csl::Tensor>::iterator  last)
{
    ...
    for (auto field = fieldProduct.rbegin(); field != fieldProduct.rend();
         ++field) {
