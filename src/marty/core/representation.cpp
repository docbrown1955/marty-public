...
csl::Expr Irrep::getCharge() const
{
    ...
    CallHEPError(mty::error::NotImplementedError,
                 "Charge not implemented " + std::string("for algebra ")
                     + toString(algebra->getType()));
    return nullptr;
}
...
bool Irrep::operator==(const Irrep &other) const
{
    return algebra == other.algebra and highestWeight == other.highestWeight;
}
...
{
    SumIrrep rep(*this);
    HEPAssert(algebra == other.getAlgebra(),
              mty::error::ValueError,
...
SumIrrep::SumIrrep(const Irrep &other)
    : vector<Irrep>({other}), algebra(other.getAlgebra())
{
}
