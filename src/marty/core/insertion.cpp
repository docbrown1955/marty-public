...
Insertion::Insertion(QuantumFieldParent *t_field) : field(t_field)
{
}

Insertion::Insertion(Particle t_field) : Insertion(t_field.get())
{
}
...
Insertion::Insertion(csl::Expr const &t_field)
{
    HEPAssert(IsOfType<QuantumField>(t_field),
              ...
              "Building insertion from " + toString(t_field) + " that is not"
                  + " a quantum field.");
    auto *field_ptr = ConvertToPtr<QuantumField>(t_field);
...
Insertion::Insertion(std::string const &name) : Insertion(name.c_str())
{
}
...
Insertion::Insertion(const char name[]) : Insertion(GetParticle(name).get())
{
}
...
csl::Expr Insertion::getExpression() const
{
    if (validExpression)
        return expression;
    csl::Expr insertion
        = (expression == CSL_UNDEF) ? field->getInstance() : expression;
...
{
    return field == other.field && incoming == other.incoming
           && particle == other.particle && mediator == other.mediator;
}
