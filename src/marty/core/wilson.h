...
struct Wilson {
    ...
    bool isNull() const
    {
        return coef.getCoefficient() == CSL_0 or op.getExpression() == CSL_0;
    }
    ...
    {
        return csl::prod_s(
            {op.getFactor(), coef.getCoefficient(), op.getOp()});
    }

    friend std::ostream &operator<<(std::ostream &out, Wilson const &wil)
    {
        wil.print(out);
        return out;
    }
...
inline Cache<csl::Expr, // Key type
             csl::Expr  // Result type
             >
    cachedWilson([](csl::Expr const &A, csl::Expr const &B) {
...
std::vector<Wilson> parseExpression(csl::Expr const &product,
