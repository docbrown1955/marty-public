...
csl::Expr CSLDataHandler::read(csl::Expr const &expr, bool abbrev)
{
    parseTensors(expr);
...
void CSLDataHandler::readJSON(std::string const &fileName)
{
    ...
    for (const auto &spaceNode : *spaceList) {
        std::string name = *JSON::Parser::parseArgument<std::string>(
            spaceNode, "name", true);
        int dim = *JSON::Parser::parseArgument<int>(spaceNode, "dim", true);
        treatSpace(name, dim);
    }
    ...
    for (const auto &tensorNode : *tensorList) {
        ...
        int conj = *JSON::Parser::parseArgument<int>(tensorNode, "conj", true);
        ...
        for (const auto &spaceNode : *spaceList) {
            std::string name = *JSON::Parser::parseArgument<std::string>(
                spaceNode, "name", true);
            int dim
                = *JSON::Parser::parseArgument<int>(spaceNode, "dim", true);
    ...
    for (const auto &literalNode : *literalList) {
        ...
        int conj
            = *JSON::Parser::parseArgument<int>(literalNode, "conj", true);
    ...
    for (const auto &indexNode : *indexList) {
        ...
        int dim = *JSON::Parser::parseArgument<int>(indexNode, "dim", true);
        int N   = *JSON::Parser::parseArgument<int>(indexNode, "number", true);
        csl::Space const *space = treatSpace(nameSpace, dim);
        treatIndices(space, N);
    }
}

csl::Space const *CSLDataHandler::treatSpace(std::string const &name, int dim)
{
...
csl::Expr CSLDataHandler::treatLiteral(std::string const &name, bool isComplex)
{
...
void CSLDataHandler::treatIndices(csl::Space const *space, size_t N)
{
    ...
    csl::VisitEachLeaf(expr, [&](csl::Expr const &sub) {
        if (csl::IsIndicialTensor(sub) and !csl::Abbrev::find_opt(sub)) {
            ...
            HEPAssert(
                tensor,
                mty::error::TypeError,
                "Somehow " + toString(sub)
                    + " does not give a csl::Tensor.") if (csl::
                                                               IsTensorField(
                                                                   sub)
                                                           or csl::
                                                               IsTensorialDerivative(
                                                                   sub))
            {
                csl::Tensor point = sub->getPoint();
                const auto  pos
                    = std::find(tensors.begin(), tensors.end(), point);
                if (pos == tensors.end())
    ...
    csl::VisitEachLeaf(expr, [&](csl::Expr const &sub) {
        if (csl::IsLiteral(sub) and !csl::Abbrev::find_opt(sub)) {
            ...
            const auto pos = std::find_if(
                literal.begin(), literal.end(), [&](csl::Expr const &other) {
...
void CSLDataHandler::parseAbbreviations(csl::Expr const &expr)
{
    csl::VisitEachLeaf(expr, [&](csl::Expr const &sub) {
        if (csl::Size(sub) == 0 and csl::Abbrev::find_opt(sub)) {
            ...
            const auto           pos    = std::find_if(abbreviations.begin(),
                                          abbreviations.end(),
...
std::unique_ptr<JSON::Node> CSLDataHandler::getSpaceList(
    std::vector<csl::Space const *> const &spaces) const
{
...
void CSLDataHandler::printJSON(std::string const &nameFile) const
{
    ...
    for (const auto &t : tensors) {
        ...
        auto conjLeaf   = JSON::Leaf<int>::make(
            "conj", t->getComplexProp() == csl::ComplexProperty::Complex);
    ...
    for (const auto &l : literal) {
        ...
        auto conjLeaf    = JSON::Leaf<int>::make(
            "conj", l->getComplexProperty() == csl::ComplexProperty::Complex);
    ...
    for (const auto &[space, N] : nIndices) {
        ...
        auto spaceLeaf
            = JSON::Leaf<std::string>::make("space", space->getName());
...
void CSLDataHandler::printComBlock(std::string_view com,
                                   std::ostream &   out,
...
void CSLDataHandler::printCSLDefinition(csl::Tensor   tensor,
                                        std::ostream &out,
                                        size_t        indentSize) const
{
    if (auto der = dynamic_cast<csl::TDerivativeParent const *>(tensor.get());
        ...
        der->csl::TDerivativeParent::printDefinition(out, indentSize, true);
             ...
             = dynamic_cast<csl::TensorFieldParent const *>(tensor.get());
             field) {
