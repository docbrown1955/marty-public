...
inline std::ostream &operator<<(std::ostream &out, Chirality chir)
{
    switch (chir) {
    case Chirality::Left:
        out << "Left";
...
/*!
 * \brief Default space-time point when not given while defining a
 * QuantumField. \details See QuantumField::operator(). To define a
 * QuantumField, one needs in general indices (except for fully scalar fields),
 * and a space-time point. This defines an object like \f$ A^{\mu A}(X) \f$.
 * That being said, it is rarely important to specify the space-time point of
 * the field. In particular, in any interaction term of the Lagrangian all
 * fields are at the same point. This variable is given by default to
 * QuantumField when no point is given. A code like \code A({mu, A}); \endcode
 * is equivalent to \code A({mu, A}, defaultSpaceTimePoint) \endcode. Then, whe
 * integrated to the Lagrangian, the point is automatically replaced by the one
 * of the Lagrangian.
 */
inline csl::Tensor defaultSpaceTimePoint = csl::tensor_s("X", &csl::Minkowski);
 ...
 * \f]
 * fermions \f$ \psi _i \f$ have respectively PartnerShips \f$ (2, 3, true)
 * \f$, \f$ (3, 2, false) \f$, \f$ (4, 1, true) \f$ and \f$ (1, 4, false) \f$.
 */
struct PartnerShip {
    int  self{-1};
    int  partner{-1};
 ...
 * ...). This field is then a QuantumField object. The QuantumFieldParent is
 * there to generate QuantumField when giving it indices and (optionnaly) a
 * point \f$ X \f$. See operator(). The QuantumField keeps a pointer to its
 * parent in order to have access to all abstract properties. \note
 * QuantumFieldParent must not be created statically. The Particle object must
 * be created instead, a std::shared_ptr<QuantumFieldParent>. \note
 * QuantumFieldParent may not be built directly (pure abstract class). Instead,
 * one must create a specialization. See ScalarBoson, VectorBoson, GaugeBoson,
 * WeylFermion, DiracFermion, GoldstoneBoson, or GhostBoson.
 */
class QuantumFieldParent : public csl::TensorFieldParent {

  public:
    /*!
     * \brief Type definition. A Propagator_func is a function called to get
     * the propagator result of the contraction of two fields. \details The
     * function is contained in a std::function. Such an object may be
     * instantiated with a c++ lambda function or a standard function defined
     * normally. The function or lambda must take four arguments. The two
     * fields contracted (QuantumField), the momentum integrated (csl::Tensor),
     * and a boolean telling if one of the two fields is external. It must
     * return an expression (csl::Expr) that is the propagator between the two
     * fields for which the factor \f$\frac{d^4p}{(2\pi)^4}\f$ has been
     * removed. Implementations assure that if the field is complex, the
     * conjugated field is always the second argument. If one of the fields is
     * external, the propagator should be adapted. For example, the propagator
     * of two scalar fields is \f[ \frac{ie^{-ipX}}{p^2-m^2}, \f] whereas if
     * one of the fields is external, the projection on the equations of motion
     * \f$ \partial _\mu\partial ^\mu + m^2 \f$ gives simply \f[ -ie^{-ipX}.
     * \f]
     * The Propagator_func must provide the entire expression of the propagator
     * but the integral \f$ \int \frac{d^4p}{(2\pi)^4} \f$ that is
     * automatically added. In quantumFieldTheory.h / quantumFieldTheory.cpp
     * all built-in propagators are written in the form of standard c++
     * functions. Propagators for any particle may be changed by the user. See
     * mty::AddCustomPropagator().
     */
    using Propagator_func = std::function<csl::Expr(
        QuantumField const &, QuantumField const &, csl::Tensor &, bool)>;
  ...
  protected:
    ...
    /*!
     * \brief Tells if the field is physical. If true, it may appear in
     * external states. Otherwise it is forbidden.
     */
    bool physical = true;
    ...
    /*!
     * \brief Particle type in graphical diagrams (straight line, arrow,
     * vector, gluon ...).
     */
    drawer::ParticleType drawType = drawer::ParticleType::None;

    /*!
     * \brief Pointer to the space-time group. For know it is the Lorentz
     * group, \f$ D_2 \f$ algebra.
     */
    Group *stGroup;
    ...
    /*!
     * \brief Position of the first space index, after flavor and gauge
     * indices.
     */
  ...
  public:
     ...
     * is created.
     * \param t_name            Name of the field.
     * \param t_spin            Integer equal to twice the spin of the
     * particle. \param t_gauge           Pointer to the gauge of the model.
     * \param t_isSelfConjugate Boolean that must be true for real fields,
     * false otherwise.
     */
    QuantumFieldParent(const std::string &t_name,
                       int                t_spin,
     ...
     * \param t_name            Name of the field.
     * \param t_spin            Integer equal to twice the spin of the
     * particle. \param irrep             Gauge representation of the field.
     * \param t_isSelfConjugate Boolean that must be true for real fields,
     * false otherwise.
     */
    QuantumFieldParent(const std::string &t_name,
    ...
    /*!
     * \brief Builds a new particle identical to another, with a different
    ...
    /*!
     * \brief Pure virtual function. Generates a partical similar to the
     * current object, changing only the name. \param t_name Name of the new
     * particle, identical to the current object. \return The new Particle.
     */
    virtual Particle generateSimilar(std::string const &t_name) const = 0;

    /*!
     * \brief Returns the chirality of the particle.
     * \details A DiracFermion or boson will return Chirality::None.
     * WeylFermion will return either Chirality::Left or Chirality::Right.
     ...
     * conjugated. When it is the case, in order to ensure that the conjugate
     * is on the right, the order is reverted. \param other    Left field of
     * the propagator. \param self     Right field of the propagator. \param P
     * Momentum integrated in the propagator. \param external \b True if one of
     * the two fields is an external leg. \return The expression of the
     * propagator without the integral term \f$ \int \frac{d^4p}{(2\pi)^4} \f$.
     */
     ...
     * \details This function is overriden in derived classes. Each particle
     has
     * a specific form for its kinetic term, including covariant derivatives
     and
     ...
     * there is one.
     * \return The GoldstoneBoson associated to the GoldstoneBoson is it
     ...
     * \details A FieldStrength is defined automatically for VectorBoson
     * fields, gauge bosons or not. It is defined as \f[ F^I_{\mu\nu} \equiv
     * \partial _\mu A^I_\nu - \partial _\nu A^I_\mu, \f] with \f$ A^A_\mu \f$
     * a vector boson with a generic index \f$ I \f$ representing all flavor
     * and gauge structures. If the field strength is defined, the kinetic term
     * is expressed as \f$ -\frac{1}{4}F^I_{\mu\nu} F^{I\mu\nu} \f$ and
     * contractions like \f$ \langle A^I_\mu F^J_{\nu\rho} \rangle \f$ or \f$
     * \langle F^I_{\mu\nu}F^J_{\rho\sigma}\rangle \f$ are defined by built-in
     * propagators. \sa VectorBoson, FieldStrength, Propagator_func,
     * quantumFieldTheory.h. \return \b True  if the particle (should be a
     * VectorBoson) has a FieldStrength defined. \return \b False else.
     */
     ...
     * fields have no space-time index, spin 1/2 and 1 have 1. However, in
     * order to potentially extend this function to higher spins with more
     * space-time indices, this function returns a std::vector. \return New
     * indices generated corresponding to the space-time indices of the field.
     * \sa getGaugeIndex(), getFlavorIndex(),
     * getFullSetOfIndices().
     */
    std::vector<csl::Index> getSpaceIndex(const std::string &nameIndex
                                          = "") const;

    /*!
     * \brief Returns a set of gauge indices corresponding to the field.
     * \return A set of new indices generated in the gauge representation
     * spaces of the field. \sa getFlavorIndex(), getSpaceIndex(),
     * getFullSetOfIndices().
     */
     ...
     * by the user. It returns an instance of the field like \f$ A^{\mu A}(X)
     * \f$ calling operator(). \param point Sapce-time point at which the field
     * is evaluated (optional). \return An expression of the field instance
     * with free and independent indices. \sa getFullSetOfIndices(),
     * operator()().
     */
     ...
     * on, the corresponding mass term is added automatically to the
     * Lagrangian. \param t_mass Name of the constant mass. \sa #mass,
     * getMass().
     */
     ...
     * \param t_physical Boolean telling if the field is physical (may appear
     * in external legs). \sa #physical, isPhysical(), setExternal().
     */
     ...
     * group.
     * \details When breaking flavor symmetry, we may sometimes be interested
     * in breaking only partially the symmetry. For example, we may break a 3
     * flavor-symmetry in one light particle and a 2-flavor symmetry. This
     ...
     * \details The Irrep given as parameter is an already built
     * representation. This function is called during gauge symmetry breaking,
     * when representation are broken. Changing the representation, the indices
     * of the tensor change and may disappear if the new representation is
     * trivial, or appear if the new representation is not while the old one
     * is. \param group  Group for which the field representation is changed.
     ...
     * the tensor change and may disappear if the new representation is
     * trivial, or appear if the new representation is not while the old one
     * is. \param group         Group for which the field representation is
     * changed. \param highestWeight Dinkin labels of the highest weight for
     * the wanted representation. \sa #irrep, getGroupIrrep(), getGaugeIrrep(),
     ...
     * \details The Irrep given as parameter is an already built
     * representation. This function is called during flavor symmetry breaking,
     * when representation are broken. Changing the representation, the indices
     * of the tensor change and may disappear if the new representation is
     * trivial, or appear if the new representation is not while the old one
     * is. \param group  Flavor group for which the field representation is
     ...
     * several parts. In this case the space is fully broken (no subspace
     ...
     * Then follow the rest of the indices, that can be unique or in a
     * std::vector. For example, one may create the polarization tensor of a
     * vector boson \f$ \epsilon ^\lambda_{\mu I}(p) \f$ with the following
     * code \code Index lambda = ...; Index mu = ...; Index I = ...; Tensor p =
     * ...; Particle A = vectorboson_s(...) csl::Expr epsilon = A(lambda, {mu,
     * I}, p); \endcode \return An expression, instance of the field. \sa
     * PolarizationField.
     */
    csl::Expr operator()(csl::Index              polarization,
  ...
  protected:
    ...
    removeRelative(QuantumFieldParent const *particle);
    ...
    /*!
     * \brief Initializes default propagators for the particle. This function
     * is called automatically in the constructor of derived classes. \sa
     ...
     * what must be given. This function may be useful for example to define
     * the propagator between a left-handed and a right-handed fermion
     * (different particles), or between a vector boson and its field
     * strength... \param field      Left field in the propagator. \param field
     * Right field in the propagator. \param propagator New propagator.
     ...
     * be the first argument. The only thing that is ensured is that if there
     * is a complex conjugated field, it will be the second argument. \sa
 ...
 * a particle (otherwise we would have to write \code csl::Expr instance =
 * (*phi)(X) \endcode which is not very practical). Particle is just managing
 * tho ownership of a QuantumFieldParent without building it (this is done by
...
class Particle : public std::shared_ptr<QuantumFieldParent> {
  ...
  public:
    ...
    /*!
     * \brief Delegates the arguments to QuantumFieldParent::operator()() of
     * the managed object. \tparam Args Variadic template parameters. \param
     * args Variadic function arguments. \return The corresponding instance of
     ...
     * \brief Delegates the arguments to QuantumFieldParent::operator()() of
     * the managed object. \tparam Args Variadic template parameters. \param
     * polar Polarization index for the polarization tensor. \param index Other
     * index. \param args  Variadic function arguments. \return The
     * corresponding instance of PolarizationField. \sa
     ...
     * \brief Delegates the arguments to QuantumFieldParent::operator()() of
     * the managed object. \tparam Args Variadic template parameters. \param
     * indices Indices as integer values. \param args    Variadic function
     * arguments. \return The corresponding instance of QuantumField. \sa
     * QuantumFieldParent::operator()().
     */
    template <class... Args>
     ...
     * \brief Delegates the arguments to QuantumFieldParent::operator()() of
     * the managed object. \tparam Args Variadic template parameters. \param
     * polar   Polarization index for the polarization tensor. \param indices
     * Rest of the indices, as integer values. \param args    Variadic function
     * arguments. \return The corresponding instance of PolarizationField. \sa
     * QuantumFieldParent::operator()().
     */
    template <class... Args>
    inline csl::Expr operator()(const csl::Index       &polar,
     ...
     * the managed object. \tparam Args Variadic template parameters. \param
     * polar   Polarization index for the polarization tensor. \param indices
     * Rest of the indices. \param args    Variadic function arguments. \return
     * The corresponding instance of PolarizationField. \sa
     * QuantumFieldParent::operator()().
     */
    template <class... Args>
    inline csl::Expr operator()(const csl::Index              &polar,
     ...
     * \brief Delegates the arguments to QuantumFieldParent::operator()() of
     * the managed object. \tparam Args Variadic template parameters. \param
     * indices Rest of the indices. \param args    Variadic function arguments.
...
/*!
 * \brief Class deriving from csl::TensorFieldElement, may enter expressions.
 * In contrary to QuantumFieldParent, it is not a inherited from any class.
 * \details Any quantum field (boson, fermion, gauge ...) will have
 * QuantumField instances. This simplifies the use of a generic instance,
 * QuantumField, that will ask its parent to know its own properties
 * (contractions, commutation, spin, mass, ...). All physical information is
 * contained in QuantumFieldParent and its derived classes, except the few
 * attributes of QuantumField that are instance-specific. QuantumField objects
 * will not be in general directly created by the user. Giving a
 * QuantumFieldParent or a Particle, one should used their operator()() to
 * create a QuantumField instance, as an actual expression that may enter in
 * mathematical expressions.
 * \sa QuantumFieldParent, Particle, csl::Expr, TensorFieldElement,
 * Particle::operator()().
 */
class QuantumField : public csl::TensorFieldElement {
  ...
  protected:
     ...
     * \sa isIncomingParticle(), isIncomingAntiParticle(),
     * isOutgoingParticle(), isOutgoingAntiParticle(), setIncoming(),
     * setExternal(), setParticle().
     */
    bool particle = true;
     ...
     * isIncomingAntiParticle(), isOutgoingParticle(),
    ...
    /*!
     * \brief Tells if the QuantumField, if external, is a incoming or
     * outgoing. \details As for the attribute \b particle, this boolean will
     * determine if the field must be complex conjugated or not. Depending on
     * its spin, if it is an incoming particle, outgoing anti-particle... \sa
     * isIncomingParticle(), isIncomingAntiParticle(), isOutgoingParticle(),
     * isOutgoingAntiParticle(), setIncoming(), setExternal(), setParticle().
     */
    bool incoming = true;
     ...
     * track of terms in \f$ \partial _\mu \f$ in computations while having
     * only QuantumField objects. \details This member allows to have an
     * internal representation of the following definition for a field \f$
     * A_\mu ^I \f$: \f[ \partial _\mu A_\nu ^I \equiv A^I_{\nu,\mu}. \f] The
     * derivative disappear and indices are transfered in the structure of the
     * field. \sa addDerivative(), hasDerivative(), getDerivativeStructure(),
     * wick::Graph, wick::WickCalculator, Expander, AmplitudeCalculator.
     */
    csl::IndexStructure derivativeIndices{};

    /**
     * @brief Wanted partner for fermion ordering in amplitude results,
     * together with the number defining the fermion.
     *
  ...
  public:
     ...
     * \details This constructor takes no index and concerns then scalar
     * fields. \param t_vector Space_time point of the field. \param t_parent
     * Parent (must be derived from QuantumFieldParent) of the field.
     */
  ...
  public:
     ...
     * \details As QuantumField inherits from csl::TensorFieldElement, its
     * parent is in the form of a csl::Parent. This function then makes a
     * conversion to a mty::QuantumFieldParent before returning the pointer.
     * \return A pointer to the QuantumFieldParent of the field.
     */
    inline QuantumFieldParent *getQuantumParent() const
    {
     ...
     * \details As QuantumField inherits from csl::TensorFieldElement, its
     * parent is in the form of a csl::Parent. This function then makes a
     * conversion to a mty::Particle before returning the pointer. \return A
     * shared pointer to the QuantumFieldParent (i.e. mty::Particle) of the
     * field.
     */
    ...
    {
        auto rep = getQuantumParent()->getGroupIrrep(
    ...
    {
        auto rep = getQuantumParent()->getFlavorIrrep(
            std::forward<FlavorGroupType>(group));
        return rep;
    }
     ...
     * consistent, it will raise an error. In particular, it should be used
     * only for fermions that are linked (one DiracFermion and two
     * WeylFermion). In this case, setting the chirality of the particle is
     * equivalent to change its parent. \b chirality may be left or right (a
     * WeylFermion takes the place of the parent) or none (a DiracFermion takes
     * the place of the parent). \param chirality New chirality of the
     * particle. \sa QuantumFieldParent::getDiracParent(),
    ...
    /*!
     * \brief Tests if the the field is exactly contractible with the other
     * one. \details The condition that does not appear in isContractibleWith()
     * is the complex conjugation of the field. In the first function, we test
     * only that a propagator exists between the two abstract fields. Here, we
     * test also if either the field is self-conjugate or one an only one of
     * the two fields is complex conjugated. Other wise, even if a propagator
     * exists, the contraction gives zero and this function returns false.
     * \param other Field with which we test the contraction.
     * \return \b True  if the actual contraction of the two fields is not
     * zero. \return \b False else.
     */
     ...
     */
    csl::Expr getLSZInsertion(const csl::Tensor &momentum,
                              bool               ruleMode = false,
     ...
     * outgoing particle (otherwise it would be complex conjugated).
     * Conjugating the particle, we get \f$ \phi ^*(X) \f$ an outgoing
     * antiparticle. \sa #particle, getConjugatedField().
     */
    void conjugate();
     ...
     * \f$ \psi _1 \f$ and \f$ \psi _2 \f$ we have \f$\{\psi _1, \psi
     * _2\}=0\f$. This function returns in this case true iif \b sign = +1
     ...
     * another expression.
     * \details This functoin overrides the one of csl::TensorFieldElement and
     * make sure to return false each time. \param expr Other expression.
     * \return \b False.
     */
     ...
     * \details Uses TensorFieldElement::operator==() and takes care of
     * comparing the derivative structures of two fields if the other
     * expression is another field (otherwise this function returns false).
     * \param other Expression to compare.
     * \return \b True  if the other expression is the same field, with the
     * same structure and space-time point. \return \b False else. \sa
    ...
    /*!
     * \brief Compares the field with another expression.
     * \details Uses TensorFieldElement::compareWithDummy() and takes care of
     * comparing the derivative structures of two fields if the other
     * expression is another field (otherwise this function returns false).
     * Dummy indices may compare succesfully with each other with different
     * names. Only the abstract index structure is compared. Free indices must
     * match exactly. \param other              Expression to compare. \param
     * constraints        Mapping constraints between already compared dummy
     * indices. \param keepAllCosntraints Tell if constraints must be relaxed
     * once pairs of indices are matched (default = false, the user should not
     * need that). \return \b True  if the other expression is the same field,
     * with the same structure (allowing different dummy indices) and
     * space-time point. \return \b False else. \sa
