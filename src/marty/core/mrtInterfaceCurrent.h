...
inline Amplitude SelfEnergy(mty::Order order, mty::Particle &particle)
{
    return SelfEnergy(order, particle, *mty::Model::current);
}

inline Amplitude SelfEnergy(mty::Particle &particle)
{
...
inline void AddParticle(mty::Particle &particle, bool initTerms = true)
{
    AddParticle(*mty::Model::current, particle, initTerms);
}

inline void AddTerm(csl::Expr const &term, bool addCC = false)
{
    AddTerm(*mty::Model::current, term, addCC);
}

inline void AddTerm(std::vector<csl::Expr> const &terms, bool addCC = false)
{
    AddTerm(*mty::Model::current, terms, addCC);
}

inline mty::Particle GetParticle(std::string const &name)
{
    return GetParticle(*mty::Model::current, name);
}

inline void PromoteGoldstone(mty::Particle &goldstone,
                             mty::Particle &gaugeBoson)
{
    PromoteGoldstone(*mty::Model::current, goldstone, gaugeBoson);
}

inline void PromoteGoldstone(char const *goldstone, char const *gaugeBoson)
{
    PromoteGoldstone(*mty::Model::current, goldstone, gaugeBoson);
}
...
inline void Rename(std::string const &oldName, std::string const &newName)
{
...
inline void Replaced(csl::Expr const &init, csl::Expr const &target)
{
    Replaced(*mty::Model::current, init, target);
}

inline void Replaced(csl::Tensor &init, csl::Expr const &target)
{
    Replaced(*mty::Model::current, init, target);
}

inline void Replaced(csl::Tensor &init, csl::Tensor &target)
{
    Replaced(*mty::Model::current, init, target);
}

inline void Replaced(Particle const &particle, csl::Expr const &newTerm)
{
    Replaced(*mty::Model::current, particle, newTerm);
}

inline void Rotate(std::vector<mty::Particle> const &         fields,
                   std::vector<mty::Particle> const &         newFields,
                   std::vector<std::vector<csl::Expr>> const &rotation,
                   bool diagonalizeMasses = false)
{
    Rotate(
        *mty::Model::current, fields, newFields, rotation, diagonalizeMasses);
}

inline void Rotate(std::vector<std::string> const &           fields,
...
{
    BreakGaugeSymmetry(
        *mty::Model::current, brokenGroup, brokenFields, newNames);
}
...
inline void
BreakFlavorSymmetry(std::string const &                          brokenFlavor,
                    std::initializer_list<std::string> const &   brokenFields,
                    std::vector<std::vector<std::string>> const &newNames)
{
    BreakFlavorSymmetry(
        *mty::Model::current, brokenFlavor, brokenFields, newNames);
}
...
inline void SetGaugeChoice(std::string const &nameParticle, gauge::Type choice)
{
    SetGaugeChoice(*mty::Model::current, nameParticle, choice);
}
