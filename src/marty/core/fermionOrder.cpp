...
bool ordered(mty::Insertion const &left, mty::Insertion const &right)
{
    auto shouldBeFirst = [](mty::Insertion const &field) {
        return field.isIncomingAntiParticle() || field.isOutgoingParticle();
    };
...
fermionsOf(std::vector<mty::Insertion> const &fields)
{
...
std::vector<int>
defaultFermionOrder(std::vector<mty::Insertion const *> const &fields)
{
...
void applyFermionOrder(std::vector<mty::Insertion> &insertions,
        ...
        HEPAssert(i1 >= 0 && i1 < static_cast<int>(fermions.size()),
            ...
            HEPAssert(i2 >= 0 && i2 < static_cast<int>(fermions.size()),
                      ...
                      "Fermion order index " + std::to_string(i2)
                          + " out of bounds for external fermions (size "
                          + std::to_string(fermions.size()) + ")") fermions[i1]
                ->setPartnerShip({i1, i2, true});
...
{
    applyFermionOrder(insertions, defaultFermionOrder(insertions));
}
} // namespace mty
