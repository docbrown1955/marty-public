...
Spectrum::Spectrum(std::vector<mty::Particle> const &         t_particles,
...
Spectrum::Spectrum(std::vector<mty::Particle> const &         t_newParticles,
    ...
    : bidiagonalization(!t_mixing2.empty()),
      ...
      replaceMixings(true),
      newFields(t_newParticles),
      mass(t_mass.size(), (t_mass.empty()) ? 0 : t_mass[0].size()),
      mixing(t_mixing.size(), (t_mixing.empty()) ? 0 : t_mixing[0].size()),
      mixing2(t_mixing2.size(), (t_mixing2.empty()) ? 0 : t_mixing2[0].size())
{
...
Spectrum::Spectrum(std::vector<mty::Particle> const &         partA,
    ...
    : bidiagonalization(true), replaceMasses(true), replaceMixings(true)
{
...
void Spectrum::setBlock(std::vector<csl::Expr> const &             terms,
                        std::vector<std::vector<csl::Expr>> const &t_mixing)
{
...
void Spectrum::updateData()
{
    ...
    bool         numerical = true;
    const size_t size
        = (bidiagonalization) ? fields.size() / 2 : fields.size();
    for (size_t i = 0; i != size; ++i) {
        for (size_t j = i; j < size; ++j) {
            // std::cout << mass(i, j) << std::endl;
            massData[i][j]
                = csl::Evaluated(mass(i, j) / ((i != j) ? 2 : 1),
...
void Spectrum::replace(csl::Expr &                   expr,
...
void Spectrum::applyOn(csl::Expr &expr) const
{
    ...
    if (!isDiagonalized())
        return;
    (bidiagonalization) ? applyBidiagonalizationOn(expr)
...
size_t Spectrum::getFieldPos(QuantumField const &field) const
{
    ...
    CallHEPError(mty::error::TypeError,
                 "Field " + toString(field.copy())
                     + " does not correspond to "
                       " mass block.") return size_t(-1);
}

Spectrum::MatrixEl Spectrum::getMassFromTerm(csl::Expr const &term) const
{
    if (csl::IsSum(term)) {
    ...
    csl::ForEachLeaf(expression, [&](csl::Expr &sub) {
        if (csl::IsPow(sub) && IsOfType<QuantumField>(sub[0])
            && csl::IsInteger(sub[1])) {
    ...
    HEPAssert(massFields.size() >= 1,
        ...
        HEPAssert(massFields.size() <= 2,
                  mty::error::RuntimeError,
                  "Term " + toString(term)
                      + " contains more than two fields !") bool fermionSign
        = massFields[0].isFermionic() && !massFields[0].isComplexConjugate()
    ...
    return Spectrum::MatrixEl({i, j, expression});
}
...
void Spectrum::bidiagonalize()
{
    ...
    HEPAssert(D2 == diagonalizer.getDiagonal(),
              ...
              "Bidiagonalization found two different diagonal matrices, "
                  + toString(D2) + " and "
                  + toString(diagonalizer.getDiagonal()))
}
...
void Spectrum::applyDiagonalizationOn(csl::Expr &expr) const
{
    ...
    for (size_t i = 0; i != size; ++i)
        for (size_t j = 0; j != size; ++j) {
            ...
            // std::cout << "Replacing" << std::endl;
            // std::cout << oldMixing[index] << "  " << newMixing[index] <<
            // std::endl;
        }
    if (replaceMixings)
