...
class ModelData {
  ...
  public:
     ...
     * \details Instantiates a null flavor, and the gauge to \b t_gauge.
     * Calling this constructor, the gauge you give to the model is no longer
     * valid as the unique ownership is transfered to it. Using this
     * constructor, the gauge is taken as is, and the model is initialized. It
     * is in particular not possible to add gauged groups afterwards. If you
     * want to add gauge groups through the addGaugedGroup() function, consider
     * using the default constructor. Here is a code snippet that show how to
     * use this constructor. \code std::unique_ptr<mty::Gauge> gauge =
     * makeGauge(); Model model(std::move(gauge));
    ...
    getPhysicalParticles(std::function<bool(mty::Particle)> const &predicate
     ...
     * with it. This coupling is stored in the model's scalar couplings that
     * you can get with this function (as a constant reference). \return The
     * list of scalar couplings of the theory.
     */
     ...
     * space-time calculations than 4D Minkowski but it does not support it
     * yet.
     ...
     * the ownership of the gauge and is not valid after the function call.
     * Here is a code snippet that show how to use this function. \code Model
     * model; std::unique_ptr<mty::Gauge> gauge = makeGauge();
     ...
     * initializing it (see init()) and adding matter particles. Given the
     * type, the name, the dimension and (optionnaly) a coupling constant, the
     ...
     */
    void addGaugedGroup(group::Type      type,
     ...
     * @details This function allow to add a gauged group to the model, before
     * initializing it (see init()) and adding matter particles. Given the
     * type, the name, and (optionnaly) a coupling constant, the corresponding
     * group, gauge boson, ghosts are created. Here the dimension is not given.
     * This function must then be used only for U(1), and exceptional algebras.
     *
     ...
     * @param initializeTerms Boolean (default = true) that tells if kinetic
     * and mass terms must be added automatically.
     *
     * @sa removeParticle()
     */
     ...
     * @sa removeParticle()
     */
    void addParticles(std::vector<mty::Particle> const &parts,
     ...
     * @brief Adds a scalar coupling constant in the model.
     *
     * @details This function adds \b coupling to the list of coupling
     * constants.
     *
    ...
    std::vector<mty::Particle>
    getParticles(std::initializer_list<std::string_view> names) const;
    /**
     ...
     * @details In case no particle is found, an error message is printed and
     * the code stops. If you want to know if a particle exists, consider using
     * findParticle() instead. The expression \b field must of course be of
     * type QuantumField, i.e. pointing to an object of this type.
     *
     * @param field Field from which you want a mty::Particle type.
     *
     * @return The mty::Particle (a std::shared_ptr to \b field's parent).
     ...
     */
    template <class GroupType, class FieldType>
    csl::Space const *getVectorSpace(GroupType &&group,
     ...
     */
    template <class GroupType, class FieldType>
    std::vector<csl::Index> generateIndices(size_t      nIndices,
                                            GroupType &&group,
                                            FieldType &&field) const;
     ...
     * must be of course gauge invariant (see checkValidity()), and have no
     * free indices (scalar term). Setting \b addHermitic to true, to hermitian
     ...
     * h.c, \f] (in that case the '+h.c' corresponds to \b addHermitic = true),
     * or \f[ \mathcal{L} \ni \frac{2}{3}e\bar{u}A\cdot \gamma u, \f] that do
     * not have an hermitian counter part.
     *
     ...
     * conjugate of the term must be also added
     */
    void addLagrangianTerm(mty::Lagrangian::TermType const &term,
     ...
     * @param out       Output stream (std::cout, standard stream by default)
     */
    void printSubPart(std::initializer_list<std::string> particles,
                      std::ostream &out = std::cout) const;
    /**
     * @brief Prints a sub part of the Lagrangian including some particles.
     *
  ...
  protected:
     ...
     *
     * @return The flavor group representation of \b part or the group \b
     * group.
     *
     * @sa getFlavorIrrep()
     */
  ...
  protected:
     ...
     * function. Only then, particles can be added using addParticle()
     * function.
     *
    ...
    /**
     * @brief Boolean telling if checks must be done with lagrangian terms
     * added to the model (gauge representation, dispatching ...)
     *
...
mty::GaugeIrrep ModelData::getGaugeIrrep(FieldType &&field) const
{
    ...
    if constexpr (field_valid)
        ...
        return getGaugeIrrep(getParticle(std::forward<FieldType>(field)));
}

template <class GroupType, class FieldType>
csl::Expr ModelData::getCharge(FieldType &&field, GroupType &&group) const
{
    constexpr bool field_valid
        = std::is_convertible<FieldType, mty::Particle const &>::value;
    ...
    if constexpr (group_valid) {
        if constexpr (field_valid)
            ...
            return getCharge(getParticle(std::forward<FieldType>(field)),
                             group);
    }
    else if constexpr (field_valid)
        return getCharge(field, getGroup(std::forward<GroupType>(group)));
    else
        return getCharge(getParticle(std::forward<FieldType>(field)),
...
template <class GroupType, class FieldType>
mty::Irrep ModelData::getGroupIrrep(FieldType &&field, GroupType &&group) const
{
    constexpr bool field_valid
        = std::is_convertible<FieldType, mty::Particle const &>::value;
    ...
    if constexpr (group_valid) {
        if constexpr (field_valid)
            ...
            return getGroupIrrep(getParticle(std::forward<FieldType>(field)),
                                 group);
    }
    else if constexpr (field_valid)
        return getGroupIrrep(field, getGroup(std::forward<GroupType>(group)));
    else
        return getGroupIrrep(getParticle(std::forward<FieldType>(field)),
                             getGroup(std::forward<GroupType>(group)));
}
...
template <class GroupType, class FieldType>
csl::Space const *ModelData::getVectorSpace(GroupType &&group,
                                            FieldType &&field) const
{
    constexpr bool group_valid
        = std::is_convertible<GroupType, mty::Group const *>::value;
    ...
    if constexpr (group_valid) {
        ...
        return getVectorSpace(getGroup(std::forward<GroupType>(group)), field);
    else
        return getVectorSpace(getGroup(std::forward<GroupType>(group)),
                              getParticle(std::forward<FieldType>(field)));
}

template <class GroupType, class FieldType>
csl::Index ModelData::generateIndex(GroupType &&group, FieldType &&field) const
{
    constexpr bool group_valid
        = std::is_convertible<GroupType, mty::Group const *>::value;
    ...
    if constexpr (group_valid) {
        if constexpr (field_valid)
            ...
            return generateIndex(group,
                                 getParticle(std::forward<FieldType>(field)));
    }
    else if constexpr (field_valid)
        return generateIndex(getGroup(std::forward<GroupType>(group)), field);
    else
...
template <class GroupType, class FieldType>
std::vector<csl::Index> ModelData::generateIndices(size_t      nIndices,
                                                   GroupType &&group,
                                                   FieldType &&field) const
{
    ...
    for (size_t i = 0; i != nIndices; ++i)
        indices[i] = generateIndex<GroupType, FieldType>(
            std::forward<GroupType>(group), std::forward<FieldType>(field));
                                                   ...
                                                   GroupType &&group) const
{
    std::vector<csl::Index> indices(nIndices);
    for (size_t i = 0; i != nIndices; ++i)
        indices[i] = generateIndex<GroupType>(std::forward<GroupType>(group));
    ...
    if constexpr (group_valid) {
        if constexpr (field_valid)
            ...
            return generateIndex(
                nameIndex, group, getParticle(std::forward<FieldType>(field)));
    }
    else if constexpr (field_valid)
        return generateIndex(
            nameIndex, getGroup(std::forward<GroupType>(group)), field);
    else
        return generateIndex(nameIndex,
                             getGroup(std::forward<GroupType>(group)),
                             getParticle(std::forward<FieldType>(field)));
}

template <class GroupType, class FieldType>
mty::Generator ModelData::getGenerator(GroupType &&group,
                                       FieldType &&field) const
{
    constexpr bool group_valid
        = std::is_convertible<GroupType, mty::Group const *>::value;
    ...
    if constexpr (group_valid) {
        if constexpr (field_valid)
            ...
            return getGenerator(group,
                                getParticle(std::forward<FieldType>(field)));
    }
    else if constexpr (field_valid)
        return getGenerator(getGroup(std::forward<GroupType>(group)), field);
    else
        return getGenerator(getGroup(std::forward<GroupType>(group)),
                            getParticle(std::forward<FieldType>(field)));
}

template <class FieldType>
void ModelData::addBosonicMass(FieldType &&field, std::string_view mass)
{
    constexpr bool field_valid
        = std::is_convertible<FieldType, mty::Particle const &>::value;
    ...
    if constexpr (field_valid)
        ...
        addBosonicMass(getParticle(std::forward<FieldType>(field)), mass);
}
template <class FieldType>
void ModelData::addBosonicMass(FieldType &&field, csl::Expr const &mass)
{
    constexpr bool field_valid
        = std::is_convertible<FieldType, mty::Particle const &>::value;
    ...
    if constexpr (field1_valid) {
        if constexpr (field2_valid)
            ...
            return addFermionicMass(
                left, getParticle(std::forward<FieldType2>(right)), mass);
    }
    else if constexpr (field2_valid)
        return addFermionicMass(
            getParticle(std::forward<FieldType1>(left)), right, mass);
    else
        return addFermionicMass(getParticle(std::forward<FieldType1>(left)),
                                 ...
                                 std::string_view mass)
{
    ...
    if constexpr (field_valid)
        ...
        addFermionicMass(getParticle(std::forward<FieldType>(diracFermion)),
                         mass);
}
template <class FieldType>
void ModelData::addFermionicMass(FieldType      &&diracFermion,
...
template <class FieldType>
void ModelData::renameParticle(FieldType &&field, std::string_view newName)
{
    constexpr bool field_valid
        = std::is_convertible<FieldType, mty::Particle const &>::value;
    ...
    if constexpr (field_valid)
        ...
        return renameParticle(getParticle(std::forward<FieldType>(field)),
                              newName);
}
