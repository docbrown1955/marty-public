...
class iterable_view {
  ...
  public:
    iterable_view(const Containor &t_iterable)
        : iterable(t_iterable), custom_begin(0), custom_end(t_iterable.size())
    {
    }

    iterable_view(const Containor &t_iterable, size_t b, size_t e)
        : iterable(t_iterable), custom_begin(b), custom_end(e)
    {
        if (not(0 <= b and b <= e and e <= iterable.size()))
            CallHEPError(mty::error::TypeError,
                         "Wrong positions in construction of "
                         "custom_iterable_view()."
                             + toString(b) + " and " + toString(e)
                             + " for a containor " + "of size "
                             + toString(iterable.size()));
    }
