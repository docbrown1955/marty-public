...
static bool
checkInsertionsForPatch(std::vector<mty::Insertion> const &insertions,
                        bool                               raiseError)
{
    ...
    size_t nF = std::count_if(
        insertions.begin(), insertions.end(), [&](mty::Insertion const &part) {
            return part.getField()->isFermionic();
        });
    size_t nmB = std::count_if(
        insertions.begin(), insertions.end(), [&](mty::Insertion const &part) {
...
static std::pair<csl::Expr, csl::Expr> getFermions(csl::Expr const &op)
{
    ...
    HEPAssert(fermions.first && fermions.second,
              ...
              "operator.") return fermions;
}

static std::pair<size_t, size_t>
checkOperators(std::vector<Wilson> const &     wilsons,
               std::vector<csl::Tensor> const &momenta)
{
    ...
    for (const auto &wil : wilsons) {
        ...
        HEPAssert((ic == posFermions.first || posFermions.first == npos)
                      && (i == posFermions.second
                          || posFermions.second == npos),
                  mty::error::RuntimeError,
                  "Inconsistent fermion alignement, consider fermion "
                  "ordering.") posFermions
            = {ic, i};
    }
    HEPAssert(posFermions.first != npos && posFermions.second != npos,
              mty::error::RuntimeError,
              "Inconsistent fermion alignement, consider fermion "
              "ordering.") return posFermions;
}
...
static PenguinPatchData getPenguinData(
    std::vector<mty::Insertion> const &insertions, size_t iConj, size_t iReg)
{
...
static void orderOperators(std::vector<Wilson> &wilsons,
                           csl::Tensor const &  p1,
                           csl::Tensor const &  p2)
{
...
static void
ensureOperator(Wilson const &model, Wilson &target, csl::Tensor const &p)
{
    ...
    HEPAssert(momentum,
              mty::error::RuntimeError,
              "Momentum not found in " + toString(op) + ".") for (auto &arg :
                                                                  op)
    {
...
static void ensureOperators(std::vector<Wilson> &wilsons,
                            csl::Tensor const &  p1,
                            csl::Tensor const &  p2)
{
...
static void applyPenguinPatch_implementation(std::vector<Wilson> &   wilsons,
                                             PenguinPatchData const &data)
{
    ...
    csl::Expr Lambda1 = data.s1 * csl::pow_s(data.m1, 2) + data.s2 * data.s_12;
    csl::Expr Lambda2 = data.s2 * csl::pow_s(data.m2, 2) + data.s1 * data.s_12;
    csl::Expr qSquared
        = data.s1 * Lambda1 + data.s2 * Lambda2 + Propagator::Peps;
    ...
    if (eval_m1 == eval_m2) {
        if (data.eta1 * data.s1 == data.eta2 * data.s2) {
            O3.coef.setCoefficient(-F * qSquared / (2 * m1_tilde));
            O3c.coef.setCoefficient(CSL_0);
        }
        else {
            O3.coef.setCoefficient(CSL_0);
            O3c.coef.setCoefficient(Fc * qSquared / (2 * m1_tilde));
        }
    }
    else {
        O3.coef.setCoefficient(-F * qSquared / (m1_tilde + m2_tilde));
        O3c.coef.setCoefficient(Fc * qSquared / (m1_tilde - m2_tilde));
    }
}
