...
class DiracSpace : public csl::Space {
  ...
  public:
    ...
    struct FiertzContraction {
        ...
        bool isChiral() const
        {
            return i_chir != Chirality::None and j_chir != Chirality::None;
        }
  ...
  public:
    ...
    void contractTensor(std::vector<csl::Index> &tensors,
                        std::vector<size_t> &    cuts,
                        size_t                   i,
                        size_t                   j) const;

    std::vector<Chain>
    simplifyGammaProd(std::vector<csl::Index> const &indices,
    ...
    std::vector<FiertzContraction>
    getFiertzContractions(Chain const &init) const;

    std::vector<Chain> simplifyFiertzProd(Chain const &init) const;

    void applyUniqueChiralityStructure(std::vector<csl::Index> &tensors,
                                       std::vector<size_t> &    cuts,
    ...
    std::vector<csl::Index> applyInvolution(csl::vector_expr &tensors,
                                            csl::Expr &       factor) const;

    void applyChiralityProp(std::vector<csl::Index> &tensors,
                            csl::Expr &              factor) const;

    void simplifySquares(std::vector<csl::Index> &tensors,
                         csl::Expr &              factor) const;

    csl::Expr applyRecursion(std::vector<csl::Index> &tensors,
                             csl::Expr &              factor) const;

    csl::Expr applyStandardRecursion(std::vector<csl::Index> const &tensors,
                                     bool first = true) const;

    csl::Expr applyChiralRecursion(std::vector<csl::Index> const &tensors,
                                   bool first = true) const;
    ...
    std::vector<csl::Index> exprToIndex(std::vector<csl::Expr> const &tensors,
                                        std::vector<size_t> &cuts) const;
...
class ConjugationSimplifier {
  ...
  public:
    ...
    bool hasContractionProperty(csl::Expr_info self,
                                csl::Expr_info other) const;

    csl::Expr contraction(csl::Expr_info self, csl::Expr_info other) const;

  private:
    bool commutation(csl::Index &first,
                     csl::Index &second,
                     csl::Expr & tensor) const;

    bool commutationWithTensor(csl::Index &first,
                               csl::Index &second,
                               csl::Expr & tensor) const;
    bool commutationWithIntegral(csl::Index &first,
                                 csl::Index &second,
                                 csl::Expr & tensor) const;
    bool commutationWithProduct(csl::Index &first,
                                csl::Index &second,
                                csl::Expr & prod) const;
    bool commutationWithSum(csl::Index &first,
                            csl::Index &second,
                            csl::Expr & sum) const;
