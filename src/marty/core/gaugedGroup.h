...
enum class GroupType { U1, SU, SO, Sp, E6, E7, E8, F4, G2 };
...
namespace mty::gauge {
/*!
 * \brief Different types of gauge ficing parameter for gauge boson
 * propagators. \details The propagator of a gauge boson is \f[
 *      -i\frac{g_{\mu\nu} - (1-\xi) \frac{p_\mu p_\nu}{p^2 - \xi
 * M^2}}{p^2-M^2}, \f]with \f$ M \f$ the mass of the particle, and \f$ p^\mu
 * \f$ its momentum. The Lorenz gauge corresponds to \f$ \xi=0\f$, the
 * propagator then reads \f[ -i\frac{g_{\mu\nu} - \frac{p_\mu
 * p_\nu}{p^2}}{p^2-M^2}. \f] The Feynman gauge corresponds to \f$ \xi=1\f$,
 * the propagator then reads \f[ -i\frac{g_{\mu\nu}}{p^2-M^2}. \f] The Unitary
 * gauge corresponds to \f$ \xi=\infty\f$, the propagator then reads \f[
 *      -i\frac{g_{\mu\nu} - \frac{p_\mu p_\nu}{M^2}}{p^2-M^2}.
 * \f]
...
class Choice {
  ...
  public:
    Choice() : choice(Type::Feynman), xi(csl::variable_s("xi"))
    {
...
class IrrepMap {
  ...
  private:
    inline static mty::Irrep &key(Pair &p)
    {
        return p.first;
    };
    inline static Type &value(Pair &p)
    {
        return p.second;
    };
    inline static mty::Irrep const &key(Pair const &p)
    {
        return p.first;
    };
    inline static Type const &value(Pair const &p)
    {
        return p.second;
    };
...
class GaugedGroup {
  ...
  protected:
    ...
    mutable std::map<size_t, csl::Tensor> d;
