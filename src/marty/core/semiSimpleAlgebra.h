 ...
 * more technical details see the manual that explains more deeply some
 * concepts and gives references.
 */
 ...
 * representation from highest weight, product of representations...) except
 * the initialization of the cartan matrix and the norms of simple roots, that
 * is overriden in derived classes, specializations of semi simple Lie
 * algebras.
 */
class SemiSimpleAlgebra {
  ...
  public:
     ...
     */
    explicit SemiSimpleAlgebra(int t_l);
     ...
     */
    virtual Irrep highestWeightRep(const AlgebraState &highestWeight,
     ...
     * \return The Irrep of highest weight \b highestWeight.
     */
    virtual Irrep highestWeightRep(const std::vector<int> &highestWeight,
                                   bool                    mult = true) const;
     ...
     */
    virtual SumIrrep
    tensorProduct(const Irrep &A, const Irrep &B, bool mult = true) const;
    virtual SumIrrep
    tensorProduct(const SumIrrep &A, const Irrep &B, bool mult = true) const;
    virtual SumIrrep
             tensorProduct(const Irrep &A, const SumIrrep &B, bool mult = true) const;
     ...
     * \param rep  std::vector of AlgebraState to sort, modified during the
     * run. \param mult multiplicities of the states, sorted at the same time
     * to keep the one to one correspondance between states and multiplicities.
     */
    virtual void sortRep(std::vector<AlgebraState> &rep,
  ...
  private:
     ...
     * group. If the state is positive, it is itself, else the function
     * operates a series of Weyl reflections until it finds the corresponding
     * positive state. The sign of the Transformation is one if the number of
     * reflections linking the two states is even, -1 else. The sign is stored
     * in \b sign during the run of the function. \param state Initial
     * AlgebraState. \param sign  Integer modified during the run, stores the
     * sign of the Transformation. \return A new AlgebraState, reflection of \b
     * state in the dominant Weyl chamber.
     */
     ...
     * \details The weyl group is stored by keeping all positive roots in
     * memory in the member \b positiveRoots. Any weyl reflection (composition
     * of weyl reflections for simple roots) is the weyl reflection wrt one
     * single positive root. Having all positive roots is then equivalent to
     * have all weyl reflections.
     */
     ...
     * \return A csl::Vector expression of the root space-equivalent of \b
     * state.
     */
     ...
     */
    std::vector<AlgebraState> getSingleChain(const AlgebraState &state,
                                             int direction) const;
     ...
     * d, then this direction will not be explored again getting the root
     * chains recursively for this state. The new states are added in the
     * std::vector \b states (copied at the beginning of the function), and
     * this std::vector is returned. \param highestWeightState Initial state of
     * the root chains. \param states             Current set of stated found
     * in the irrep. \param directions         Set of already explored
     * directions (along simple roots) for the state \b highestWeightState,
     * that we do not compute again. \return A modified copy of \b states,
     * where new states have been added.
     */
    std::vector<AlgebraState>
    getRootChain(const AlgebraState &      highestWeightState,
                 std::vector<AlgebraState> states
    ...
    void
    getRootChainExperimental(const AlgebraState &       highestWeightState,
                             std::vector<AlgebraState> &states,
                             std::vector<std::vector<bool>> &directions) const;

    /*!
     * \brief Computes the multiplicity of the state \b state.
     * \details The multiplicity of the state depends on those of highest
     * weight states. This function is then recursive and calculates on the go
     * the multiplicities it needs if they have not been alreadt calculated.
  ...
  protected:
     ...
     * integers, the 1/2 factor is discarded in this member, it corresponds
     * then strictly to 2*rho in standard conventions.
     */
    std::vector<int> rho;

    /*!
     * \brief Absolute norms of simple roots used in the computation of the
     * quadratic Casimir operator. See
     * SemiSimpleAlgebra::getQuadraticCasimir().
     */
    ...
    mutable std::vector<std::pair<AlgebraState, Irrep>> irreps;
...
class R : public mty::SemiSimpleAlgebra {
  ...
  public:
    ...
    /*!
     * \brief Overrides the function of mty::SemiSimpleAlgebra and does nothing
     * (no Cartan matrix).
     */
     ...
     */
    mty::Irrep highestWeightRep(const mty::AlgebraState &highestWeight,
                                bool mult = true) const override;

    /*!
     * \brief Creates a representation from a std::vector of int containing
     * only the numerator and the denominator of the U(1) charge. \return The
     * U(1) Irrep with the right charge.
     */
    mty::Irrep highestWeightRep(const std::vector<int> &highestWeight,
                                bool mult = true) const override;
...
class B : public mty::SemiSimpleAlgebra {
  ...
  public:
    /*!
     * \brief Constructor with one parameter.
     * \param t_l Initializes the order \b l of the algebra. Generates
     * so(2l+1).
     */
    explicit B(int t_l);
