...
PolarizationField::PolarizationField() : PolarizationField(nullptr, nullptr)
{
}

PolarizationField::PolarizationField(const Tensor &impulsion,
                                     const Parent &t_parent)
    : QuantumField(impulsion, t_parent), lockConjugation(false)
{
...
PolarizationField::PolarizationField(const Tensor &            impulsion,
    ...
    : QuantumField(impulsion, t_parent, indices), lockConjugation(false)
...
PolarizationField
PolarizationField::complexConjugatedField(bool keepFermionOrder) const
{
...
{
    return index.size() > 0 and index[0] == other->getIndexStructureView()[0];
}
...
bool PolarizationField::hasContractionProperty(csl::Expr_info other) const
{
    ...
    if (fermion and csl::IsIndicialTensor(other)
...
csl::Expr PolarizationField::contraction(csl::Expr_info other) const
{
    HEPAssert(csl::IsIndicialTensor(other),
              mty::error::TypeError,
              "Polarization field " + toString(copy())
                  + " does not contract "
                    "with non tensor "
                  + toString(other->copy()) + ".");
    if (onShell and csl::IsIndicialTensor(other)
        and other->getParent_info() == point.get())
        return CSL_0;
...
csl::Expr
PolarizationField::matrixChargeConjugation(csl::Expr_info other) const
{
    if (lockConjugation) {
...
bool PolarizationField::hasFieldChargeConjugation(
    mty::PolarizationField const *other) const
{
...
csl::Expr PolarizationField::sumPolarization(csl::Expr_info other) const
{
    if (!conjugated or other->isComplexConjugate()) {
        ...
        case 1: {
            ...
            if (!signMassTerm) {
                auto res = deltaFactor * C
                           * (dirac4.gamma({+mu, alpha, beta}) * P(mu)
        ...
        default:
            CallHEPError(mty::error::NotImplementedError,
                         "Spin(x2) " + toString(spin_x_2)
                             + " not recognized for " + "polarization sum.");
            return CSL_UNDEF;
        }
    }
    else {
...
void PolarizationField::printCode(int mode, std::ostream &out) const
{
