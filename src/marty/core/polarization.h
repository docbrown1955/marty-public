 ...
 * polarization sums in cross-sections. Is derived from TensorFieldElement:
 * possess at least one csl::Index (polarization) and one "space-time point"
 * that is the impulsion of the particle.
 */
class PolarizationField : public mty::QuantumField {
  ...
  public:
    ...
    friend std::ostream &operator<<(std::ostream &           fout,
                                    const PolarizationField &pol);

  private:
    PolarizationField complexConjugatedField(bool keepFermionOrder
                                             = false) const;
    ...
    csl::Expr
    fieldChargeConjugation(mty::PolarizationField const *other) const;
