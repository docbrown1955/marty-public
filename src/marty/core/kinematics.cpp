    ...
    : insertions(t_insertions), indices(t_indices)
{
    ...
    std::generate(begin(momenta), end(momenta), [&, i = 0]() mutable {
        auto p
            = csl::Tensor("p_" + std::to_string(indices[i]), &csl::Minkowski);
        ++i;
        return p;
    });
    initMomentaSquared(indices);
...
{
    return A.getField() == B.getField() && A.isIncoming() == B.isIncoming()
...
void Kinematics::setExternalSpinTensors(std::vector<csl::Expr> const &terms)
{
    HEPAssert(terms.size() == size(),
              mty::error::IndexError,
              "Invalid number of spin tensors (" + std::to_string(terms.size())
                  + ") for a Kinematics object of size "
                  + std::to_string(size()) + ".") auto isSpinTensor
...
void Kinematics::setSquaredMomentum(size_t i, size_t j, csl::Expr const &res)
{
...
void Kinematics::applyPermutation(std::vector<size_t> const &pos)
{
    HEPAssert(pos.size() == size(),
              ...
              "Expecting permutation of the same size as the object "
                  ...
                  + std::to_string(size())
                  + ".") if (std::is_sorted(begin(pos), end(pos))) return;
    *this = subset(pos);
}
...
Kinematics Kinematics::alignedWith(Kinematics const &other) const
{
    HEPAssert(size() == other.size(),
              ...
              "Cannot align the following kinematics " + toString(*this) + '\n'
                  + toString(other)) std::vector<mty::Insertion>
                             alignedInsertions = insertions;
...
Kinematics Kinematics::applyIndices(std::vector<size_t> const &t_indices) const
{
                         ...
                         Kinematics const &k2)
{
    HEPAssert(k1.size() == k2.size(),
              ...
              "Kinematics should have the same size for replacement, got "
                  + std::to_string(k1.size()) + " and "
                  + std::to_string(k2.size())
                  + ".") if (k1.externalSpinTensors.size()
                             == k2.externalSpinTensors.size())
    {
        for (size_t i = 0; i != k1.externalSpinTensors.size(); ++i) {
            ...
            HEPAssert(isSpin1Def == isSpin2Def,
                      ...
                      "kinematics replacement.") if (isSpin1Def)
            {
...
Kinematics Kinematics::merge(Kinematics const &k1, Kinematics const &k2)
{
    Kinematics   res;
