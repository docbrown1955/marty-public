...
std::vector<csl::Index> DiracIndices(size_t n, std::string const &nameIndex)
{
    std::vector<csl::Index> indices(n);
    for (size_t i = 0; i != n; ++i)
...
SelfEnergy(mty::Order order, mty::Particle &particle, mty::Model &model)
{
    ...
    return model.computeAmplitude(
        order, {Incoming(OffShell(particle)), Outgoing(OffShell(particle))});
}
...
void Display(std::vector<FeynmanRule> const &rules, std::ostream &out)
{
    if (mty::option::searchAbreviations and mty::option::displayAbbreviations)
...
void Show(std::vector<FeynmanRule> const &rules, size_t first, size_t last)
{
    HEPAssert(first < last && first < rules.size() && last < rules.size(),
              ...
              "Bad indices " + std::to_string(first) + " and "
                  + std::to_string(last)
                  + " for Feynman rule container of size "
                  + std::to_string(rules.size()))
        Show(std::vector<FeynmanRule>{rules.begin() + first,
                                      rules.begin() + last});
}
void Show(std::vector<std::shared_ptr<wick::Graph>> const &diagrams,
          ...
          size_t                                           last)
{
    HEPAssert(first < last && first < diagrams.size()
                  && last < diagrams.size(),
              mty::error::IndexError,
              "Bad indices " + std::to_string(first) + " and "
        ...
        Show(std::vector<std::shared_ptr<wick::Graph>>{
            diagrams.begin() + first, diagrams.begin() + last});
}
...
void SaveDiagrams(std::string const &fileName, mty::Amplitude const &ampl)
{
    SaveDiagrams(fileName, ampl.obtainGraphs());
}

void Display(WilsonSet const &wilsons, std::ostream &out)
{
    if (mty::option::searchAbreviations and mty::option::displayAbbreviations)
        DisplayAbbreviations(out);
...
void DisplayAbbreviations(std::string const &name, std::ostream &out)
{
...
void AddParticle(mty::Model &model, mty::Particle &particle, bool initTerms)
{
    model.addParticle(particle, initTerms);
}

void AddTerm(mty::Model &model, csl::Expr const &term, bool addCC)
{
...
{
    model.promoteToGoldstone(std::string(goldstone), std::string(gaugeBoson));
}
...
void BreakFlavorSymmetry(mty::Model &                      model,
                         std::string const &               brokenFlavor,
...
void IntegrateOutParticle(mty::Particle &particle, bool value)
{
