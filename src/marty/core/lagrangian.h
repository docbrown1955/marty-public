...
class Lagrangian {
  ...
  public:
    ...
    friend std::ostream &operator<<(std::ostream &fout, const Lagrangian &L);
...
inline size_t Lagrangian::size() const
{
    return interaction.size();
}

inline size_t Lagrangian::fullSize() const
{
    return kinetic.size() + mass.size() + interaction.size();
}

inline bool Lagrangian::empty() const
{
    return begin() == end();
}

inline bool Lagrangian::totalEmpty() const
{
    return kinetic.empty() and mass.empty() and interaction.empty();
}

inline Lagrangian::TermType Lagrangian::operator[](size_t pos) const
{
    return interaction[pos];
}

inline Lagrangian::TermType &Lagrangian::operator[](size_t pos)
{
    return interaction[pos];
}

inline std::vector<Lagrangian::TermType>::iterator Lagrangian::begin()
{
    return interaction.begin();
}

inline std::vector<Lagrangian::TermType>::iterator Lagrangian::end()
{
    return interaction.end();
}

inline std::vector<Lagrangian::TermType>::const_iterator
Lagrangian::begin() const
{
    return interaction.begin();
}

inline std::vector<Lagrangian::TermType>::const_iterator
Lagrangian::end() const
{
    return interaction.end();
}
