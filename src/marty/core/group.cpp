...
{
    switch (type) {
    case mty::group::Type::Trivial:
        fout << "Trivial";
        break;
    case mty::group::Type::O:
        fout << "O";
        break;
    case mty::group::Type::SO:
        fout << "SO";
        break;
    case mty::group::Type::U1:
        fout << "U1";
        break;
    case mty::group::Type::SU:
        fout << "SU";
        break;
    case mty::group::Type::Sp:
        fout << "Sp";
        break;
    case mty::group::Type::E6:
        fout << "E6";
        break;
    case mty::group::Type::E7:
        fout << "E7";
        break;
    case mty::group::Type::E8:
        fout << "E8";
        break;
    case mty::group::Type::F4:
        fout << "F4";
        break;
    case mty::group::Type::G2:
        fout << "G2";
        break;
    case mty::group::Type::Lorentz:
        fout << "Lorentz";
        break;
    }
...
AbstractGroup::AbstractGroup(int t_dim, const string &t_name)
    : dim(t_dim), name(t_name)
{
}
...
void SemiSimpleGroup::setAlgebra(const SemiSimpleAlgebra &t_algebra)
{
...
mty::Irrep
SemiSimpleGroup::highestWeightRep(const vector<int> &highestWeight) const
{
...
SumIrrep
SemiSimpleGroup::tensorProduct(const initializer_list<int> &rep1,
                               const initializer_list<int> &rep2) const
{
...
const Space *SemiSimpleGroup::getVectorSpace(const mty::Irrep &irrep) const
{
    ...
    auto         iter  = find_if(vectorSpaces.begin(),
                        vectorSpaces.end(),
    ...
    if (iter == vectorSpaces.end()) {
        ...
        vectorSpaces.push_back(
            make_pair(irrep.getHighestWeight(),
                      new color::ColorSpace(
                          this, name, irrep.getDim(), irrep.getDim() == dim)));
...
csl::Expr
SemiSimpleGroup::getQuadraticCasimir(const AlgebraState &highest) const
{
    auto pos = quadraticCasimir.find(highest);
    if (pos != quadraticCasimir.end())
        return pos->second;

    quadraticCasimir[highest] = getAlgebra()->getQuadraticCasimir(highest);
...
Index SemiSimpleGroup::getCorrespondingIndex(const std::string &index_name,
...
U1::U1(const string &t_name) : SemiSimpleGroup(mty::algebra::R(), t_name)
{
}
...
SU::SU(int t_dim) : SemiSimpleGroup(mty::algebra::A(t_dim - 1), "")
{
    dim = t_dim;
}

SU::SU(const string &t_name, int t_dim)
...
SO::SO(int t_dim) : SemiSimpleGroup("")
{
    dim = t_dim;
    if (t_dim % 2 == 0)
...
SO::SO(const string &name, int t_dim) : SemiSimpleGroup(name)
{
    dim = t_dim;
    if (t_dim % 2 == 0)
        algebra.reset(new mty::algebra::D(t_dim / 2));
...
Sp::Sp(const string &t_name, int t_dim)
...
E6::E6(const string &t_name) : SemiSimpleGroup(mty::algebra::E6(), t_name)
{
}
...
E7::E7(const string &t_name) : SemiSimpleGroup(mty::algebra::E7(), t_name)
{
}
...
E8::E8(const string &t_name) : SemiSimpleGroup(mty::algebra::E8(), t_name)
{
}
...
F4::F4(const string &t_name) : SemiSimpleGroup(mty::algebra::F4(), t_name)
{
}
...
G2::G2(const string &t_name) : SemiSimpleGroup(mty::algebra::G2(), t_name)
{
}
...
Lorentz::Lorentz() : SemiSimpleGroup(mty::algebra::D(2), "")
{
    vectorSpaces.push_back(pair<mty::AlgebraState, const Space *>(
        mty::AlgebraState({1, 0}), &mty::dirac4));

    vectorSpaces.push_back(pair<mty::AlgebraState, const Space *>(
        mty::AlgebraState({0, 1}), &mty::dirac4));

    vectorSpaces.push_back(pair<mty::AlgebraState, const Space *>(
        mty::AlgebraState({1, 1}), &Minkowski));
}
...
unique_ptr<SemiSimpleGroup>
createGroup(group::Type type, std::string const &name, int dim)
{
...
unique_ptr<SemiSimpleGroup> createGroup(group::Type type, int dim)
{
    if (type == group::Type::U1) {
