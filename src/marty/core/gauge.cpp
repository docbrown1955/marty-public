                     ...
                     const csl::Expr             &coupling)
{
    ...
    gaugedGroups.push_back(createGaugedGroup(groups[groups.size() - 1].get(),
                                             this,
                                             nameBoson,
                                             true,
                                             actualCoupling));
...
GaugeIrrep Gauge::getRepresentation(const vector<vector<int> > &highestWeights)
{
    vector<Irrep> reps(0);
...
bool Gauge::containsTrivialRep(std::vector<QuantumField> const &fields) const
{
    if (fields.empty())
        return true;
    for (size_t i = 0; i != gaugedGroups.size(); ++i) {
        ...
        for (size_t j = 1; j != fields.size(); ++j) {
            decompo = groups[i]->getAlgebra()->tensorProduct(
                decompo, fields[j].getGroupIrrep(groups[i].get()));
        }
                                     ...
                                     Tensor                   &point)
{
    ...
    for (const auto &gauged : gaugedGroups)
        cov = cov
              + gauged->covariantDerivative(field, mu, fieldIndices, point);
...
void Gauge::setGaugeChoice(size_t pos, gauge::Type choice)
{
    HEPAssert(pos < gaugedGroups.size(),
              mty::error::IndexError,
              "Index " + toString(pos) + " out of bounds of gauge with size "
...
GaugeIrrep::GaugeIrrep(Gauge *t_gauge)
    : gauge(t_gauge), rep(vector<Irrep>(t_gauge->size()))
{
}

GaugeIrrep::GaugeIrrep(Gauge *t_gauge, const vector<Irrep> &t_rep)
...
bool GaugeIrrep::operator==(const GaugeIrrep &other) const
{
    ...
    for (size_t i = 0; i != size(); ++i)
        if (not gauge->getGaugedGroup(i)->isBroken() and rep[i] != other[i])
            return false;
    return true;
}
