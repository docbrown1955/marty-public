...
std::vector<FermionCurrent>
parseChains(csl::Expr &product, DiracSpace const *diracSpace, bool findAbrev)
{
...
FermionCurrent::FermionCurrent(std::vector<csl::Expr> const &t_tensors,
    ...
    : diracSpace(t_diracSpace), left(CSL_1), right(CSL_1)
{
...
csl::Expr FermionCurrent::getExpression() const
{
    ...
    return csl::prod_s({factor, left, csl::prod_s(tensors, true), right},
                       true);
}
    ...
    std::vector<csl::Expr> const &tensors)
{
    if (tensors.empty())
        return;
    auto [t_first, t_last]
        = diracSpace->getBorderOfChain(tensors.begin(), tensors.end());
    first = t_first;
    last  = t_last;
}
...
{
    return diracSpace == other.diracSpace and type == other.type;
}
...
{
    switch (type) {
    case FermionCurrent::Scalar:
        out << "Scalar";
        break;
    case FermionCurrent::PseudoScalar:
        out << "PseudoScalar";
        break;
    case FermionCurrent::LScalar:
        out << "LScalar";
        break;
    case FermionCurrent::RScalar:
        out << "RScalar";
        break;
    case FermionCurrent::Vector:
        out << "Vector";
        break;
    case FermionCurrent::Axial:
        out << "Axial";
        break;
    case FermionCurrent::LVector:
        out << "LVector";
        break;
    case FermionCurrent::RVector:
        out << "RVector";
        break;
    case FermionCurrent::Magnetic:
        out << "Magnetic";
        break;
    case FermionCurrent::AMagnetic:
        out << "AMagnetic";
        break;
    case FermionCurrent::LMagnetic:
        out << "LMagnetic";
        break;
    case FermionCurrent::RMagnetic:
        out << "RMagnetic";
        break;
    }
    return out;
...
std::ostream &operator<<(std::ostream &out, FermionCurrent const &current)
{
    ...
    if (current.factor != CSL_1)
        out << current.factor << "*";
    out << current.left << prod_s(current.tensors, true) << current.right
        << std::endl;
