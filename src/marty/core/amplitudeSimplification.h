 ...
 * rountines can for the most part be called separatly, but for a full
 * successful simplification the order is extremely important and the
 * mty::simpli::simplify() method is a reference.
 *
...
isColorStructure(csl::IndexStructure const &structure);
 ...
 */
csl::Expr
colorAbbreviation(std::vector<csl::Expr> const &                exprs,
 ...
 * divergent integral \f$ I = \frac{a}{\epsilon}  + b +
 * \mathcal{O}(\epsilon)\f$, as the epsilon terms cancel each other to produce
 * another constant term. In this simple case we obtain (removing the purely
 * divergent term): \f[ D\cdot I = (4 - 2\epsilon)(\frac{a}{\epsilon} + b) = 4b
 * + Local, \f] with \f[ Local = -2a. \f]
 *
