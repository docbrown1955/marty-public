    ...
    std::vector<std::vector<size_t>> const &terms)
{
    ...
    csl::ScopedProperty p5(
        &mty::option::excludePentagons,
        mty::option::excludePentagons
            || !(options.getTopology() & Topology::Pentagon));

    Amplitude res = (feynRuleMode) ? ruledCalculation(terms)
                                   : fullQuantumCalculation(terms);
...
Amplitude AmplitudeInitializer::fullQuantumCalculation(
    std::vector<std::vector<size_t>> const &terms)
{
    csl::Expr LSZInsertion = getLSZInsertions(insertions, kinematics, true);
    ...
    for (size_t index = 0; index != terms.size(); ++index) {
        ...
        std::vector<FeynmanDiagram> amplitude
            = wick::WickCalculator::getDiagrams(
                model, options, fieldProd, vertexIds);
        for (size_t i = 0; i != amplitude.size(); ++i) {
            simplifyFullQuantumCalculation(
                amplitude[i], integral_cpy, termPos, externalSym);
        }
...
Amplitude AmplitudeInitializer::ruledCalculation(
    std::vector<std::vector<size_t>> const &terms)
{
    csl::Expr LSZInsertion = getLSZInsertions(insertions, kinematics, false);
    ...
    for (size_t index = 0; index != terms.size(); ++index) {
        ...
        initMomentumVertices(localRules,
                             vertexIds,
                             vertices,
                             witnessVertices,
                             integral_cpy,
        ...
        std::vector<mty::FeynmanDiagram> amplitude
            = wick::WickCalculator::getDiagrams(model,
                                                options,
                                                fieldProd,
                                                vertexIds,
                                                momentumMapping,
        ...
        ////////////////////////////////////////
        for (size_t i = 0; i != amplitude.size(); ++i) {
            simplifyRuledCalculation(amplitude[i],
                                     integral_cpy,
                                     witnessVertices,
                                     momentumMapping[i]);
        }
    ...
    std::vector<std::vector<size_t>> &externalSym)
{
    ...
    if (res != CSL_0) {
        ...
        csl::Expr  PSum
            = mty::getMomentumSum(insertions, kinematics.getMomenta(), ind);
        ...
        if (res != CSL_0) {
            csl::vector_expr appliedSym = applyAllExternalSymmetries(
                {res}, externalSym.begin(), externalSym.end());
    ...
    FeynruleMomentum               &momentumMapping)
{
    ...
    csl::Expr res
        = 1 / csl::int_s(diagram.getDiagram()->getTotalDegeneracyFactor())
        ...
        if (res != CSL_0) {
            HEPAssert(csl::TestIndexSanity(res),
                      ...
                      "Bad index property for "
                          + toString(res)) if (options.simplifyAmplitudes)
                simpli::simplify(res,
                                 insertions,
                                 kinematics.getMomenta(),
                                 options,
    ...
    std::vector<FeynmanDiagram> &diagrams)
{
    auto last = std::remove_if(
        begin(diagrams), end(diagrams), [&](FeynmanDiagram const &diag) {
            return diag.isZero();
        });
    ...
    bool                                  feynRuleCalculation)
{
    ...
    HEPAssert(insertions.size() == momenta.size(),
        ...
        std::vector<csl::Expr>
            terms(insertions.size());
    bool    firstFermionFound = false;
    for (size_t i = 0; i != insertions.size(); ++i) {
        auto &field = insertions[i];
        if (!firstFermionFound && field.isFermionic()
            && !field.isSelfConjugate()) {
            firstFermionFound = true;
            terms[i]
                = field.getLSZInsertion(momenta[i], feynRuleCalculation, true);
        }
        else {
            terms[i] = field.getLSZInsertion(momenta[i], feynRuleCalculation);
        }
    }
    ...
    std::vector<csl::Expr>         &fieldVertices)
{
    ...
    for (size_t i = 0; i != localRules.size(); ++i) {
        for (size_t j = 0; j != localRules[i].getSize(); ++j)
            witnessVertices.push_back(
                csl::tensor_s("W_" + toString(index++), &csl::Minkowski));
    }
    ...
    for (size_t i = 0; i != localRules.size(); ++i) {
        ...
        vertexIds[vertices[i]]               = i;
        fieldVertices[insertions.size() + i] = localRules[i].getFieldProduct(
            vertices[i], firstWitness, lastWitness);
    }
}

std::vector<std::vector<size_t>>
AmplitudeInitializer::getExternalSymmetries(csl::Expr &fieldProd) const
{
    csl::Expr firstFieldExpr;
    csl::FirstOfLeaf(fieldProd, [&](csl::Expr const &expr) {
        if (IsOfType<QuantumField>(expr)
            and ConvertToPtr<QuantumField>(expr)->isExternal()) {
            firstFieldExpr = expr;
            return true;
        }
        return false;
    ...
    for (size_t i = 0; i < indicesLeft.size() - 1; ++i) {
        ...
        for (size_t j = i + 1; j < indicesLeft.size(); ++j) {
            if (wick::areDegenerate(field, insertions[indicesLeft[j]])) {
    ...
    while (std::next_permutation(perm.begin(), perm.end())) {
            ...
            if (el->isIndexed()) {
                for (size_t i = 0; i != deg_momentum.size(); ++i)
                    if (el->getParent_info() == deg_momentum[i].get()) {
                        ...
                        if (deg_insertions[i].isIncoming()
                            xor deg_insertions[perm[i]].isIncoming())
                            el = -el;
                        break;
                    }
            }
        ...
        if (fermionic) {
            for (size_t k = 0; k != perm.size(); ++k) {
                if (k < perm[k]) {
                    ...
                    csl::Index alpha
                        = deg_indices[(k + 1) * nIndices - 1]; // Last = Dirac
                    csl::Index beta
                        = deg_indices[(perm[k] + 1) * nIndices - 1];
                    ...
                    csl::Expr   sign
                        = (deg_insertions[k].isComplexConjugate()
        ...
        csl::ForEachLeaf(copyRes, [&](csl::Expr &el) {
            ...
            if (not mty::IsOfType<mty::PolarizationField>(el)
                ...
                csl::IndexStructure &structure = el->getIndexStructureView();
    ...
    csl::Expr                      &amplitude)
{
    for (const auto &w : witnessVertices) {
        ...
        HEPAssert(!csl::DependsExplicitlyOn(amplitude, w),
                  mty::error::RuntimeError,
