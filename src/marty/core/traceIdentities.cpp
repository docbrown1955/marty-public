    ...
    : tensorSize(t_tensorSize)
{
    ...
    while (!oldTerms.empty()) {
        ...
        for (const auto &term : oldTerms) {
            std::vector<std::vector<Tensor>> interm = step(term);
            if (interm.empty())
...
std::vector<std::vector<TraceIdentity::Tensor>>
TraceIdentity::step(std::vector<Tensor> const &term) const
{
    if (isFull(term))
        return {};
    ...
    for (size_t i = 0; i != term.size(); ++i) {
        ...
        if (tensor.empty()) {
            auto pos = std::find(
                explored.begin(), explored.end(), tensor.finalSize);
            if (pos != explored.end())
                continue;
            else
...
size_t TraceIdentity::totalIndices(std::vector<Tensor> const &term) const
{
    size_t ni = 0;
...
bool TraceIdentity::isSame(TraceIdentity const &other) const
{
    return other.tensorSize.size() == tensorSize.size()
           and std::is_permutation(tensorSize.begin(),
                                   tensorSize.end(),
    ...
    for (size_t ni = 2; ni <= n; ni += 2)
        for (size_t mi = 1; ni * mi <= n; ++mi) {
            ...
            for (auto &nSet : subPartition) {
                ...
                newPartition.insert(
                    newPartition.end(), nSet.begin(), nSet.end());
...
std::vector<TraceIdentity> traceIdentity(algebra::Type type, size_t n)
{
    ...
    case algebra::Type::E6:
        return E6TraceIdentity(n);
    case algebra::Type::E7:
        return E7TraceIdentity(n);
    case algebra::Type::E8:
        return E8TraceIdentity(n);
    case algebra::Type::F4:
        return F4TraceIdentity(n);
    case algebra::Type::G2:
        return G2TraceIdentity(n);
    default:
        CallHEPError(mty::error::ValueError,
                     "Trace identity for algebra " + toString(type)
                         + " needs"
...
traceIdentity(algebra::Type type, size_t l, size_t n)
{
    ...
    case algebra::Type::B:
        return BTraceIdentity(l, n);
    case algebra::Type::C:
        return CTraceIdentity(l, n);
    case algebra::Type::D:
        return DTraceIdentity(l, n);
    case algebra::Type::E6:
        return E6TraceIdentity(n);
    case algebra::Type::E7:
        return E7TraceIdentity(n);
...
std::vector<TraceIdentity> ATraceIdentity(size_t l, size_t n)
{
    ...
    std::vector<std::vector<PartitionPair>> partitions = evenPartition(n);
