...
static std::vector<csl::Index> gammas(std::vector<GammaIndex> const &indices)
{
...
static bool
isAntiSymmetric(size_t i, size_t j, std::vector<GammaIndex> const &indices)
{
    if (j < i)
...
static void cutIndex(std::vector<GammaIndex> &init, size_t i)
{
    size_t n = 0;
...
static void cutIndex(std::vector<GammaIndex> &init, size_t i, size_t j)
{
...
static std::vector<GammaIndex> cutIndices(std::vector<GammaIndex> const &init,
                                          size_t                         i1,
                                          size_t                         j1,
                                          size_t                         i2,
                                          size_t                         j2)
                    ...
                    csl::Expr const               &Ddirac)
{
    ...
    if (N == 2) {
        LOG("Res :")
        LOG(4
            * metricindex_s(indices[0].indices()[0], indices[1].indices()[0]))
        return 4
               * metricindex_s(indices[0].indices()[0],
                               indices[1].indices()[0]);
    }
    ...
    for (size_t i = 1; i != indices.size(); ++i) {
        for (size_t l = 0; l != indices[i].indices().size(); ++l) {
            GExpr g        = metricindex_s(indices[0].indices()[0],
                                    indices[i].indices()[l]);
            auto  newIndex = cutIndices(indices, 0, 0, i, l);
            LOG("New term :", cslexpr_s(sign) * g);
            terms.push_back(cslexpr_s(sign) * g
                            * standardTrace(newIndex, Ddirac));
            sign *= -1;
        }
    }
