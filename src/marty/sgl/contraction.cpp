                  ...
                  csl::Index const &b)
{
    if (Mu.isSigma() && Nu.isSigma()) {
        ...
        if (fullContraction) {
            return cslexpr_s(sign) * sgl::DMinko * (1 - sgl::DMinko)
                   * indexchain_s(a, b);
        }
        return cslexpr_s(sign)
               * ((2 - sgl::DMinko)
                      * indexchain_s(gammaindex_s({nu, rho}), a, b)
                  + (1 - sgl::DMinko) * metricindex_s(nu, rho)
                        * indexchain_s(a, b));
    }
    ...
    if ((Mu.isGamma5() && Nu.isP_L()) || (Mu.isP_L() && Nu.isGamma5())) {
        return -indexchain_s(gammaindex_s(6), a, b);
    }
    if ((Mu.isGamma5() && Nu.isP_R()) || (Mu.isP_R() && Nu.isGamma5())) {
...
static GExpr epsilonContraction(csl::Index const &mu, csl::Index const &nu)
{
    return epsilonCombinatorial(3) * g(mu, nu);
}

static GExpr epsilonContraction(csl::Index const &mu,
                                csl::Index const &nu,
                                csl::Index const &rho,
...
GExpr epsilonContraction(std::vector<csl::Index> A, std::vector<csl::Index> B)
{
    ...
    switch (nContraction) {
    case 1:
        res = sign * epsilonContraction(A[1], A[2], A[3], B[1], B[2], B[3]);
        break;
    case 2:
        res = sign * epsilonContraction(A[2], A[3], B[2], B[3]);
        break;
    case 3:
        res = sign * epsilonContraction(A[3], B[3]);
        break;
    case 4:
                               ...
                               size_t            j)
{
    ...
    if (init.isSigma(m_argument[i]) && init.isGammaMu(m_argument[j])) {
        ...
        LOG("Res :",
            (sign * momentumindex_s(p, sigma) * momentumindex_s(p, rho) * expr
                 * chain1.copy()
             - sign * cslexpr_s(p(rho) * p(+rho)) * expr * chain2.copy()))
        return sign * momentumindex_s(p, sigma) * momentumindex_s(p, rho)
                   * expr * chain1.copy()
               - sign * cslexpr_s(p(rho) * p(+rho)) * expr * chain2.copy();
    }
    ...
    if (init.isSigma(m_argument[i]) && init.isSigma(m_argument[j])) {
        ...
        auto chain1 = chain;
        chain1.m_argument
            = std::vector<GExpr>{gammaindex_s(+rho), gammaindex_s(lambd)};
        auto chain2 = chain;
        chain2.m_argument
            = std::vector<GExpr>{gammaindex_s(+rho), gammaindex_s(sigma)};
        auto chain3 = chain;
        chain3.m_argument
            = std::vector<GExpr>{gammaindex_s(sigma), gammaindex_s(lambd)};
        LOG("Res :",
            (sign * momentumindex_s(p, sigma) * momentumindex_s(p, lambd)
                 * chain.copy()
             + sign * momentumindex_s(p, sigma) * momentumindex_s(p, rho)
                   * chain1.copy()
             - sign * momentumindex_s(p, lambd) * momentumindex_s(p, rho)
                   * chain2.copy()
             - sign * cslexpr_s(p(rho) * p(+rho)) * chain3.copy()))
        return sign * momentumindex_s(p, sigma) * momentumindex_s(p, lambd)
                   * chain.copy()
               + sign * momentumindex_s(p, sigma) * momentumindex_s(p, rho)
                     * chain1.copy()
               - sign * momentumindex_s(p, lambd) * momentumindex_s(p, rho)
