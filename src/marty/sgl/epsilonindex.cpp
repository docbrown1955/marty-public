...
EpsilonIndex::EpsilonIndex(csl::Index const &mu,
                           csl::Index const &nu,
                           csl::Index const &rho,
...
GExpr EpsilonIndex::chisholmIdentity1(csl::Index const &mu,
                                      csl::Index const &a,
                                      csl::Index const &b) const
{
    ...
    terms.reserve(4);
    terms.push_back(indexchain_s(std::vector<GExpr>{gammaindex_s(nu[0]),
                                                    gammaindex_s(nu[1]),
                                                    gammaindex_s(nu[2]),
                                                    gammaindex_s(5)},
                                 a,
    ...
    terms.push_back(
        ...
        * indexchain_s(
            std::vector<GExpr>{gammaindex_s({nu[0]}), gammaindex_s(5)}, a, b));
    terms.push_back(
        metricindex_s(nu[0], nu[2])
        * indexchain_s(
            std::vector<GExpr>{gammaindex_s({nu[1]}), gammaindex_s(5)}, a, b));
    terms.push_back(
        -metricindex_s(nu[0], nu[1])
        * indexchain_s(
            std::vector<GExpr>{gammaindex_s({nu[2]}), gammaindex_s(5)}, a, b));
    LOG("Structure res :", sgl::sum_s(terms))
    LOG("End Chisholm")
    return factor * sgl::sum_s(terms);
}

GExpr EpsilonIndex::chisholmIdentity2(csl::Index const &mu,
                                      csl::Index const &nu,
                                      csl::Index const &a,
                                      csl::Index const &b) const
{
    SCOPELOG
    LOG("Chisholm idendity for",
        mu,
        "and",
        nu,
        "in",
                                       ...
                                       csl::Index const &             b) const
{
    ...
    LOG("Structure res: ",
        (indexchain_s(std::vector<GExpr>{gammaindex_s({rho[0], rho[1]}),
                                         gammaindex_s(5)},
                      a,
                      b)))
...
GExpr EpsilonIndex::propertyWith(GExpr const &other) const
{
    if (IsType<EpsilonIndex>(other)) {
        return epsilonContraction(indices(), other->indices());
    }
...
GExpr epsilonindex_s(csl::Index const &mu,
                     csl::Index const &nu,
                     csl::Index const &rho,
                     csl::Index const &sigma)
{
