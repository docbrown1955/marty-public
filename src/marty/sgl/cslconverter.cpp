...
GExpr csl_simplified(GExpr const &expr, TensorSet const &tensors)
{
    return sgl::DeepRefreshed(
        csl_to_sgl(csl::DeepRefreshed(sgl_to_csl(expr, tensors)), tensors));
}
...
GExpr csl_to_sgl(csl::Expr const &expr, TensorSet const &tensors)
{
    ...
    if (parent == tensors.gamma_chir) {
        LOG(indexchain_s(
            gammaindex_s(5), expr->getIndex(0), expr->getIndex(1)))
        return indexchain_s(
            gammaindex_s(5), expr->getIndex(0), expr->getIndex(1));
    }
    if (parent == tensors.P_L) {
        LOG(indexchain_s(
            gammaindex_s(6), expr->getIndex(0), expr->getIndex(1)))
        return indexchain_s(
            gammaindex_s(6), expr->getIndex(0), expr->getIndex(1));
    }
    if (parent == tensors.P_R) {
        LOG(indexchain_s(
            gammaindex_s(7), expr->getIndex(0), expr->getIndex(1)))
        return indexchain_s(
            gammaindex_s(7), expr->getIndex(0), expr->getIndex(1));
    }
    if (parent == tensors.C) {
    ...
    for (const auto iter : tensors.gamma)
        if (iter.second == parent) {
            ...
            LOG(factor * indexchain_s(gammaindex_s(minkoIndices), a, b))
            return factor * indexchain_s(gammaindex_s(minkoIndices), a, b);
        }
    ...
    if (parent == minkoSpace->getEpsilon()) {
        LOG(epsilonindex_s(expr->getIndex(0),
                           expr->getIndex(1),
                           expr->getIndex(2),
                           expr->getIndex(3)))
        return epsilonindex_s(expr->getIndex(0),
                              expr->getIndex(1),
                              expr->getIndex(2),
                              expr->getIndex(3));
    }
