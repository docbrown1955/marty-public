                                    ...
                                    bool keepAllCosntraints) const
{
    ...
    if (not isIndexed())
        return *this == expr;
    return argument->compareWithDummy(
        expr->getArgument().get(), constraints, keepAllCosntraints);
}
...
optional<Expr> AbstractFunc::findSubExpression(Expr_info   subExpression,
                                               const Expr &newExpression) const
{
    if (*this == subExpression)
        return newExpression;
    optional<Expr> sub
        = argument->findSubExpression(subExpression, newExpression);
    if (sub) {
        Expr res = Copy(this);
...
optional<Expr> AbstractFunc::replaceIndex(const Index &indexToReplace,
                                          const Index &newIndex,
                                          bool         refresh) const
{
    optional<Expr> opt
        = argument->replaceIndex(indexToReplace, newIndex, refresh);
    if (opt) {
        Expr copyExpr = copy();
...
std::optional<Expr>
AbstractFunc::replaceIndices(std::vector<csl::Index> const &oldIndices,
                             std::vector<csl::Index> const &newIndices,
                             bool                           refresh,
                             bool                           flipped) const
{
    optional<Expr> opt
        = argument->replaceIndices(oldIndices, newIndices, refresh, flipped);
    if (opt) {
        Expr copyExpr = copy();
        copyExpr->setArgument(opt.value());
        if (refresh)
...
optional<Expr> AbstractFunc::collect(std::vector<Expr> const &factors,
                                     bool                     full) const
{
    if (full) {
...
optional<Expr> AbstractFunc::expand(bool full, bool inPlace) const
{
    if (full) {
...
optional<Expr> AbstractFunc::evaluate(csl::eval::mode user_mode) const
{
    optional<Expr> evalArg = argument->evaluate(user_mode);
    if (evalArg) {
        if (evalArg.value()->getPrimaryType() == csl::PrimaryType::Numerical
    ...
    else if (argument->getPrimaryType() == csl::PrimaryType::Numerical
             and eval::isContained(user_mode, csl::eval::numerical))
...
Expr const &AbstractFunc::operator[](int i) const
{
    CSL_ASSERT_SPEC(i == 0,
                    CSLError::IndexError,
                    "Simple function has only one arg, index " + toString(i)
                        + " given.");
    return argument;
}
...
bool AbstractFunc::operator==(Expr_info expr) const
{
    ...
    return (expr->getType() == this->getType()
            and *argument == expr->getArgument().get());
}
...
{
    return (argument[0]->getCommutable() and argument[1]->getCommutable());
}
...
optional<Expr>
AbstractDuoFunc::findSubExpression(Expr_info   subExpression,
                                   const Expr &newExpression) const
{
    if (*this == subExpression)
        return newExpression;
    optional<Expr> subArg0
        = argument[0]->findSubExpression(subExpression, newExpression);
    optional<Expr> subArg1
        = argument[1]->findSubExpression(subExpression, newExpression);
...
optional<Expr> AbstractDuoFunc::replaceIndex(const Index &indexToReplace,
                                             const Index &newIndex,
                                             bool         refresh) const
{
    optional<Expr> opt0
...
std::optional<Expr>
AbstractDuoFunc::replaceIndices(std::vector<csl::Index> const &oldIndices,
                                std::vector<csl::Index> const &newIndices,
                                bool                           refresh,
                                bool                           flipped) const
{
    optional<Expr> opt0 = argument[0]->replaceIndices(
        oldIndices, newIndices, refresh, flipped);
    optional<Expr> opt1 = argument[1]->replaceIndices(
        oldIndices, newIndices, refresh, flipped);
    if (opt0 or opt1) {
        Expr copyExpr = copy();
        copyExpr->setArgument(opt0.value_or(argument[0]), 0);
...
optional<Expr> AbstractDuoFunc::collect(std::vector<Expr> const &factors,
                                        bool                     full) const
{
    if (full) {
...
{
    return (*this == expr or argument[0]->dependsOn(expr)
            or argument[1]->dependsOn(expr));
}

bool AbstractDuoFunc::dependsOn(const AbstractParent *parent) const
{
    return argument[0]->dependsOn(parent) or argument[1]->dependsOn(parent);
}

bool AbstractDuoFunc::dependsExplicitlyOn(Expr_info expr) const
{
    return (operator==(expr) or argument[0]->dependsExplicitlyOn(expr)
...
optional<Expr>
AbstractMultiFunc::findSubExpression(Expr_info   subExpression,
                                     const Expr &newExpression) const
{
    if (*this == subExpression)
        return newExpression;
    ...
    for (size_t i = 0; i != argument.size(); ++i) {
        argSub[i]
            = argument[i]->findSubExpression(subExpression, newExpression);
...
optional<Expr> AbstractMultiFunc::getComplexConjugate() const
{
    ...
    if (conjugFound) {
        Expr conjugate = Copy(this);
        for (size_t i = 0; i != argument.size(); ++i)
            conjugate->setArgument(optConjug[i].value_or(argument[i]), i);
        return Refreshed(conjugate);
    }
...
optional<Expr> AbstractMultiFunc::replaceIndex(const Index &indexToReplace,
                                               const Index &newIndex,
                                               bool         refresh) const
{
    ...
    for (size_t i = 0; i != argument.size(); ++i)
        if (optional<Expr> opt
            = argument[i]->replaceIndex(indexToReplace, newIndex, refresh);
            opt) {
            newArg[i] = opt.value();
            replaced  = true;
        }
...
std::optional<Expr>
AbstractMultiFunc::replaceIndices(std::vector<csl::Index> const &oldIndices,
                                  std::vector<csl::Index> const &newIndices,
                                  bool                           refresh,
                                  bool                           flipped) const
{
    ...
    bool             replaced = false;
    for (size_t i = 0; i != argument.size(); ++i)
        if (optional<Expr> opt = argument[i]->replaceIndices(
                oldIndices, newIndices, refresh, flipped);
            opt) {
            newArg[i] = opt.value();
            replaced  = true;
...
optional<Expr> AbstractMultiFunc::collect(std::vector<Expr> const &factors,
                                          bool                     full) const
{
    if (full) {
...
optional<Expr> AbstractMultiFunc::expand(bool full, bool inPlace) const
{
    if (full) {
        // Expr result = Copy(this);
    ...
    std::function<bool(Expr const &)> const &f, bool full, bool inPlace) const
{
    if (full) {
        ...
        for (size_t i = 0; i != res->size(); ++i)
            res->setArgument(
                res[i]->expand_if(f, true, inPlace).value_or(res[i]), i);
