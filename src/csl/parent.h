 ...
 * forms in an expression, we want its properties to appear only at one place
 * in the program, and we want an object that can generate the Element. This is
 * the Parent. The parent then has three important features: \n 1. It is the
 * only one to contain the properties of the object, if an element wants to
 * access its own properties it has to ask its parent. \n 2. It is the only
 * object able to generate properly an Element of its type. \n 2. It cannot
 * appear in an expression (since it is not derived from Abstract). And it is
 * normal because in a expression we want a \f$ g_{\mu\nu} \f$ with given \f$
 * \mu \f$ and \f$ \nu \f$, not a simple \f$ g \f$.
 */
class AbstractParent : public std::enable_shared_from_this<AbstractParent> {
  ...
  protected:
    ...
    /*! Commutability property of the parent (will be given to the elements
     * when created).
     */
    bool commutable;
  ...
  public:
    ...
    virtual void printDefinition(std::ostream &out        = std::cout,
                                 int           indentSize = 4,
                                 bool          header     = false) const = 0;

    void
    printPropDefinition(std::ostream &out, int indentSize, bool header) const;
    ...
    virtual std::optional<Expr> evaluate(Expr_info       self,
    ...
    /*! \return \b True if the parent is indicial and fully antisymmetric in
     * its indices. \return \b False else.
     */
    ...
    /*! \brief Adds an antisymmetry between indices in position \b i1 and \b
     * i2. \param i1 Index of the first index in the symmetry. \param i2 Index
     * of the second index in the symmetry.
     */
    ...
    virtual std::optional<Expr> getHermitianProperty(Expr_info    self,
                                                     const Space *space) const;

    virtual std::optional<Expr>
    getTransposedProperty(Expr_info self, const Space *space) const;
