...
class IndexStructureView {
  ...
  public:
    ...
    IndexStructureView(IndexStructure_new<N> const &structure)
        : first(structure.begin()), last(structure.end())
    {
    }
...
class IndexStructure_new {
  ...
  public:
    ...
    explicit IndexStructure_new(std::array<csl::Index, N> const &t_indices)
        : indices(t_indices)
    {
    }

    explicit IndexStructure_new(std::array<csl::Index, N> &&t_indices)
    ...
    template <size_t M>
    IndexStructure_new<M + N>
    operator+(IndexStructure_new<M> const &other) const
    {
    ...
    /*!
     * \brief Compares the IndexStructure_new<N> with \b structure. Each index
     * must match exactly (see Index::exactMatch()) with the Index at the same
     * place in the second structure.
     *
    ...
    /*!
     * \brief operator==, compares the IndexStructure_new<N> to \b structure in
     * terms of free structure, independently of the dummy indices or the order
     * of free indices.
     *
    ...
    /*!
     * \brief operator!=, returns the opposite of
     * IndexStructure_new<N>::operator==().
     *
