...
class Functional {
  ...
  public:
    Functional(Expr const &t_init, Args... t_args) : init(t_init)
    {
    ...
    Expr operator()(T_Args... t_args)
    {
        ...
        Replacer<Args...> rep;
        rep.template replace<0, T_Args...>(res, args, new_args);
    ...
    struct Replacer {
                     ...
                     std::tuple<Arg2...> &newArg)
        {
            if constexpr (i == sizeof...(Arg2))
                ...
                init
                    = Replaced(init, std::get<i>(oldArg), std::get<i>(newArg));
                return replace<i + 1, Arg2...>(init, oldArg, newArg);
            }
        }
    };
};
