...
Equation::Equation() : leftHandSide(CSL_0), rightHandSide(CSL_0)
{
    searchBuildingBlocks();
}

Equation::Equation(const Expr &LHS) : leftHandSide(LHS), rightHandSide(CSL_0)
{
    searchBuildingBlocks();
}
...
void Equation::replace(const Expr &oldE, const Expr &newE)
{
    ...
    if (oldE->isBuildingBlock())
        for (auto arg = buildingBlocks.begin(); arg != buildingBlocks.end();
             ++arg)
            if (**arg == oldE) {
...
void Equation::isolationStep(const Expr &expr)
{
    ...
    if (rightHandSide->getType() == csl::Type::Sum) {
        for (auto arg = rightHandSide->begin(); arg != rightHandSide->end();
             ++arg) {
            if (IsIndicialTensor(*arg)) {
                if (**arg == expr)
                    sum = sum_s(sum, *arg);
        ...
        for (auto arg = leftHandSide->begin(); arg != leftHandSide->end();
            ...
            if ((*arg)->getType() == csl::Type::Pow
                ...
                for (auto argModif = leftHandSide->begin();
                     argModif != leftHandSide->end();
                     ++argModif)
                    (*argModif) = (*argModif) * foo;
                product = product * foo;
            }
            else if ((*arg)->getType() == csl::Type::Prod) {
                for (auto arg2 = (*arg)->begin(); arg2 != (*arg)->end();
                     ++arg2) {
    ...
    if (leftHandSide->getPrimaryType() == csl::PrimaryType::ScalarFunction) {
        ...
        case csl::Type::Exp:
            rightHandSide = log_s(rightHandSide);
            break;
        case csl::Type::Log:
            rightHandSide = exp_s(rightHandSide);
            break;
        case csl::Type::Cos:
            rightHandSide = acos_s(rightHandSide);
            break;
        case csl::Type::Sin:
            rightHandSide = asin_s(rightHandSide);
            break;
        case csl::Type::Tan:
            rightHandSide = atan_s(rightHandSide);
            break;
        case csl::Type::ACos:
            rightHandSide = cos_s(rightHandSide);
            break;
        case csl::Type::ASin:
            rightHandSide = sin_s(rightHandSide);
            break;
        case csl::Type::ATan:
            rightHandSide = tan_s(rightHandSide);
            break;
        case csl::Type::Cosh:
            rightHandSide = acosh_s(rightHandSide);
            break;
        case csl::Type::Sinh:
            rightHandSide = asinh_s(rightHandSide);
            break;
        case csl::Type::Tanh:
            rightHandSide = atanh_s(rightHandSide);
            break;
        case csl::Type::ACosh:
            rightHandSide = cosh_s(rightHandSide);
            break;
        case csl::Type::ASinh:
            rightHandSide = sinh_s(rightHandSide);
            break;
        case csl::Type::ATanh:
            rightHandSide = tanh_s(rightHandSide);
            break;
        default:
            return;
        }
    ...
    if (leftHandSide->getPrimaryType() == csl::PrimaryType::MultiFunction) {
        ...
        case csl::Type::Pow:
            if (not leftHandSide->getArgument(1)->dependsExplicitlyOn(
                    expr.get())) {
                rightHandSide
                    = pow_s(rightHandSide, 1 / leftHandSide->getArgument(1));
...
ostream &operator<<(ostream &fout, cslEquation::Type type)
{
    ...
    case cslEquation::Regular:
        fout << "Regular";
        break;
    case cslEquation::Indicial:
        fout << "Indicial";
        break;

    default:
        callError(cslError::UnknownEquation,
                  "operator<<(ostream&, cslEquation::Type)",
                  (int) type);
    }
