 ...
 * In particular all mathematical functions will be derived classes. In the
 * case of the call of a simplification function, if recursive, the function
 * will be first applied on the \b argument, then to the function itself.
 */
class AbstractFunc : public Abstract {
  ...
  public:
    ...
    replaceIndices(std::vector<csl::Index> const &oldIndices,
     ...
     * \details Except particular cases (cos(0), exp(0) etc) this function
     * returns the same AbstractFunc with \b argument.evaluate() as argument.
...
class AbstractDuoFunc : public Abstract {
  ...
  protected:
    std::array<Expr, 2>
        argument; /*!< \brief std::array containing the arguments. */
  ...
  public:
    ...
    AbstractDuoFunc(const Expr &left, const Expr &right);
...
class AbstractMultiFunc : public Abstract {
  ...
  protected:
    csl::vector_expr
        argument; /*!< \brief std::vector containing the arguments. */
...
inline AbstractDuoFunc::AbstractDuoFunc(const Expr &left, const Expr &right)
    : Abstract()
{
    argument[0] = left;
    argument[1] = right;
