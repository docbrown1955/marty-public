...
class symmetricCounter {
  ...
  public:
    ...
    symmetricCounter(size_t N) : symmetricCounter(N, N)
    {
    }

    symmetricCounter(size_t N, size_t t_max) : max(t_max), counter(N, 0)
    {
    }
  ...
  protected:
    ...
    void adjust()
    {
        if (counter.size() < 2)
            return;
        T min = counter.back();
        for (auto iter = counter.rbegin() + 1; iter != counter.rend();
             ++iter) {
            if (*iter < min)
                *iter = min;
            else if (*iter > min)
