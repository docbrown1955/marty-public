...
struct ExpansionChain {
    ...
    std::vector<size_t> posToExpand;
};

bool haveCommonIndices(csl::Expr const & a,
                       csl::Expr const & b,
...
bool alreadyInChain(size_t pos, std::vector<ExpansionChain> const &chains)
{
    ...
    while (first != end) {
        for (size_t i : first->posToExpand) {
            if (i == pos)
                return true;
        }
        ++first;
...
csl::Expr const *getEmitter(csl::Expr const &      expr,
...
bool any_of(csl::Expr const *       emi,
            csl::Expr const &       rec,
...
size_t getNTerms(csl::Expr const &       prod,
                 ExpansionChain const &  toExpand,
                 ExpanderReceiver const &isReceiver)
{
    csl::Expr emitter = nullptr;
    size_t    nTerms  = 1;
    for (size_t pos : toExpand.posToExpand) {
        auto const &arg = prod[pos];
        if (csl::IsSum(arg)) {
            const size_t sum_sz    = arg->size();
            size_t       nSumTerms = 0;
...
csl::Expr applyExpansion(csl::Expr const &       prod,
                         ExpansionChain const &  toExpand,
                         ExpanderEmitter const & isEmitter,
                         ExpanderReceiver const &isReceiver,
                         bool                    applyRecursively)
{
    const size_t     sz      = toExpand.posToExpand.size();
    csl::Expr const *emitter = toExpand.emitter;
    const size_t     nTerms  = getNTerms(prod, toExpand, isReceiver);
    ...
    for (size_t pos : toExpand.posToExpand) {
        ...
        if (csl::IsSum(arg)) {
            ...
            const size_t nTerms  = expanded.size();
            const size_t resSize = res.size();
            for (size_t i = 0; i != (nTerms - 1) * resSize; ++i) {
                res.push_back({});
                auto &vec = res.back();
                vec.reserve(sz);
                vec = res[i % resSize];
    ...
    for (size_t i = 0; i != res.size(); ++i) {
        ...
        if (applyRecursively) {
            csl::PartialExpandImplementation(
                sumTerms[i], isEmitter, isReceiver);
        }
    }
    return csl::sum_s(sumTerms);
}

bool hasCommonIndex(std::vector<size_t> const &sortedA,
...
bool mergeChains(std::vector<ExpansionChain> &toExpand)
{
    ...
    for (size_t i = 0; i != toExpand.size(); ++i) {
        for (size_t j = i + 1; j < toExpand.size(); ++j) {
            if (toExpand[i].posToExpand == toExpand[j].posToExpand) {
                toExpand.erase(toExpand.begin() + j);
                --j;
            }
            else if (hasCommonIndex(toExpand[i].posToExpand,
...
void PartialExpandImplementation(Expr &                  prod,
                                 ExpanderEmitter const & isEmitter,
                                 ExpanderReceiver const &isReceiver)
{
    if (!csl::IsProd(prod))
    ...
    for (size_t i = 0; i != toExpand.size(); ++i) {
        bool hasSum = false;
        for (size_t pos : toExpand[i].posToExpand) {
    ...
    //     for (size_t i : chain.posToExpand)
    //         std::cout << i << " ; ";
    //     std::cout << std::endl;
    // }
...
Expr PartialExpanded(Expr const &            init,
                     ExpanderEmitter const & isEmitter,
                     ExpanderReceiver const &isReceiver)
{
...
Expr DeepPartialExpanded(Expr const &            init,
                         ExpanderEmitter const & isEmitter,
                         ExpanderReceiver const &isReceiver)
{
...
void DeepPartialExpand(Expr &                  init,
                       ExpanderEmitter const & isEmitter,
                       ExpanderReceiver const &isReceiver)
{
