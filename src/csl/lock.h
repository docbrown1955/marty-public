...
class Lock {
  ...
  public:
     ...
     * may be totally useless and must be abbreviated in order to minimize
     * their impact on memory and computation time. To be clear, \b f should
     * return \b true on objects that are important for the calculation, i.e.
  ...
  private:
     ...
     * @sa lock()
     */
    static bool doLock(Expr &init, ID_t id, predicate const &f);
     ...
     * @sa unlock()
     */
    static void doUnlock(Expr &init, ID_t id);

  private:
    /**
     * @brief Static map containing lock names as a function of their id.
     */
    static inline std::map<ID_t, std::string> lockName;
};
