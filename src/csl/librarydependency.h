...
namespace csl {
    ...
    class LibDependency {
        ...
        static LibDependency const& cmath() {
            ...
            if (!created) {
                cmath.addInclude("cmath", "math.h", true);
                created = true;
            }
            return cmath;
        }
        ...
        static LibDependency const& complex() {
            ...
            static bool created = false;
            if (!created) {
                cmath.addInclude("complex", "complex.h", true);
                created = true;
            }
            return cmath;
        }
        ...
        struct IncludeType {
            ...
            IncludeType(std::string const& cppname, std::string const &cname)
                :name(cppname),
                cname(cname),
                global(false)
            {}

            IncludeType(std::string const& cppname,
        ...
        void addInclude(std::string const& cppinclude,
                        std::string const& cinclude,
                        bool               global = false) {
            IncludeType inc{cppinclude, cinclude, global};
            addInclude(inc);
        }
        ...
        void removeInclude(std::string const& t_include) {
            for (auto iter = include.begin(); iter != include.end(); ++iter)
                if (iter->name == t_include || iter->cname == t_include) {
                    include.erase(iter);
                    return;
                }
        }
        ...
        void printInclude(std::ostream& out, bool isCSource) const {
            for (const IncludeType& inc : include) {
                char separator[2]; 
                separator[0] = (inc.global) ? '<' : '\"';
                separator[1] = (inc.global) ? '>' : '\"';
                out << "#include " << separator[0] 
                    << ((isCSource && !inc.cname.empty()) ? inc.cname : inc.name)
                    << separator[1];
                out << '\n';
            }
        }
