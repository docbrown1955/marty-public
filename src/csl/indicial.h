 ...
 * bug if all the indices don't lie in the same space. When taking
 * permutations, indices from different spaces will be exchanged and the tensor
...
class SelfContraction {
  ...
  public:
    ...
    /*! \brief Constructor. Takes two tensors and determines the contractions
     * between the two. \param A Must be an TensorElement. \param B Must be an
     * TensorElement.
     */
    SelfContraction(
        const Expr &                                             A,
        const Expr &                                             B,
        std::optional<std::function<bool(Expr_info, Expr_info)>> t_condition
        = std::nullopt);
  ...
  private:
    ...
    IndexStructure getFreeContractionIndex(const Expr &expr, int index) const;
...
class ContractionChain {
  ...
  public:
    ...
    explicit ContractionChain(const csl::vector_expr &args,
                              const Expr &            res = CSL_UNDEF);

    explicit ContractionChain(const Expr &expr, const Expr &res = CSL_UNDEF);
  ...
  public:
    ...
    std::optional<std::vector<int>> comparison(const ContractionChain &other,
                                               Expr &remnant) const;
  ...
  private:
    ...
    getPermutations(const ContractionChain &other) const;

    std::optional<ContractionChain>
    createChain(int nChains, const std::vector<int> &chainNumber) const;

    static void
    mergeChains(std::vector<int> &chainNumber, int chain1, int chain2);
    ...
    bool
    isGoodIndex(csl::Space const *space, size_t iTensor, size_t iIndex) const;

    std::pair<csl::Index, csl::Index>
    getContractedIndices(csl::Space const *space, size_t iTensor) const;
...
/*!
 * \brief Parent of an indicial object, allows to generate them
 * (TensorElement).
 */
class TensorParent : public AbstractParent {
  ...
  protected:
    ...
    mutable std::map<const csl::Space *, std::vector<csl::Parent>> brokenParts;
  ...
  public:
    template <class... Args>
    friend std::shared_ptr<TensorParent> tensor_s(Args &&... args);

    template <class... Args>
    friend std::shared_ptr<TensorParent>
    tensor_s(std::string const &               name,
    ...
    void addContractionProperty(csl::vector_expr const &leftHandSide,
                                const Expr &rightHandSide) override;

    void removeContractionProperty(csl::vector_expr const &leftHandSide,
                                   const Expr &rightHandSide) override;

    std::optional<Expr> getComplexProperty(Expr_info self) const override;

    std::optional<Expr>
    getHermitianProperty(Expr_info self, const Space *t_space) const override;

    std::optional<Expr>
    getTransposedProperty(Expr_info self, const Space *t_space) const override;
...
template <class... Args>
std::shared_ptr<TensorParent>
tensor_s(std::string const &               name,
         std::vector<const Space *> const &indices,
         Args &&... args)
{
    return csl::make_shared<TensorParent>(
        name, indices, std::forward<Args>(args)...);
}
...
class MetricParent : public TensorParent {
  ...
  public:
    ...
    MetricParent(const Space *t_space, const std::string &t_name = "g");
...
class Tensor : public std::shared_ptr<TensorParent> {
  ...
  public:
     ...
     * \param t_space List of the n spaces in which each index lives.
     */
    Tensor(const std::string &               t_name,
           const std::vector<const Space *> &t_space)
        : Tensor(tensor_s(t_name, t_space))
    {
    }
...
class TensorElement : public AbstractElement {
  ...
  public:
    ...
    std::optional<Expr> getTransposed(const Space *space,
                                      bool applyProp = true) const override;

    std::optional<Expr>
    getTransposed(const std::vector<const Space *> &t_spaces,
                  bool applyProp = true) const override;
     ...
     * TensorElement, i.e. if \b expr is equal to the tensor to (may be) a
     * renaming of index. \details If \b exact is set to \b true, the function
     * returns \b true if the IndexStructure matches exactly. Else, the
    ...
    /*!
     * \brief Applies a permutation of the indices on a copy of the
     * TensorElement, and returns it.
     *
  ...
  protected:
    ...
    csl::vector_expr
    applyBrokenIndices(std::vector<Parent> &             brokenParents,
                       const Space *                     broken,
                       const std::vector<const Space *> &newSpaces,
 ...
 * to name elements of a vector \f$ V \f$: \f$ \left(
 * V_0,V_1,V_2,V_3\right)\f$.
 *
...
/*!
 * \brief Specialization of a Prod object to handle TensorElement objects in
 * the product (apply contraction of indices etc).
 */
class ISum : public Sum {
  ...
  public:
    ...
    std::optional<Expr> getTransposed(const std::vector<const Space *> &spaces,
                                      bool applyProp = true) const override;
