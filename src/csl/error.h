...
enum Error {
    ...
    ContractDummy,      /*!<  Trying to contract an index that is already
                           contracted. */
    ...
    BadCommutationProp, /*!< Setting the commutation of an object for which it
                           is determined. */
    NegativeGroupDimension, /*!<  Negative dimension in creation of group. */
    NoContractionProperty,  /*!<  Calling a non-exiting special contraciton of
                               ITensors. */
    BadSelfContraction, /*!<  Self-contraction property of TensorParent badely
                           initialized. */
...
enum Warning {
    ...
    NCharatersNotImplemented, /*!<  The function getNCharacters is not
                                 implemented for an object. */
 ...
 * of the caller function \b caller and a possible specificity of the error \b
 * spec. For example spec is the index for OutOfBound error. This function
 * stops the program. \param error The type of error. \param caller The caller
 * function. \param spec A printable specificity of the error.
 */
template <typename T>
inline void callError(cslError::Error error, const std::string &caller, T spec)
{
    ...
    case AbstractFuncCalled:
        std::cout << "Abstract function \"" << caller
                  << "\" called. Should not be, virtual function.\n";
        break;
    case BadCopyConstructor:
        std::cout << "Copy constructor called with wrong type in " << caller
                  << ": " << spec << ".\n";
        break;
    case OutOfBounds:
    ...
    case ElementSequence:
        std::cout << "Initializing a vector element with a sequence in "
                  << caller << ".\n";
        break;
    case EvalNotValued:
        std::cout << "Evaluating the value of non valued Abstract in "
                  << caller << ".\n";
        break;
    case UndefinedBehaviour:
        ...
        break;
    case ContractDummy:
        std::cout << "Contracting dummy index in " << caller << ": " << spec
                  << std::endl;
        break;
    case ContractionMismatch:
        std::cout << "Mismatch in contraction (" << caller << "): " << spec
                  << std::endl;
        break;
    ...
    case UnknownProperty:
        std::cout << "Unknown property in " << caller << ": " << spec
                  << std::endl;
        break;
    case InvalidITensor:
        std::cout << "Bad set of indices in creation of TensorElement in "
                  << caller << std::endl;
        break;
    case InvalidIndicialParent:
        std::cout << "Invalid initialization of TensorParent in " << caller
                  << ": bad tensor.\n";
        break;
    case InvalidIndicialSum:
        std::cout << "Invalid sum of indicial objects in " << caller
                  << std::endl;
        break;
    case BadDependency:
        ...
        break;
    case BadType:
        std::cout << "Bad type of expression sent in " << caller
                  << ": requires " << spec << ".\n";
        break;
    case BadSymmetry:
        std::cout << "Bad symmetry set in TensorParent (wrong dimension) in "
                  << caller << ".\n";
        break;
    case BadContraction:
    ...
    case BadPropertySetting:
        std::cout << "Setting a property with something else than an equality "
                     "in "
                  << caller << ": " << spec << ".\n";
        break;
    case ArbitraryEvaluated:
        std::cout << "An arbitrary abstract has been evaluated from " << caller
                  << ". Should not be.\n";
        break;
    ...
    case BadCommutationProp:
        std::cout << "Setting commutability property of an object for which "
                     "it is not possible in "
                  << caller << ".\n";
        break;
    case NegativeGroupDimension:
        std::cout << "Negative dimension encountered in " << caller << ": "
                  << spec << std::endl;
        break;
    case NoContractionProperty:
        std::cout << "Calling a special contraction of TensorElement in "
                  << caller << ", but there is no contraction.\n";
        break;
    case BadSelfContraction:
        std::cout << "Initializing badly a self-contraction property of "
                     "TensorParent in "
                  << caller << ".\n";
        break;
    case BadFieldInitialization:
        std::cout << "Bas initialization of field in " << caller
...
/*! \fn callWarning(cslError::Warning warning, const std::string& caller, T
 * spec) \brief Displays a warning message depending on the warning \b warning,
 * the name of the caller function \b caller and a possible specificity of the
 * warning \b spec. For example spec is the dimension for InvalidDimension
 * error. This function does not stop the program. \param warning The type of
 * warning. \param caller The caller function. \param spec A printable
 * specificity of the warning.
 */
template <typename T>
inline void
callWarning(cslError::Warning warning, const std::string &caller, T spec)
{
    using namespace cslError;
    ...
    case InvalidDimension:
        ...
        break;
    case FactorialFloat:
        std::cout << "Float value encountered in Factorial. Replacing by "
                     "rounded value.\n";
        break;
    case NotValued:
        std::cout << "Evaluating non valued in " << caller
                  << ". Replacing by 0.\n";
        break;
    case Infinity:
    ...
    case NCharatersNotImplemented:
        std::cout << "The function getNCharacters() is not implemented for "
                     "type "
                  << spec << " in " << caller << ".\n";
        break;
