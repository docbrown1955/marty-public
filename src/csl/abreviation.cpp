                   ...
                   std::vector<AbstractParent *> const &v)
{
    auto iter = csl::dichotomyFindIf(
        v.begin(), v.end(), [&](AbstractParent const *parent) {
...
bool Abbrev::compareParents::operator()(AbstractParent *const &A,
                                        AbstractParent *const &B)
{
    return compare(A->getName(), B->getName()) == -1;
}

std::vector<AbstractParent *> &
Abbrev::getAbbreviationsForName(std::string_view name)
{
    for (auto iter = abbreviationData.begin(); iter != abbreviationData.end();
         ++iter) {
...
void Abbrev::addAbbreviation(AbstractParent *ptr, std::string const &name)
{
...
void Abbrev::removeAbbreviation(AbstractParent *ptr, std::string const &name)
{
    auto &abbreviations = getAbbreviationsForName(name.data());
...
static std::vector<csl::Expr> commonFactors(csl::Expr const &prod1,
    ...
    for (size_t i = 0; i != abbreviations.size(); ++i) {
        ...
        for (size_t j = i + 1; j < abbreviations.size(); ++j) {
            ...
            if (!csl::IsProd(encaps_j) || csl::IsIndexed(encaps_j))
                continue;
            const int diff = std::abs(static_cast<int>(csl::Size(encaps_i))
                                      - static_cast<int>(csl::Size(encaps_j)));
            if (diff <= 2) {
                [[maybe_unused]] std::vector<csl::Expr> factors
                    = commonFactors(encaps_i, encaps_j);
            }
        }
    }
}
...
void Abbrev::printAbbreviations(std::string_view name, std::ostream &fout)
{
    auto abbreviations = getAbbreviationsForName(name);
    std::sort(abbreviations.begin(),
              abbreviations.end(),
    ...
    for (const auto &ab : abbreviations) {
        fout << ab->getName() << " = " << ab->getEncapsulated() << std::endl;
    }
}
...
csl::IndexStructure
Abbrev::getFreeStructure(csl::IndexStructure const &structure)
{
...
csl::IndexStructure Abbrev::getFreeStructure(Expr const &expr)
{
...
Expr termWithoutExponent(Expr const &expr)
{
    if (expr->getType() == csl::Type::Pow) { // Pow
        if (expr->getArgument(1)->getPrimaryType()
...
Expr Abbrev::makeSubAbbrev(std::vector<Expr> const &encapsulated, bool isProd)
{
    ...
    return (isProd) ? makeAbbreviation(prod_s(encapsulated), false)
                              ...
                              bool        split)
{
    ...
    if (name == "Ab" && split && (csl::IsSum(encaps) || csl::IsProd(encaps))) {
    ...
    Tensor parent = csl::make_shared<Abbreviation<TensorParent>>(
        encaps, name, spaces, CSL_UNDEF);
...
{
    return makeAbbreviation("Ab", encapsulated, split);
}
