...
class counter_iterator {
  ...
  public:
    counter_iterator(counter<T> *t_ptr) : ptr(t_ptr)
    {
    }

    template <class U>
    counter_iterator(counter_iterator<U> const &other) : ptr(other.ptr)
    {
    }
...
class counter {
  ...
  public:
    ...
    counter(size_t t_max) : max(t_max), value(T(0))
    {
    }
    ...
    counter(const counter &other) : max(other.max), value(other.value)
    {
    ...
    friend std::ostream &operator<<(std::ostream &out, counter const &c)
    {
        out << c.value;
