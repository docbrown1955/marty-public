...
class ObjectPermutation {
  ...
  public:
    ...
    explicit ObjectPermutation(const std::map<T, T> &otherMap);

    explicit ObjectPermutation(std::map<T, T> &&otherMap);
...
class ObjectSymmetry {
  ...
  public:
    ...
    explicit ObjectSymmetry(const ObjectPermutation<T> &first);

    explicit ObjectSymmetry(const std::vector<ObjectPermutation<T>> &t_perm);

    explicit ObjectSymmetry(std::vector<ObjectPermutation<T>> &&t_perm);
...
template <class T>
ObjectPermutation<T>::ObjectPermutation(T a, T b, bool symmetric)
{
    add(a, b);
    if (symmetric)
        add(b, a);
...
template <class T>
void ObjectPermutation<T>::add(T a, T b)
{
    permutation[a] = b;
    checkRightForm();
}
...
{
    return (rightForm == other.rightForm and permutation == other.permutation);
}
...
template <class T>
ObjectSymmetry<T>
ObjectPermutation<T>::operator+(const ObjectSymmetry<T> &other) const
{
    return other + *this;
}
...
template <class T>
std::vector<T>
ObjectPermutation<T>::operator[](const std::vector<T> &keys) const
{
    if (not rightForm)
...
template <class T>
ObjectSymmetry<T>::ObjectSymmetry(std::vector<ObjectPermutation<T>> &&t_perm)
    : permutations(t_perm)
{
}

template <class T>
void ObjectSymmetry<T>::push_back(T a, T b, bool symmetric)
{
...
ObjectSymmetry<T>::operator[](const std::vector<T> &keys) const
{
