...
void LibraryExpander::apply(Expr &expr, csl::Space const *space)
{
...
bool LibraryExpander::toExpand(Expr const &term, csl::Space const *space)
{
    ...
    return csl::AllOfLeafs(term, [&](Expr const &el) {
        return (!el->isIndexed() or el->getParent() == g
                or el->getParent() == del);
    });
}

Structure LibraryExpander::parseTerm(Expr const &term, csl::Space const *space)
{
    Structure   s;
    csl::Tensor g   = space->getMetric();
    csl::Tensor del = space->getDelta();
    ...
    auto apply = [&](Expr const &el) {
        ...
        CSL_ASSERT_SPEC(parent == g or parent == del,
                        ...
                        toString(el)
                            + " encoutered, should have only metric or "
                              "delta.");
...
std::vector<Structure> LibraryExpander::parse(Expr const &      isum,
                                              csl::Space const *space)
{
    ...
    CSL_ASSERT_SPEC(isum->getType() == csl::Type::Sum and isum->isIndexed(),
                    CSLError::TypeError,
...
size_t LibraryExpander::nCommonIndices(Structure const &s1,
                                       Structure const &s2)
{
    size_t nCommon = 0;
...
void LibraryExpander::merge(std::vector<Structure> &      A,
...
LibraryExpander::merge(std::vector<std::vector<Structure>> &init,
                       csl::Space const *)
{
    if (init.empty())
        return {};
    ...
    while (!indicesLeft.empty()) {
        ...
        for (auto iter = indicesLeft.begin(); iter != indicesLeft.end();
             ++iter)
            if (*iter == iMaxCommon) {
...
Expr LibraryExpander::toExpr(Structure const &init, csl::Space const *space)
{
    csl::Tensor       g = space->getMetric();
...
Expr LibraryExpander::toExpr(std::vector<Structure> const &init,
                             csl::Space const *            space)
{
    std::vector<Expr> res;
...
bool LibraryExpander::expand(Expr &iprod, csl::Space const *space)
{
...
Structure::Structure() : factor(CSL_1)
{
    indices.reserve(reserveStorage);
}

Structure::Structure(Structure const &other) : Structure()
{
...
void Structure::mult(ipair const &p, Expr const &t_factor)
{
...
int Structure::commonIndex(ipair const &p1, ipair const &p2)
{
