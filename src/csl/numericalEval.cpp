...
NumericalEval::NumericalEval(long double t_value) : value(t_value)
{
}

NumericalEval::NumericalEval(long double t_value, long double t_delta)
    : value(t_value), delta_plus(t_delta), delta_minus(t_delta)
{
}
...
NumericalEval::NumericalEval(long double t_value,
    ...
    : value(t_value), delta_plus(t_delta_plus), delta_minus(t_delta_minus)
{
}
...
void NumericalEval::printCode(int, std::ostream &out) const
{
...
std::string NumericalEval::printLaTeX(int mode) const
{
    if (value < 0) {
        return "-"
               + NumericalEval(-value, delta_minus, delta_plus)
                     .printLaTeX(mode);
    }
...
std::optional<Expr> NumericalEval::evaluate(csl::eval::mode) const
{
    return std::nullopt;
}
...
Expr NumericalEval::addition_own(const Expr &other) const
{
    ...
    return csl::make_shared<NumericalEval>(
        value + other->evaluateScalar(), new_delta_plus, new_delta_minus);
}
...
Expr NumericalEval::multiplication_own(Expr const &other, bool) const
{
    ...
    return csl::make_shared<NumericalEval>(
        new_value, new_delta_plus, new_delta_minus);
}
...
Expr NumericalEval::exponentiation_own(const Expr &other) const
{
    CSL_ASSERT_SPEC(other->getType() != csl::Type::Complex,
                    ...
                    "Cannot exponentiate a NumericalEval with a Complex "
                    "number.");
                     ...
                     long double delta_minus)
{
    ...
    return csl::make_shared<NumericalEval>(value, delta_plus, delta_minus);
}
