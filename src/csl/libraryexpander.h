...
class LibraryExpander {
  ...
  public:
    static void apply(Expr &iprod, csl::Space const *space = &csl::Minkowski);

  private:
    static bool expand(Expr &iprod, csl::Space const *space);

    static bool toExpand(Expr const &term, csl::Space const *space);

    static Structure parseTerm(Expr const &iprod, csl::Space const *space);

    static std::vector<Structure> parse(Expr const &      isum,
                                        csl::Space const *space);

    static size_t nCommonIndices(Structure const &s1, Structure const &s2);

    static void merge(std::vector<Structure> &      A,
                      std::vector<Structure> const &B);

    static std::vector<Structure>
    merge(std::vector<std::vector<Structure>> &init, csl::Space const *space);

    static Expr toExpr(Structure const &init, csl::Space const *space);

    static Expr toExpr(std::vector<Structure> const &init,
                       csl::Space const *            space);
};
...
struct Structure {
    ...
    struct ipair {
      ...
      public:
        ...
        inline csl::Index const &getFirst() const
        {
            return first;
        }
        inline csl::Index const &getSecond() const
        {
            return second;
        }
