...
class float_sap {
  ...
  public:
    ...
    explicit float_sap(long double value);

    explicit float_sap(const std::vector<short> &t_digits);
  ...
  private:
    ...
    friend float_sap<base> abs(const float_sap<base> &a)
    {
    ...
    friend float_sap<base> floor(const float_sap<base> &a)
    {
        if (a.exponent < 1) {
            if (a.getMinusSign())
    ...
    friend float_sap<base> ceil(const float_sap<base> &a)
    {
        if (a.exponent >= a.mantissa.size()) {
    ...
    friend float_sap<base> round(const float_sap<base> &a)
    {
        if (a.exponent >= a.mantissa.size())
            return floor(a);
                                    ...
                                    const float_sap<base> &floating)
    {
        ...
        if (f.isScientific()) {
            fout << convertDigit(
                floating.mantissa[floating.mantissa.size() - 1])
                 << ".";
            for (size_t i = 1; i != floating.mantissa.size(); ++i)
                if (i > f.getMaxDigits())
                    ...
                    fout
                        << floating.mantissa[floating.mantissa.size() - i - 1];
        ...
        for (size_t i = 0; i != floating.mantissa.size(); ++i) {
            ...
            if (floatingPoint) {
                ...
                fout << convertDigit(
                    floating.mantissa[floating.mantissa.size() - 1 - i]);
        }

        return fout;
    }

    friend float_sap<base> operator+(long long int a, const float_sap<base> &b)
    {
        return float_sap<base>(a) + b;
    }

    friend float_sap<base> operator+(const float_sap<base> &a, long long int b)
    {
        return a + float_sap<base>(b);
    }

    friend float_sap<base> operator+(const float_sap<base> &a,
                                     const float_sap<base> &b)
    {
        float_sap<base> c = a;
        return c += b;
    }

    friend float_sap<base> &operator+=(float_sap<base> &a, float_sap<base> b)
    {
        int diff;
        if (a.exponent > b.exponent) {
            diff = (a.exponent - b.exponent
                    - (a.mantissa.size() - b.mantissa.size()))
                       .toCInteger();
        }
        else if (b.exponent > a.exponent) {
            diff = (b.exponent - a.exponent
                    - (b.mantissa.size() - a.mantissa.size()))
                       .toCInteger();
        }
        else
            diff = 0;
    ...
    friend float_sap<base> &operator+=(float_sap<base> &a, long long int b)
    {
        return a += float_sap<base>(b);
    }
    ...
    friend float_sap<base> operator++(float_sap<base> &a, int)
    {
        float_sap<base> b = a;
        a += 1;
        return b;
    ...
    friend float_sap<base> &operator++(float_sap<base> &a)
    {
        a += 1;
        return a;
    }
    ...
    {
        if (b == 0) {
            std::cout << "Division by 0 encountered in float_sap<" << base
                      << ">.\n";
            std::raise(SIGFPE);
        }
    ...
    friend float_sap<base> pow(long long int a, const float_sap<base> &b)
    {
        return pow(float_sap<base>(a), b);
    }

    friend float_sap<base> pow(const float_sap<base> &a, long long int b)
    {
        return pow(a, float_sap<base>(b));
    }

    friend float_sap<base> pow(const float_sap<base> &a,
                               const float_sap<base> &b)
    {
        if (b <= 0) {
            if (b == 0) {
    ...
    friend bool operator==(long long int a, const float_sap<base> &b)
    {
        return float_sap<base>(a) == b;
    }

    friend bool operator==(const float_sap<base> &a, long long int b)
    {
        return a == float_sap<base>(b);
    }

    friend bool operator==(const float_sap<base> &a, const float_sap<base> &b)
    {
...
float_sap<base>::float_sap()
    : mantissa(int_ap<base>(1)), exponent(int_ap<base>(0))
{
}
...
float_sap<base>::float_sap(const int_ap<base> &t_mantissa,
                           const int_ap<base> &t_exponent)
    : mantissa(t_mantissa), exponent(t_exponent)
{
}
