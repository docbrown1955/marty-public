...
void compress_impl(std::vector<csl::Expr> &expressions, size_t nIter)
{
...
void compress(std::vector<csl::Expr> &expressions, size_t nIter)
{
        ...
        csl::ForEachNode(expr, [&](csl::Expr &sub) {
            if (csl::Abbrev::isAnAbbreviation(sub) && !csl::IsIndexed(sub)) {
                ...
                if (pos != compressedAbbrevs.end()) {
                    ...
                    std::string nameAbbrev
                        = sub->getParent_info()->getBaseName();
...
Node::findBestMatch(std::vector<csl::Expr>::const_iterator first,
...
csl::Expr Node::getChainExpr(std::vector<csl::Expr> const &args, ExprType type)
{
    return (type == ExprType::Sum) ? csl::sum_s(args, true)
...
csl::Expr Node::makeAbbreviation(csl::Expr const &expr, ExprType type)
{
    if (type == ExprType::Sum) {
        return csl::Abbrev::makeAbbreviation("SubSum", expr);
    }
    else {
        return csl::Abbrev::makeAbbreviation("SubProd", expr);
    }
}

csl::Expr Node::makeExpression(std::vector<csl::Expr> const &args,
                               ExprType                      type)
{
    if (type == ExprType::Sum) {
...
void Node::setAbbreviation(std::vector<Tree *> &trees, ExprType type)
{
    if (abbreviation)
        return;
...
void Node::parse(std::vector<csl::Expr>::const_iterator first,
                 std::vector<csl::Expr>::const_iterator last)
{
...
Node *Node::build(csl::Expr const &t_expr, Node *t_parent)
{
...
void Node::removeSingle(Node *&node)
{
    if (node->parent && node->nOccurences == 1) {
        ...
        auto last = std::remove_if(node->children.begin(),
                                   node->children.end(),
                                   [](Node *subNode) { return !subNode; });
...
void Node::print(int indent) const
{
    if (parent) {
        ...
        if (abbreviation) {
            ...
            std::cout << "                   AB : " << abbreviation << " = "
                      << csl::Evaluated(abbreviation, csl::eval::abbreviation);
            std::cout << std::endl;
            std::cout << "                   AB : " << chainAbbreviation
                      << " = "
                      << csl::Evaluated(chainAbbreviation,
...
Tree::Tree(Node::ExprType t_type) : type(t_type), root(Node::build(CSL_UNDEF))
{
}
...
std::pair<Node *, std::vector<csl::Expr>::const_iterator>
Tree::findBestMatch(std::vector<csl::Expr> const &vec, size_t minElements)
{
...
Tree::getChainAbbreviationFor(csl::Expr const &    init,
...
void Tree::findAllAbbreviations(Node *               node,
                                Tree *               t,
                                std::vector<Tree *> &trees)
{
    if (node->empty()) {
