...
Expr FindLeaf(Expr const &init, Expr const &value, size_t depth)
{
    if (init->size() == 0) {
        if (*init == value.get())
            return init;
...
Expr FindNode(Expr const &init, Expr const &value, size_t depth)
{
    if (*init == value)
        return init;
    else if (depth != 0) {
...
void VisitEachNodeCut(Expr const &                             init,
                      std::function<bool(Expr const &)> const &f,
                      int                                      depth)
{
    if (f(init))
...
void VisitEachNodeCut(Expr_info                             init,
                      std::function<bool(Expr_info)> const &f,
                      int                                   depth)
{
    if (f(init))
...
{
    return csl::AllOfNodes(
        expr, [&](csl::Expr const &sub) { return sub.use_count() == 1; });
}
...
size_t CountIf(csl::Expr const &                             expr,
...
{
    size_t count = 0;
    csl::VisitEachNode(init, [&count](Expr const &) { ++count; });
...
{
    size_t count = 0;
    csl::VisitEachLeaf(init, [&count](Expr const &) { ++count; });

    return count;
}

void ForEachNode(Expr &init, std::function<void(Expr &)> const &f, int depth)
{
    f(init);
    if (depth != 0) {
        for (size_t i = 0; i != init->size(); ++i)
...
void ForEachLeaf(Expr &init, std::function<void(Expr &)> const &f, int depth)
{
    if (init->size() == 0)
        f(init);
    else if (depth != 0) {
...
bool Transform(Expr &init, std::function<bool(Expr &)> const &f, int depth)
{
