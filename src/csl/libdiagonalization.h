...
class Diagonalizer {
  ...
  public:
    ...
    static void applyDiagonalization(std::initializer_list<complex> massMatrix,
                                     std::initializer_list<complex *> transfer,
                                     std::initializer_list<real *>    masses);

    static void
    applyBiDiagonalization(std::initializer_list<complex>   massMatrix,
                           std::initializer_list<complex *> transfer,
                           std::initializer_list<complex *> transfer2,
                           std::initializer_list<real *>    masses);

    Diagonalizer(std::initializer_list<complex> massMatrix,
                 bool                           biDiag = false);
  ...
  private:
    ...
    void positiveDiagonal(std::vector<complex> &transfer);

    std::vector<complex> hermitian(std::vector<complex> const &init) const;

    std::vector<complex> dot(std::vector<complex> const &A,
                             std::vector<complex> const &B) const;
