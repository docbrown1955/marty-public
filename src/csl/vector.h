...
class Matrix : public AbstractVectorial {
  ...
  public:
    ...
    Matrix(int         t_x_nArgs,
           int         t_y_nArgs,
           const Expr &expr,
           const Expr &index_x = CSL_0,
...
Expr matrix_s(
    const std::initializer_list<std::initializer_list<Expr>> &t_matrix);
...
class HighDTensor : public AbstractVectorial {
  ...
  public:
    ...
    explicit HighDTensor(const std::vector<int> &shape, const Expr &filler);
...
Expr highdtensor_s(
    const std::initializer_list<
        std::initializer_list<std::initializer_list<Expr>>> &t_tensor);
