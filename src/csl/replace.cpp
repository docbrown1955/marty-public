...
{
    Replace(expr, std::vector<csl::Expr>{from}, std::vector<csl::Expr>{to});
}
             ...
             std::vector<csl::Parent> const &to)
{
    if (from.empty())
        return;
    CSL_ASSERT_SPEC(from.size() == to.size(),
                    CSLError::ValueError,
    ...
    applyThroughAbbreviations(
        expr, [&](csl::Expr const &sub, bool isPredicate) {
            return tensorReplacement(sub, from, to, isPredicate);
        });
}
             ...
             std::vector<csl::Expr> const &to)
{
    ...
    applyThroughAbbreviations(
        expr, [&](csl::Expr const &sub, bool isPredicate) {
            return scalarReplacement(sub, from, ccFrom, to, isPredicate);
        });
}
                     ...
                     std::vector<csl::Expr> const &to)
{
    ...
    std::transform(
        ...
        from.end(),
        parentFrom.begin(),
        [&](csl::Expr const &sub) { return sub->getParent_info(); });

    // Asserting the correctness of replacements
    for (size_t i = 0; i != from.size(); ++i) {
        CSL_ASSERT_SPEC(parentFrom[i],
                        CSLError::TypeError,
                        "Expected indicial tensor in replace, "
                            + toString(expr) + " given.");
    ...
    applyThroughAbbreviations(
        expr, [&](csl::Expr const &sub, bool isPredicate) {
            return indicialReplacement(sub, from, parentFrom, to, isPredicate);
        });
}
             ...
             bool                            refresh)
{
    if (from.empty())
        return;
    CSL_ASSERT_SPEC(from.size() == to_init.size(),
                    CSLError::ValueError,
                    "Expecting vectors of the same size, vectors of size "
                        + toString(from.size()) + " and "
                        + toString(to_init.size()) + " given.")
    // Getting copies with dummy indices reset from to_init
    csl::WeakDeepCopy(expr);
    std::vector<csl::Expr> to(to_init.size());
    std::transform(to_init.begin(),
                   to_init.end(),
                   to.begin(),
                   [&](csl::Expr const &expr) {
    ...
    applyThroughAbbreviations(
        expr, [&](csl::Expr const &sub, bool isPredicate) {
            return tensorExpressionReplacement(sub, from, to, isPredicate);
        });
    if (refresh)
...
{
    bool commonIndex
        = std::any_of(begin(from), end(from), [&to](csl::Index const &i) {
    ...
    if (commonIndex) {
        ...
        return csl::Replaced(
            csl::Replaced(expr, from, interm, refresh), interm, to, refresh);
    }
                  ...
                  bool                          isPredicate)
{
    ...
    if (csl::IsPow(expr)) {
        ...
        auto        pos
            = std::find_if(begin(from), end(from), [&](csl::Expr const &f) {
                    ...
                    bool                                 isPredicate)
{
    ...
    if (posF != end(parentFrom)) {
        ...
        if (not structureToSave.empty())
            csl::ApplyIndices(
                newSub, from[i]->getIndexStructure(), structureToSave);
        return newSub;
    }
    return std::nullopt;
}
                  ...
                  bool                            isPredicate)
{
    ...
    auto pos = std::find_if(begin(from), end(from), [&](csl::Parent const &p) {
        return p && p.get() == parent;
    });
    auto posPoint
        = std::find_if(begin(from), end(from), [&](csl::Parent const &p) {
    ...
    if (csl::IsField(expr) && posPoint != end(from)) {
        ...
        CSL_ASSERT_SPEC(tensorPoint,
                        CSLError::TypeError,
                        "Expecting a tensor for replacing point "
                            + point->getName() + ", " + to[i]->getName()
                            + " given.")
                            ...
                            bool                            isPredicate)
{
    ...
    if (pos != end(from)) {
        ...
        if (to[i] == CSL_0)
            return CSL_0;
        IndexStructure structureToSave = expr->getFreeIndexStructure();
        csl::Expr      res;
...
void ApplyIndices(csl::Expr &expr, csl::IndexStructure const &freeStructure)
{
