                       ...
                       bool                                     refactor)
{
    ...
    csl::DeepExpandIf(
        expression,
        [&](Expr const &sub) {
            return csl::AnyOfNodes(sub,
                                   [&](Expr const &node) { return f(node); });
        },
        inplace);
    if (refactor) {
...
Expr DeepExpandedIf_lock(Expr const &                             expression,
                         std::function<bool(Expr const &)> const &f,
                         int                                      lockID,
                         bool                                     inplace,
                         bool                                     refactor)
...
Expr Distributed(Expr const &expression, Parent_info factor, bool full)
{
...
void Distribute(Expr &expr, Parent_info factor, bool full)
{
...
Expr Distributed(Expr const &expression, Expr_info factor, bool full)
{
    Expr res = DeepCopy(expression);
    Distribute(res, factor, full);
    return res;
...
void Distribute(Expr &expr, Expr_info factor, bool full)
{
    if (expr->getType() == csl::Type::Prod) {
        for (size_t i = 0; i != expr->size(); ++i) {
...
Expr DistributedIf(Expr const &                      expression,
...
void DistributeIf(Expr &expr, std::function<bool(Expr const &)> f, bool full)
{
    if (expr->getType() == csl::Type::Prod) {
        for (size_t i = 0; i != expr->size(); ++i) {
            if (f(expr[i])) {
...
Expr GetTransposed(const Expr &expression, const Space *space, bool applyProp)
{
             ...
             bool         refresh)
{
    ...
    Expr res = Replaced(
        expression, index1, static_cast<csl::Index const &>(foo), false);
    res = Replaced(res, index2, index1, false);
    res = Replaced(res, static_cast<csl::Index const &>(foo), index2, false);
    if (refresh)
             ...
             bool        refresh)
{
    ...
    Expr res = Replaced(expression, index1, foo);
    res      = Replaced(res, index2, index1);
    res      = Replaced(res, foo, index2);
    if (refresh)
        return res->deepRefresh();
    return res;
}
             ...
             bool          refresh)
{
    ...
    Expr   res = Replaced(expression, parent1, foo);
    res        = Replaced(res, parent2, parent1);
    res        = Replaced(res, foo, parent2);
    if (refresh)
        return res->deepRefresh();
    return res;
}
...
{
    auto parent = std::dynamic_pointer_cast<TensorParent>(GetParent(tensor));
    CSL_ASSERT_SPEC(parent,
                    CSLError::TypeError,
                    "Expression " + toString(tensor)
                        + " has no tensor parent.");
    return parent;
}
TensorField GetTensorFieldParent(Expr const &tensor)
{
    auto parent
        = std::dynamic_pointer_cast<TensorFieldParent>(GetParent(tensor));
    CSL_ASSERT_SPEC(parent,
                    CSLError::TypeError,
                    "Expression " + toString(tensor)
                        + " has no tensor field parent.");
    return parent;
}
TDerivative GetTDerivativeParent(Expr const &tensor)
{
    auto parent
        = std::dynamic_pointer_cast<TDerivativeParent>(GetParent(tensor));
    CSL_ASSERT_SPEC(parent,
                    CSLError::TypeError,
                    "Expression " + toString(tensor)
...
bool CheckValidity(Expr const &init, std::vector<Expr_info> encountered)
{
    if (not init)
        return false;
...
size_t MemorySizeOf(Expr const &expression)
{
    ...
    if (csl::IsIndicialTensor(expression)) {
        ...
        size
            += expression->getIndexStructureView().size() * sizeof(csl::Index);
    }
