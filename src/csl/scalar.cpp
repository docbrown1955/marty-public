...
Expr Scalar::deepRefresh() const
{
    return scalar_
        (
         DeepRefreshed(argument[0]),
         DeepRefreshed(argument[1])
        );
}
        ...
        bool                              keepAllCosntraints) const
{
    ...
    if (argument[0]->compareWithDummy(
                other->getArgument(0).get(),
                constraints,
                keepAllCosntraints)
            and argument[1]->compareWithDummy(
                other->getArgument(1).get(),
                constraints,
                keepAllCosntraints)) {
        return true;
    }
