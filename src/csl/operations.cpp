...
Sum::Sum(const Expr &leftOperand, const Expr &rightOperand, bool explicitSum)
    : AbstractMultiFunc()
{
    ...
    if (leftOperand->getType() == csl::Type::Sum
    ...
    else if (rightOperand->getType() == csl::Type::Sum
             and leftOperand->getType() != csl::Type::Sum) {
...
bool Sum::isPurelyImaginary() const
{
    bool imag = std::all_of(
        argument.begin(), argument.end(), [](csl::Expr const &arg) {
...
void Sum::insert(const Expr &expr, bool explicitSum)
{
    ...
    if (not explicitSum) {
        ...
        for (auto arg = argument.begin(); arg != argument.end(); ++arg) {
            ...
            if (*term == term2) {
                *arg = prod_s(
                    (*arg)->getNumericalFactor()->addition_own(numericalFact),
                    term);
                if (**arg == CSL_0) {
                    // x-x = 0
...
void Sum::printCode(int, std::ostream &out) const
{
    out << "csl::sum_s({";
...
optional<Expr> Sum::evaluate(csl::eval::mode user_mode) const
// Evaluates the expression replacing valued Variables and constants by
// their values
{
    bool number = true;
    // In the loop we keep track separately of numbers and expression that do
    // not evaluate to numbers to save time.
...
// (2*x ~ x etc). Ex: 2 + 2*x + 1/2 + 1/2*x -> 5/2 + 5/2*x.
{
    ...
    for (size_t i = numericalFactor; i + 1 < argument.size(); i++) {
        ...
        for (size_t j = i + 1; j < argument.size(); j++) {
            ...
            if (*term == term2) {
                factor
                    = factor->addition_own(argument[j]->getNumericalFactor());
                matched = true;
    ...
    if (argument.size() == 2) {
        // cos^2 + sin^2 = 1
        if (csl::IsPow(argument[0]) and csl::IsPow(argument[1])
            and csl::IsCos(argument[0][0]) and csl::IsSin(argument[1][0])
            and argument[0][1] == CSL_2 and argument[1][1] == CSL_2
...
void testSimplificationRule(std::vector<Expr> const &arg)
{
    ...
    for (size_t i = 0; i != arg.size(); ++i)
        for (size_t j = 0; j < arg.size(); ++j) {
            ...
            if (!(arg[i] < arg[j]) and !(arg[j] < arg[i])
                and !(arg[i] == arg[j])) {
...
optional<Expr> Sum::derive(Expr_info expr) const
{
    ...
    for (size_t i = 0; i < argument.size(); i++) {
        if (not argument[i]->dependsOn(expr))
            continue;
        if (Expr derivative = Derived(argument[i], expr); derivative != CSL_0)
...
Expr Sum::getNumericalFactor() const
{
    const auto isValidFactor = [](csl::Expr const &num) {
        ...
        return (csl::IsInteger(num) && std::fabs(num->evaluateScalar()) < lim)
               || (csl::IsIntFraction(num)
                   && std::fabs(num->evaluateScalar()) < lim);
    };
    ...
    for (size_t i = 0; i != sz; ++i) {
        ...
        if (arg != CSL_0) {
            auto factor
                = (isValidFactor(arg)) ? arg : arg->getNumericalFactor();
...
int Sum::getOrderOf(Expr_info expr) const
{
    ...
    for (size_t i = 1; i != argument.size(); ++i)
        if (auto newOrder = argument[i]->getOrderOf(expr); newOrder < order)
            order = newOrder;
    return order;
}
...
void Sum::gatherFactors(csl::vector_expr &factors,
                        csl::vector_expr &arg,
                        bool              full) const
{
    arg = argument;
...
{
    // Here we clear redundant factors, i.e. different powers of the same
    // object. For example if x, x^2, x^3 are possible we only keep x^3.
...
std::optional<Expr> Sum::collect(std::vector<Expr> const &factors,
                                 bool                     full) const
{
    for (const auto &f : factors)
        CSL_ASSERT_SPEC(!f->isIndexed(),
                        CSLError::TypeError,
    ...
    for (size_t arg1 = 0; arg1 != indicesLeft.size(); ++arg1) {
        ...
        if (std::all_of(expo1.begin(), expo1.end(), [](Expr const &e) {
                return e == CSL_0;
            }))
            continue;
        sameFactors.clear();
        ...
        if (sameFactors.size() > 1) {
            ...
            for (size_t i = sameFactors.size(); i-- > 0;) {
                ...
                for (size_t j = 0; j != factors.size(); ++j) {
                    if (expo1[j] != CSL_0) {
                        auto removed
                            = term->suppressExponent(factors[j], expo1[j]);
                        CSL_ASSERT_SPEC(removed,
                                        CSLError::RuntimeError,
                                        "Expression " + toString(term)
                                            + " has not "
                                              "removed the factor "
                                            + toString(factors[j])
                                            + " correctly.")
...
Expr sum_s(const Expr &leftOperand, const Expr &rightOperand, bool explicitSum)
...
// in particular cases
{
    ...
    if (leftOperand->isIndexed() or rightOperand->isIndexed())
        foo = csl::make_shared<ISum, alloc_isum>(
            leftOperand, rightOperand, explicitSum);
    else
...
optional<Expr> Prod::getTerm() const
{
    if (argument.empty()
        ...
        foo.erase(foo.begin());
        auto res = (isIndexed()) ? csl::make_shared<IProd>()
...
bool Prod::askTerm(Expr_info expr, bool exact) const
{
    for (size_t i = 0; i < argument.size(); i++) {
        if (Comparator::freeIndexComparison(argument[i].get(), expr))
            return true;
        else if (!exact // and argument[i]->getType() == csl::Type::Pow and //
                        // Pow
                 && !argument[i]->isIndexed() && argument[i]->askTerm(expr))
            return true;
    }
...
Expr Prod::suppressTerm(Expr_info expr) const
{
    ...
    for (size_t i = 0; i < sz; i++) {
        ...
        if (not matched) {
            ...
            else if (arg->getType() == csl::Type::Sum and arg->askTerm(expr)) {
                newArg.push_back(arg->suppressTerm(expr));
...
std::optional<Expr> Prod::suppressExponent(Expr const &factor,
...
optional<Expr> Prod::getRealPart() const
{
    ...
    for (size_t i = 1; i < argument.size(); i++) {
        foo      = Copy(realPart);
        realPart = sum_s(
            prod_s(realPart, GetRealPart(argument[i])),
            prod_s(CSL_M_1,
                   prod_s(imaginaryPart, argument[i]->getImaginaryPart())));
        if (i + 1 < size())
            imaginaryPart
                = sum_s(prod_s(foo, argument[i]->getImaginaryPart()),
...
optional<Expr> Prod::getHermitianConjugate(const Space *space) const
{
    return getHermitianConjugate(vector<const Space *>(1, space));
}

optional<Expr>
Prod::getHermitianConjugate(const vector<const Space *> &space) const
{
    vector<optional<Expr>> newArg(argument.size());
    for (size_t i = 0; i != argument.size(); ++i) {
...
optional<Expr> Prod::findSubExpression(Expr_info   subExpression,
...
void Prod::printCode(int, std::ostream &out) const
{
    out << "csl::prod_s({";
    for (size_t i = 0; i != argument.size(); ++i) {
...
void Prod::leftInsert(const Expr &expr)
{
    ...
    for (size_t i = 0; i < max; i++)
        if (not option::checkCommutations or expr < argument[i]
            or *Commutation(expr, argument[i]) != CSL_0) {
...
optional<Expr> Prod::evaluate(csl::eval::mode user_mode) const
{
    bool             number       = true;
...
bool Prod::mergeTerms()
{
    ...
    for (size_t i = 0; i + 1 < argument.size(); i++) {
        ...
        if (csl::IsNumerical(argument[i])
            || csl::IsNumerical(argument[i + 1])) {
            matched = false;
            continue;
        }
        if (not matched) {
        ...
        if (*term == term2) {
            matched = true;
            factor  = factor->addition_own(
                factor2); // factor->addition_own(factor2);
            // if (term->isInteger()
...
void Prod::orderTerms()
{
    ...
    for (size_t i = 0; i != argument.size(); i++) {
        if (argument[i]->getType() == csl::Type::Prod) {
            // csl::vector_expr newArg(argument.size() + argument[i]->size() -
        ...
        if (simpler != i) {
            if (not GetCommutable(argument[simpler])) {
                ...
                for (size_t j = i + 1; j < simpler; ++j)
                    if (option::checkCommutations
                        and Commutation(argument[simpler], argument[j])
                                != CSL_0) {
                        commutes = false;
                        break;
                    }
...
optional<Expr> Prod::expand(bool full, bool inPlace) const
{
    return expand_if([&](Expr const &) { return true; }, full, inPlace);
}

optional<Expr> Prod::expand_if(std::function<bool(Expr const &)> const &f,
                               bool                                     full,
                               bool inPlace) const
{
    Expr copy = Copy(this);
    ...
    for (const auto &arg : copy) {
        if (csl::IsPow(arg) and arg[1]->isInteger()
    ...
    for (size_t i = 0; i != args.size(); ++i) {
        if (args[i]->getType() == csl::Type::Sum and f(args[i])) {
            expanded              = true;
    ...
    std::optional<Expr> expandedDenom
        = (denom.empty()) ? std::nullopt
...
bool Prod::partialComparison(Expr_info expr) const
{
    ...
    for (size_t i = 0; i < argument.size(); i++) {
        ...
        for (size_t j = 0; j < indicesLeft.size(); j++) {
            ...
            if (option::checkCommutations
                and *Commutation(argument[i], foo) != CSL_0)
                break;
            if (*argument[i] == foo) {
                indicesLeft.erase(indicesLeft.begin() + j);
                matched = true;
...
void applyOperator(Expr &product)
{
    CSL_ASSERT_SPEC(product->getType() == csl::Type::Prod,
                    CSLError::TypeError,
                    "should get a csl::Prod, " + toString(product->getType())
                        + " given.");
    ...
    for (auto arg = argument.begin(); arg != argument.end() - 1; ++arg) {
        if ((*arg)->isAnOperator() and (*arg)->isEmpty()) {
            Expr right
                = prod_s(csl::vector_expr(arg + 1, argument.end()), true);
            if ((*arg)->operatorAppliesOn(right.get())) {
                product = CSL_1;
                if (arg != argument.begin())
                    product = prod_s(csl::vector_expr(argument.begin(), arg),
                                     true);
            ...
            //    right = prod_s(csl::vector_expr(arg+1, argument.end()),
            //    true);
            ...
            bool        explicitProd)
{
    ...
    //        {
    //    if (not indexed)
    //        return csl::make_shared<Prod,
    //        alloc_prod>(csl::vector_expr({leftOperand,rightOperand}),true);
    //    else
    //        return
    //        csl::make_shared<IProd>(csl::vector_expr({leftOperand,rightOperand}),true);
    //}
    if (not explicitProd and leftOperand->isAnOperator()
        and leftOperand->isEmpty()
    ...
    if (lType == csl::Type::Polynomial) {
        if (rType == csl::Type::Polynomial and explicitProd) {
            if (not indexed)
                return csl::make_shared<Prod, alloc_prod>(leftOperand,
                                                          rightOperand);
            else
                return csl::make_shared<IProd, alloc_iprod>(leftOperand,
                                                            rightOperand);
        }
        return leftOperand->multiplication_own(rightOperand);
    }
    ...
    if (csl::option::canonicalSumNumericalFactor and lType == csl::Type::Sum
    ...
    if (csl::option::canonicalSumNumericalFactor and rType == csl::Type::Sum
...
{
    const auto rPType = rightOperand->getPrimaryType();
    if (rPType == csl::PrimaryType::Numerical and rightOperand == CSL_0) {
    ...
    if (lPType == csl::PrimaryType::Numerical
        ...
        if (rightOperand->getType() == csl::Type::IntFraction)
            return leftOperand->multiplication_own(intfraction_s(
                rightOperand->getDenom(), rightOperand->getNum()));
    ...
    if (rightOperand->isInteger())
        return prod_s(intfraction_s(1, rightOperand->evaluateScalar()),
                      leftOperand);
...
Pow::Pow(const Expr &leftOperand, const Expr &rightOperand, bool explicitPow)
    : AbstractDuoFunc()
{
    argument[0] = leftOperand;
    argument[1] = rightOperand;
...
Expr Pow::getNumericalFactor() const
{
    if (argument[1]->isInteger()) {
        if (Expr factor = argument[0]->getNumericalFactor(); factor != CSL_1) {
...
csl::vector_expr Pow::getFactors() const
{
    ...
    if (argument[1]->getType() == csl::Type::Integer
        or argument[1]->getType() == csl::Type::Float) {
        ...
        for (int delta_num = 0; sgn(num + delta_num) == sgn(num);
             delta_num -= sgn(num) * denom)
            foo.push_back(
                pow_s(argument[0], intfraction_s(num + delta_num, denom)));

        if (PGCD(num, denom) != 1) {
            for (int i = sgn(num); abs(i) < abs(num * 1.) / denom;
                 i += sgn(num))
                foo.push_back(pow_s(argument[0], int_s(i)));
        }
    }
...
bool Pow::askTerm(Expr_info expr, bool exact) const
{
    ...
    if (expr->getType() == csl::Type::Pow) {
        ...
        if (a->getPrimaryType() != csl::PrimaryType::Numerical
            or b->getPrimaryType() != csl::PrimaryType::Numerical)
            return false;
        if (a->getType() == csl::Type::Complex
            or b->getType() == csl::Type::Complex)
            return false;

        if (a->getType() == csl::Type::Integer
            or a->getType() == csl::Type::Float) {
            ...
            double a_val   = a->evaluateScalar();
            double b_val   = b->evaluateScalar();
            return (a_denom == b_denom and abs(a_val) < abs(b_val)
                    and sgn(a_val) == sgn(b_val));
        }
        return ((*this) == expr);
    }
    else
        return (*argument[0] == expr
                and (argument[1]->isInteger()
                     or argument[1]->getType() == csl::Type::IntFraction)
                and argument[1]->evaluateScalar() > 0);
}

Expr Pow::suppressTerm(Expr_info expr) const
{
    // Caution here: we do not re-test if the term corresponds (we do not call
    // askTerm we suppose it has been done before calling this function...)
    if (expr->getType() == csl::Type::Pow) {
        return pow_s(argument[0], minus_(argument[1], expr->getArgument(1)));
    }
    else
        return pow_s(argument[0], minus_(argument[1], CSL_1));
}

std::optional<Expr> Pow::suppressExponent(Expr const &factor,
                                          Expr const &exponent) const
{
    return (factor == argument[0] and exponent == argument[1])
               ? std::optional<Expr>(CSL_1)
               : std::nullopt;
}
...
void Pow::printCode(int, std::ostream &out) const
{
    out << "csl::pow_s(";
    for (size_t i = 0; i != size(); ++i) {
...
optional<Expr> Pow::evaluate(csl::eval::mode user_mode) const
{
...
{
    return csl::make_shared<Pow, alloc_pow>(
        argument[0]->deepCopy(), argument[1]->deepCopy(), true);
}
...
{
    return pow_s(argument[0]->deepRefresh(), argument[1]->deepRefresh());
}
...
bool Pow::mergeTerms()
{
    ...
    if (argument[0]->getType() == csl::Type::Pow) {
    ...
    else if (argument[0]->isInteger() and argument[1]->isInteger()
        ...
        argument[1] = CSL_1;
             ...
             and argument[1]->evaluateScalar() < 0) {
        argument[0]
            = intfraction_s(argument[0]->getDenom(), argument[0]->getNum());
        ...
        if (argument[0]->getNum() > 0 and round(valueNum) == valueNum
        ...
        const long long denom
            = round(csl::pow_s(argument[0]->getDenom(), argument[1])
                        ->evaluateScalar());
        argument[0] = csl::intfraction_s(num, denom);
        argument[1] = CSL_1;
    }
    else if (argument[0]->getPrimaryType() == csl::PrimaryType::Numerical
             and argument[1]->getType() == csl::Type::IntFraction
             and std::abs(argument[1]->getNum()) == 1
             and argument[1]->getDenom() == 2) {
        if (argument[0]->isInteger()) {
            double value = sqrt(argument[0]->evaluateScalar());
            if (value == round(value)) {
                argument[0] = (argument[1]->getNum() < 0)
                                  ? intfraction_s(1, value)
                                  : int_s(value);
                argument[1] = CSL_1;
            }
        }
...
optional<Expr> Pow::expand(bool full, bool inplace) const
{
    if (full) {
        ...
        if (foo2->isInteger() and foo2->evaluateScalar() > 0
                              ...
                              bool inplace) const
{
    if (full) {
        ...
        Expr foo2 = opt2.value_or(argument[1]);
        if (foo2->isInteger() and foo2->evaluateScalar() > 0
            and foo1->getType() == csl::Type::Sum and f(foo1)) {
            double value  = foo2->evaluateScalar();
            int    iMax   = round(abs(value));
            Expr   result = CSL_1;
            for (int i = 0; i < iMax; i++)
...
{
    if (*argument[0] == t_variable and argument[1]->isInteger()
        and argument[1]->evaluateScalar() == order)
        return CSL_1;
    else if (order == 0)
        return nullopt;
    else
...
Expr pow_s(const Expr &leftOperand, const Expr &rightOperand)
{
    ...
    if (*leftOperand == CSL_I and (rightOperand->isInteger())) {
    ...
    if (leftOperand->isIndexed() and rightOperand->isInteger()
    ...
    if (*arg0 == CSL_0 and arg1->getType() == csl::Type::Complex) {
        return CSL_UNDEF;
    }
    if (*arg1 == CSL_0) {   // x^0 = 1
        if (*arg0 == CSL_0) // 0^0
    ...
    : AbstractMultiFunc()
{
    ...
    if (expr->getType() == csl::Type::Polynomial
        and *t_variable != expr->getVariable())
    ...
    if (exprBis->getType() == csl::Type::Sum) {
        ...
        for (int i = 0; i < exprBis->getNArgs(); i++) {
            ...
            if (argument_order > max_order)
                for (int i = 0; i < argument_order - max_order; i++)
                    argument.push_back(CSL_0);
            argument[argument_order] = sum_s(
                argument[argument_order],
                GetPolynomialTerm(foo, t_variable.get(), argument_order));
        }
    }
    else {
        ...
        for (int i = 0; i < max_order + 1; i++)
            argument.push_back(CSL_0);
        argument[max_order]
            = GetPolynomialTerm(exprBis, t_variable.get(), max_order);
    }
    mergeTerms();
}

Polynomial::Polynomial(const csl::vector_expr &terms, const Expr &t_variable)
    : AbstractMultiFunc()
{
    argument = terms;
    variable = t_variable;
...
void Polynomial::printCode(int, std::ostream &out) const
{
...
long double Polynomial::evaluateScalar() const
{
    ...
    for (size_t i = 0; i < argument.size(); i++) {
        if (argument[i]->getDim() > 0)
            return 0;
        sum += argument[i]->evaluateScalar()
               * pow(variable->evaluateScalar(), i);
    }
    return sum;
}

optional<Expr> Polynomial::evaluate(csl::eval::mode user_mode) const
{
...
Expr Polynomial::multiplication_own(const Expr &expr, bool side) const
{
    ...
    // side = true;
    if (*variable == foo->getVariable()) {
        csl::vector_expr foo_argument(argument.size() + foo->size() - 1,
                                      CSL_0);
        for (size_t i = 0; i < foo->size(); i++) {
            if (option::checkCommutations
                and *Commutation(variable, foo->getArgument(i)) != CSL_0) {
                if (side)
                    return prod_s(getRegularExpression(),
                                  foo->getRegularExpression());
                else
                    return prod_s(foo->getRegularExpression(),
            ...
            for (size_t j = 0; j < argument.size(); j++) {
                ...
                if (side)
                    foo_argument[i + j]
                        = sum_s(foo_argument[i + j],
                                prod_s(argument[j], foo->getArgument(i)));
                else
                    foo_argument[i + j]
                        = sum_s(foo_argument[i + j],
                                prod_s(foo->getArgument(i), argument[j]));
            }
        }
...
{
    Expr foo = Copy(expr);
    if (foo->getType() != csl::Type::Polynomial
        or *foo->getVariable() != variable)
    ...
    */
    ...
    int  iter    = 0;
    int  maxiter = 10;
    while (*r != CSL_0 and r->getNArgs() >= foo->getNArgs()
           and iter < maxiter) {
        // cout<<"q = "; q->print();
        // cout<<"r = "; r->print();
...
optional<Expr> Polynomial::factor(bool) const
{
    if (argument.size() == 3) {
        ...
        Expr delta
            = sum_s(pow_s(argument[1], CSL_2),
                    prod_s(prod_s(int_s(-4), argument[0]), argument[2]));
        Expr mean  = prod_s(CSL_M_1, argument[1]);
        Expr diff  = sqrt_s(delta);
        Expr root1 = fraction_s(sum_s(mean, diff), prod_s(CSL_2, argument[2]));
...
Expr polynomial_s(const Expr &expr, const Expr &t_variable)
{
    if (expr->getPrimaryType() == csl::PrimaryType::Vectorial) {
        ...
        for (int i = 0; i < expr->getNArgs(); i++)
            foo->setArgument(polynomial_s(expr->getArgument(i), t_variable),
                             i);
        return foo;
    }
...
void Derivative::printCode(int, std::ostream &out) const
{
...
optional<Expr> Derivative::evaluate(csl::eval::mode user_mode) const
{
    if (argument[0]->getType() == csl::Type::Prod) {
        ...
        for (size_t i = 0; i != t_argument.size(); ++i) {
            ...
            t_argument[i]->setArgument(
                derivative_s(argument[0]->getArgument(i), argument[1], 1), i);
        }
        if (order > 1)
            return derivative_s(sum_s(t_argument), argument[1], order - 1)
                ->evaluate(user_mode);
        else
    ...
    return derivative_s(Evaluated(argument[0]), Evaluated(argument[1]), order);
}

unique_Expr Derivative::copy_unique() const
{
    return make_unique<Derivative>(argument[0], argument[1], order, empty);
}

Expr Derivative::deepCopy() const
{
    return csl::make_shared<Derivative>(
        argument[0]->deepCopy(), argument[1]->deepCopy(), order, empty);
}

Expr Derivative::refresh() const
{
    return derivative_s(argument[0], argument[1], order, empty);
}

Expr Derivative::deepRefresh() const
{
    return derivative_s(
        argument[0]->deepRefresh(), argument[1]->deepRefresh(), order, empty);
}
...
bool Derivative::operator==(Expr_info expr) const
{
    ...
    if (expr->getType() != csl::Type::Derivative)
        return false;
    if (*argument[0] != expr->getOperand()
        or *argument[1] != expr->getVariable() or order != expr->getOrder())
        return false;
                  ...
                  bool        t_empty)
{
    ...
    if (leftOperand->getPrimaryType() == csl::PrimaryType::Vectorial) {
        ...
        for (int i = 0; i != res->getNArgs(); ++i)
            res->setArgument(
                derivative_s(
                    leftOperand->getArgument(i), rightOperand, order, t_empty),
                i);
    ...
    if (rightOperand->getPrimaryType() == csl::PrimaryType::Vectorial) {
        ...
        for (int i = 0; i != res->getNArgs(); ++i)
            res->setArgument(
                derivative_s(
                    leftOperand, rightOperand->getArgument(i), order, t_empty),
                i);
    ...
    case csl::Type::Derivative:
        if (rightOperand == leftOperand->getVariable())
            ...
            return csl::make_shared<Derivative>(
                leftOperand, rightOperand, order, t_empty);
        break;
...
void Integral::printCode(int, std::ostream &out) const
{
    out << "csl::integral_s(";
    for (size_t i = 0; i != size(); ++i) {
        argument[i]->printCode(1, out);
...
optional<Expr> Integral::evaluate(csl::eval::mode user_mode) const
{
    ...
    //              foo->isEmpty(), foo->getInfBoundary(),
    //              foo->getSupBoundary()).
    //                  evaluateIntegral();
    //  return foo->evaluate();
}
...
{
    return make_unique<Integral>(argument[0], argument[1], empty, inf, sup);
}
...
{
    return integral_s(argument[0], argument[1], empty, inf, sup);
}
...
{
    return AbstractDuoFunc::dependsOn(expr) or inf->dependsOn(expr)
...
bool Integral::operator==(Expr_info expr) const
{
    ...
    if (expr->getType() != csl::Type::Integral)
        return false;
    if (*argument[0] != expr->getOperand()
        or *argument[1] != expr->getVariable())
        return false;
...
Expr integral_s(const Expr &leftOperand, const Expr &rightOperand)
{
    ...
    //         argument[i] = integral_s(leftOperand,
    //         rightOperand->getArgument(i));
                ...
                bool        t_empty)
{
    ...
    if (leftOperand->getPrimaryType() == csl::PrimaryType::Vectorial) {
        ...
        for (int i = 0; i != res->getNArgs(); ++i)
            res->setArgument(
                integral_s(leftOperand->getArgument(i), rightOperand, t_empty),
                i);
    ...
    //     for (int i = 0; i != rightOperand->getNArgs(); ++i) {
    //         Expr foo =
    //         csl::make_shared<Integral>(rightOperand->getArgument(i)); res =
    //         foo->applyOperator(res,t_empty);
    //     }
    //     return res;
    // }
    if (not leftOperand->dependsOn(rightOperand.get())
                ...
                const Expr &t_sup)
{
    ...
    if (leftOperand->getPrimaryType() == csl::PrimaryType::Vectorial) {
        Expr res = vectorialtensor_s(leftOperand->getShape());
        for (int i = 0; i != res->getNArgs(); ++i)
            res->setArgument(integral_s(leftOperand->getArgument(i),
                                        rightOperand,
                                        t_empty,
                                        t_inf,
                                        t_sup),
    ...
    Expr res
        = csl::make_shared<Integral>(CSL_1, rightOperand, true, t_inf, t_sup);
    return res->applyOperator(leftOperand, t_empty);
}
