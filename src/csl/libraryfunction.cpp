...
std::string LibFunction::getTensorName(csl::Parent const &tensor) const
{
    if (tensor->getType() == cslParent::Metric
        ...
        return std::string(tensor->getName()) + "_"
               + LibraryGenerator::regularName(
                   tensor->getSpace()[0]->getName());
                        ...
                        std::string const &initInstruction) const
{
    ...
    if (header)
        ...
        for (const auto &param : parameters) {
            out << LibraryGenerator::indent(1) << "const " << param.type << ' '
...
{
    out << (group->hasComplexReturn() ? LibraryGenerator::complexUsing
...
void LibFunction::sortParameters(std::vector<LibParameter> &parameters,
                                 int                        tensorParameter)
{
    ...
    if (static_cast<size_t>(tensorParameter) < parameters.size()) {
        std::sort(
            parameters.begin(), parameters.begin() + tensorParameter, comp);
        std::sort(
            parameters.begin() + tensorParameter, parameters.end(), comp);
    }
    else
