 ...
 * }
 * \endcode
 * In this example, consider the expression \f$ A\left(1 + \cos \frac{2\pi
 * t}{T} \right) \f$. If you call a node algorithm on it, the function you pass
 * will be called with successively:
 ...
 * \note The order of call is reversed for the Transform() algorithm that
 * begins with the leafs of the tree. \note When giving a function that
 * concerns only leafs, use a leaf algorithm. It may be considerably faster if
 * both the initial expression and the given function are complicated.
 */
