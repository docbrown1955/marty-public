...
double getTotalLength(double curvature,
                      double length)
{
    if (curvature == 0.)
        return length;
...
void applyCurvature(QPainterPath& path,
                    double        curvature,
                    double        length)
{
    if (curvature == 0.)
    ...
    for (int i = 0; i != N; ++i) {
        ...
        double theta = (L-point.x())/L * (theta_max - theta_min) + theta_min;
        double y = point.y();
        QPointF newPoint(length/2 + std::cos(theta)/c + y*std::cos(theta),
