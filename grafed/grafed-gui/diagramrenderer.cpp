...
void DiagramRenderer::setTable()
{
    ...
    auto graph = new DiagramWidget(
                scene, 1000*iCol, 1000*iRow, this, diagrams[indexDiagram]);
...
bool DiagramRenderer::setDiagram(qint32 i)
{
    ...
    diag = new DiagramWidget(
                scene, 1000*iCol, 1000*iRow, this, diagrams[i]);
    diag->setSizePolicy(
...
std::pair<Diagram*, DiagramWidget*> DiagramRenderer::generateDiagram(
        ...
        qint32 j)
{
    Diagram *diagram = new Diagram(scene, 1000 * j , 1000 * i, this);
    ...
    if (ret == QMessageBox::Yes) {
        ...
        for (int i = 0; i != diagrams.size(); ++i) {
            if (iCol != iColNew or iRow != iRowNew) {
                if (diagrams[i]) {
                    diagrams[i]->moveTo(1000 * iColNew, 1000 * iRowNew);
                    auto graph = new DiagramWidget(
                                scene,
                                1000 * iColNew,
                                1000 * iRowNew,
