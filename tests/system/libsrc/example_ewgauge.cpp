...
initKinematics(param_t &param, double ECM, double theta)
{
    Kinematics<2, 2> kinematics((double) param.M_W,
                                (double) param.M_Z,
                                (double) param.M_W,
                                (double) param.M_Z,
                                ECM,
...
{
    return 1 / (32 * M_PI * M_PI * ECM * ECM) * std::sqrt(std::abs(pf2 / pi2))
           * M2_int;
}

template <class Func>
double sigma(Func &&f, param_t &param, double E_CM, size_t Npts = 100)
{
    double integ = 0;
    for (size_t i = 0; i != Npts; ++i) {
        ...
        integ
            += xsec(std::sin(theta) * (double) REAL(f(param)), E_CM, pi2, pf2);
    }
    return integ;
}
