...
int main() {
    ...
    std::ofstream file("C7_marty_tanb50_ss.txt");
    file.precision(10);
    file << std::left;
    for (size_t i = 0; i != nPoints; ++i) {
        for (size_t j = 0; j != nPoints; ++j) {
            ...
            const __complex128 c7 = C7_mu(M2, mu);
            file << std::setw(20) << (double)M2;
            file << std::setw(20) << (double)mu;
            file << std::setw(20) << (double)crealq(c7);
            file << std::setw(20) << (double)cimagq(c7);
            file << std::endl;
        }
    }
    file.close();
