#ifndef MTY_VLQ_MODEL_H_INCLUDED
#define MTY_VLQ_MODEL_H_INCLUDED
...
class VectorLike_Model : public mty::SM_Model {
  ...
  public:
    ...
    void addVectorLike(std::string const &name,
                       int                irrepSU2,
...
std::vector<std::string> VectorLike_Model::getVLQMasses(Model const &model)
{
...
VectorLike_Model::VectorLike_Model(Model const &model) : SM_Model(false)
{
    initContent();
    if (model.model5)
...
void VectorLike_Model::addVectorLike(std::string const &name,
                                     int                irrepSU2,
                                     csl::Expr const   &hyperCharge,
                                     csl::Expr const   &mass)
{
    HEPAssert(csl::IsInteger(hyperCharge) || csl::IsIntFraction(hyperCharge),
              ...
              "Expecting integer or rational for the hypercharge, "
                  + toString(hyperCharge) + " given.") const int num
        = hyperCharge->getNum();
    const int denom = hyperCharge->getDenom();

    mty::Particle VLQ_L
        = mty::weylfermion_s(name + "_L", *this, mty::Chirality::Left);
    mty::Particle VLQ_R
        = mty::weylfermion_s(name + "_R", *this, mty::Chirality::Right);
        ...
        if (num == 2 && denom == 3) { // Tu
            ...
            addLagrangianTerm(
                -2 * LTd(I) / sqrt2 * GetComplexConjugate(QL({I, c, i[0], a}))
                    * VLQ_R({c, p, a}) * sig_half({p, i[0], i[1]}) * H(i[1]),
                true);
        }
    }
    if (irrepSU2 == 1) {
        if (num == 1 && denom == 6) {
            ...
            addLagrangianTerm(-Ld(I) * GetComplexConjugate(VLQ_L({c, i[0], a}))
                                  * DR({I, c, a}) * H(i[0]),
                              true);
            addLagrangianTerm(-Lu(I) * GetComplexConjugate(VLQ_L({c, i[0], a}))
            ...
            addLagrangianTerm(-Lx(I) * GetComplexConjugate(VLQ_L({c, i[0], a}))
                                  * UR({I, c, a}) * H(i[0]),
                              true);
        }
            ...
            addLagrangianTerm(-Ly(I) * GetComplexConjugate(VLQ_L({c, i[0], a}))
        ...
        if (num == -1 && denom == 3) {
            ...
            addLagrangianTerm(-L5d(I)
                                  * GetComplexConjugate(QL({I, c, i[0], a}))
                                  * VLQ_R({c, a}) * H(i[0]),
                              true);
        }
