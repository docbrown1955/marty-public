...
void addSquaredAmplitude(mty::Model                   &model,
                         mty::Library                 &library,
                         std::vector<Insertion> const &insertions)
{
    auto ampl = model.computeAmplitude(Order::TreeLevel, insertions);
    if (ampl.empty())
        return;
...
int main()
{
    ...
    toyModel.addLagrangianTerm(
        -cc(Li({I, i, a})) * eps({i, j}) * Ye({I, J}) * Ei({J, a}) * cc(H(j)),
        true // Add also the complex conjugate of this term
    );

    toyModel.addLagrangianTerm(
        -H(i) * cc(Li({I, i, a})) * YN({I, J}) * Ni({J, a}),
        true // Add also the complex conjugate of this term
    );

    toyModel.addLagrangianTerm(
        -cc(Si({I, a})) * MNs({I, J}) * Ni({J, a}),
        true // Add also the complex conjugate of this term
    );
    toyModel.addLagrangianTerm(-sig * cc(XL({a})) * YXL({I}) * Ni({I, a}),
                               true);
    toyModel.addLagrangianTerm(-cc(sig) * cc(Si({I, a})) * YXR({I}) * XR({a}),
                               true);
    ...
    mty::Library myLib("NLepto", "libs");

    std::vector<Particle> N = toyModel.getPhysicalParticles([&](Particle p) {
        return p->getSpinDimension() == 2 // fermion
               && p->getName()[0] == 'N'; // name starts with 'N'
    });
    std::vector<Particle> L = toyModel.getPhysicalParticles([&](Particle p) {
        return p->getSpinDimension() == 2 // fermion
               && p->getName()[0] == 'L'; // name starts with 'L'
    });

    addSquaredAmplitude(toyModel,
                        myLib,
                        {Incoming("ss"),
                         Incoming(AntiPart("ss")),
                         Outgoing("H"),
                         Outgoing(AntiPart("H"))});
    ...
    for (size_t kk = 0; kk != 1; ++kk) {
        // ss --> X Li H^* , (ss, X) conjugated or not, (Li, H^*) conjugated or
        // not
