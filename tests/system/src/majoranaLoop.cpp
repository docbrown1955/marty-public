...
void Check(std::vector<csl::Expr> const &ampl)
{
    for (const auto &a : ampl)
        if (csl::AnyOfLeafs(a, [&](csl::Expr const &sub) {
                return csl::IsITensor(sub) and mty::dirac4.isGammaTensor(sub);
            })) {
            if (csl::IsSum(a))
...
int main()
{
    ...
    csl::Expr     SP_term
        = CSL_HALF * Q * Q * csl::pow_s(cc(S) * P - cc(P) * S, 2);
    csl::Expr psiL_term
        = CSL_I * Q * cc(psi(a1))
          * (S * delta({a1, a2}) + CSL_I * gamma5({a1, a2}) * P) * lambda(a2);
    model.addLagrangianTerm(SP_term);
    model.addLagrangianTerm(psiL_term, true);

    model.addLagrangianTerm(CSL_HALF * csl::GetComplexConjugate(lambda(a1))
                            * A(mu) * gamma({+mu, a1, a2}) * lambda(a2));
    ...
    auto majoranaLoop = model.computeAmplitude(mty::Order::OneLoop,
                                               {Incoming(S),
                                                Incoming(AntiPart(S)),
                                                Outgoing(S),
                                                Outgoing(AntiPart(S))});
        ...
        auto OK = [&](csl::Expr const &expr) {
            return csl::AllOfLeafs(expr, [&](csl::Expr const &sub) {
                return !csl::IsIndicialTensor(sub)
                       || bool(
                           dynamic_cast<mty::QuantumField const *>(sub.get()));
            });
        };
        if (!OK(diagExpr)) {
    ...
    majoranaLoop = model.computeAmplitude(mty::Order::OneLoop,
                                          {Incoming(P),
                                           Incoming(AntiPart(P)),
                                           Outgoing(P),
                                           Outgoing(AntiPart(P))});
