...
{
    const auto ampl
        = (loop) ? model.computeAmplitude(Order::OneLoop, insertions)
...
int main()
{
    ...
    TIDM.addLagrangianTerm(m2H * cc(H(i))
                           * H(i)); // Sign for Mexican hat potential
    ...
    TIDM.addLagrangianTerm(mp * cc(phi)
                           * phi); // Sign for Mexican hat potential
    ...
    TIDM.addLagrangianTerm(-lAB1 * cc(etaA(i)) * etaA(i) * cc(etaB(j))
                           * etaB(j));
    TIDM.addLagrangianTerm(
        -lAB2 * cc(etaA(i)) * etaB(i) * cc(etaA(j)) * etaB(j), true);
    TIDM.addLagrangianTerm(
        -lAB3 * cc(etaA(i)) * etaA(i) * cc(etaA(j)) * etaB(j), true);
    TIDM.addLagrangianTerm(
        -lAB4 * cc(etaB(i)) * etaB(i) * cc(etaB(j)) * etaA(j), true);

    TIDM.addLagrangianTerm(-lHA1 * cc(H(i)) * H(i) * cc(etaA(j)) * etaA(j));
    TIDM.addLagrangianTerm(-lHB1 * cc(H(i)) * H(i) * cc(etaB(j)) * etaB(j));
    TIDM.addLagrangianTerm(-lHAB1 * cc(H(i)) * H(i) * cc(etaA(j)) * etaB(j),
                           true);

    TIDM.addLagrangianTerm(-lHA3 * cc(H(i)) * etaA(i) * cc(etaA(j)) * H(j),
                           true);
    TIDM.addLagrangianTerm(-lHB3 * cc(H(i)) * etaB(i) * cc(etaB(j)) * H(j),
                           true);
    TIDM.addLagrangianTerm(-lHAB3 * cc(H(i)) * etaA(i) * cc(etaB(j)) * H(j),
                           true);
    ...
    TIDM.addLagrangianTerm(-CSL_HALF * YX * phi * XL(a) * C({a, b}) * XL(b),
                           true);
    TIDM.addLagrangianTerm(
        -CSL_HALF * YXc * cc(phi) * XR(a) * C({a, b}) * XR(b), true);
    ...
    TIDM.addLagrangianTerm(-cc(QL({I, A, i, a})) * Yu({I, J}) * UR({J, A, a})
                               * eps({i, j}) * cc(H(j)),
                           true // Add also the complex conjugate of this term
    );
    TIDM.addLagrangianTerm(
        -cc(QL({I, A, i, a})) * Yd({I, J}) * DR({J, A, a}) * H(i),
        true // Add also the complex conjugate of this term
    );
    TIDM.addLagrangianTerm(-cc(LL({I, i, a})) * Ye({I, J}) * ER({J, a}) * H(i),
                           true // Add also the complex conjugate of this term
    );
    ...
    TIDM.addLagrangianTerm(
        -YX1A(I) * XL(a) * C({a, b}) * LL({I, i, b}) * cc(etaA(i)), true);
    TIDM.addLagrangianTerm(
        -YX1B(I) * XL(a) * C({a, b}) * LL({I, i, b}) * cc(etaB(i)), true);
    ...
    TIDM.diagonalizeYukawa("Y_e",                    // diagonalizing Y_e
    ...
    */
    ...
    auto fermions  = TIDM.getPhysicalParticles(
    ...
    [[maybe_unused]] int sysres = system("rm -rf libs/SM_2IDM");
    mty::Library         TIDMLib("SM_2IDM", "libs");
