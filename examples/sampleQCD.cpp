...
int main() {
    ...
    auto diff = QCD.computeAmplitude(
                Order::TreeLevel,
    ...
    auto _66bar = QCD.computeAmplitude(
                Order::TreeLevel,
                { Incoming(X), Incoming(AntiPart(X)),
    ...
    crossSection = crossSection  / (64*CSL_PI*CSL_PI*s)
        * sqrt_s((1 - 4 * pow_s(m_u, 2) / s)
        / (1 - 4 * pow_s(m_X, 2) / s));
