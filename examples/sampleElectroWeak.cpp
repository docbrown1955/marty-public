...
int main()
{
    ...
    Timer    t;
    auto     WW_to_ZZ = ElectroWeak.computeAmplitude(
        Order::TreeLevel,
        {Incoming(W), Incoming(AntiPart(W)), Outgoing(Z), Outgoing(Z)});
    Display(WW_to_ZZ);
    Show(WW_to_ZZ);

    t.restart();
    auto ZZ_to_ZZ = ElectroWeak.computeAmplitude(
        Order::OneLoop, {Incoming(Z), Incoming(Z), Outgoing(Z), Outgoing(Z)});
    Display(ZZ_to_ZZ);
    Show(ZZ_to_ZZ);
